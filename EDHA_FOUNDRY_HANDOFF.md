...
**(1) The refund race — the mechanism the bench named was RIGHT, and the reason it is nondeterministic is sharper than "a race".** `edhaRefundCost` did a read-modify-write of an absolute clamped value; the question nobody had answered was *why the read is stale*. It is stale **by construction**, and cosmere-rpg 2.1.0's `Item#use()` says so in three lines: the cost deduction is pushed onto the `postRoll` list as `void options.actor.update({… value: currentAmount − consumption.value.actual})` — an **absolute** value computed up front and issued **un-awaited**; the LAST postRoll entry is `Hooks.callAll(HOOKS.USE_ITEM, …)`; and the whole list runs via `postRoll.forEach(a => a())`, i.e. **one synchronous tick**. The `use` item-event registers with no `host`, and `registerItemEventType` defaults `host` to `"source"` — so our executor runs on that same client a few microtasks later, long before the charge's socket round-trip returns. The refund therefore read the PRE-consume number and wrote `cur + amount` as a SECOND absolute value. Whichever landed second won. **The arithmetic reproduces the bench's two numbers exactly** on a 4-max actor paying 2: charge-last gives 4 → 2 ("paid, never refunded"); refund-last gives `Math.min(4, 3 + 2)` = 4 from a start of 3 ("gained 1"). **The fix could not be a delta write, because the cosmere system has no resource API at all** — every resource write in `index.js` is a raw absolute `actor.update`, and `modifyTokenAttribute`'s `isDelta` resolves to an absolute from a client-side read, i.e. the same shape. So the fix is **ordering**: a generic `preUseItem` hook snapshots the pre-cost values (that hook runs before ANY consumption, and had an earlier veto returned false `Hooks.call` would have aborted the use, so there would be nothing to refund), and the refund waits for the charge to LAND against that snapshot before it re-reads and credits. A wait that times out means nothing was charged (`use({shouldConsume: true})` skips consumption entirely in 2.1.0), so it refunds nothing rather than minting. **Family: 29 call sites, ~8 of them racing** — this one plus the no-scene/no-token guards and the ledger-cap refusals refuse in the SAME tick as the use; the other ~21 sit behind a canvas click or a template create and were correct only by the accident of that round-trip. Fixed once, at the helper. ⚠️ **The architecturally better shape, where it fits, is a `preUseItem` VETO** — "nothing spent" has no race at all, and H1/H3/H12/H15/`edha-next-test-mod` all carry one; that is now recorded in ENGINE_INDEX next to the helper. It was NOT applied to Phantom Double here, deliberately: the rule's own description promises a refund, so converting it would drift authored text and turn an engine-only pass into an owed leyline rebuild for zero player-visible difference.

**(2) The doubled dissipates card — the bench's cause was right and its SCOPE was wrong: SIX sites, not one.** Foundry hooks fire on every connected client, so `if (!game.user?.isGM)` means once per GM, and Ben's table has two. The reported branch also deletes the actor and toggles the DEFEATED overlay, so all three happened twice. The sweep found five more: the `edha-zone-react {defeat-in-zone}` ignite sweep (**worse** — every GM posted the card AND dropped its own duplicate hazard **Region**), the summon last-token cleanup (every GM issued `actor.delete()`, which is exactly the server-side "Actor does not exist" race the Illusion section's own hook documents from 07-17), both barrier wall-clear doors, and the ready-time `applyButtonsTo` world setting. All six move onto the existing `edhaDefBuffGmGate()`. The three `render*` hooks that still carry a raw `isGM` are **correct and stay** — injecting a sidebar or sheet button is per-client work.

**(3) The orphaned token — the engine's comment was a lie, and the file had already disproved it 3,700 lines earlier.** Verified in Foundry v13 source rather than inferred: neither the client `Actor` class nor `dist/database/documents/actor.mjs` has any dependent-token delete, so **Foundry never cascades actor → token**. It DOES cascade token → combatant (`TokenDocument._onDeleteOperation` deletes combatants whose `tokenId` went away) and nothing cascades actor → combatant — which is the whole tail the bench reported: the orphan kept a live combatant and Advanced Encounters threw from its `initiative` getter on every later combatant add. **Family of three against two already-correct implementations in the same file**: the illusion "dissipates" branch (reported), the barrier "destroyed" branch one screen below it, and the scene-end barrier sweep on `deleteCombat`; `edhaClearPhantomDoubles` (which carries the 07-17 lesson in a comment) and `edhaCivDismantleGM` had it right. Rather than patch three sites the shape became **`edhaDeleteActorWithTokens`** and all five consumers moved onto it, so "does the actor-deleted path do this correctly?" is now a structural question. The ownership split is preserved exactly: a summon-flagged actor that HAD a token is deleted by the last-token cleanup, never here.

**New REUSABLE primitives:** `edhaDeleteActorWithTokens(actor)` (token-first teardown, with the combatant cascade explained); `edhaAwaitCostCharged(actor, list, base, ms, step)` + the `EDHA_PRE_COST_RES` pre-cost snapshot (the ordering barrier any future refund path gets for free); `edhaRefundCost` is now **async** — await it in new code.

**The gate: `lint-refs` pass 15 — a hook that writes the world may not be gated on a raw `isGM`.** Chosen over gating the refund race, and that choice is worth recording: the refund rule ("no absolute read-modify-write against a system-owned resource") would today be a 14-entry allowlist of legitimate cost sites that mostly encodes "here are the places that do this", whereas the two-GM family has now bitten **four times across two marathons** (Apex Form's double injury, the 07-27b 2bW-13 sweep of ~70 sites, and run 13's card plus the two more its sweep found) and keeps regrowing after each sweep. Pass 15 delimits every top-level hook by paren balance on the string-blanked copy, flags a raw-`isGM` body that performs a database write, exempts `render*`, and self-checks that it delimited a plausible number of hooks. **Mutation-verified against all five sites fixed this pass** — each one, put back, is caught. The refund race gets the other artifact iron rule 4 asks for: `tests/refund-race.test.js`, which pins BOTH directions and, with the ordering removed, reproduces the bench's exact numbers (2 and 4).

**⚑ Could not self-verify (no Foundry here):** every one of the three fixes — all are re-test rows with explicit positive AND negative controls in the player-client window section, plus the rewritten 2bAA-8. The negatives are the load-bearing halves: a successful Phantom Double cast must still CHARGE 2 (an over-firing refund makes every cast free), the DEFEATED skull must still appear (it shares the guard that changed), and deleting a summon's TOKEN must still delete its actor. Two of the six one-applier sites — the ignite Region and the barrier teardown — have **never been benched at all** and get their own row. Nothing to deploy but the engine sync.) Prior: **2026-07-27p** (BENCH RUN 13 — **THE PLAYER-CLIENT WINDOW, OPEN AT LAST: 10 ROWS RETIRED, 2 NEW DEFECTS, 1 FAIL.** Ben's new passwordless PLAYER user **`PlayerBench`** (`yF9LHvfhB7otsHYY`) joined in a second browser-pane tab beside `Bench`, and **the feared cookie displacement did NOT happen** — `Bench`, `Gamemaster` and `PlayerBench` were all active at once, verified from Bench's own live socket. That unblocked a batch that had sat at the bottom of **twelve** solo passes.

**THE FLAGSHIP — the client veil is PROVEN, in all three directions, on three machines.** A fresh `Bench Adv — Mistheron` (Cognitive 14) used The Seeming; the copy spawned adjacent on the HOSTILE side wearing the bird's **plain** token name `Mistheron (3)`, and four PCs rolled Perception vs 14 (Chaos 13 · Sovereignty 5 fooled; Black 19 · Life 18 saw through). Then, narrowing `PlayerBench` to **one** PC at a time to reproduce Ben's one-PC-per-computer design: owning the **fooled** Chaos → the ORIGINAL `isVisible === false` and the COPY `true`; owning the **seer** Black → exactly inverted; the **GM** client → **both** true. Base vision was healthy throughout (Stitchmother, three Thralls and other PCs all rendered), so the suppression was specific. ⚠️ Getting there required finding that **bench PCs carry a 10 ft "sense" vision range**, which makes a player client render almost nothing — the first `isVisible` read was `false` for **all 54** non-owned tokens, and calling that "the veil" would have been a false PASS.

Also retired: **the break** (veil dropped — the real bird returned for the fooled — and "the illusion breaks" posted), **late viewer** (only the newcomer rolled; all four earlier totals unchanged), **ally-targeted double** and **max-1 recast**, **White Draw Mana** (healed an ally `PlayerBench` had *zero* ownership of, [Tier]=2 HP, **no permission error**), **Black Draw Mana** (the 🕵️ GM sweep card never rendered on the player's screen — public card said 5, GM card said 8 with "1 hidden, 2 behind a wall"; and it still posts for a GM casting directly), **sense-through reveals** (Omen-marked token visible to the Void Sense owner, identically-obscured unmarked control invisible), **CAE use-grants** (all three clauses as the player, incl. Through the Fray landing its reaction group on the **targeted ally's** combatant), and the **sync-button** row (GM footer had both buttons, the player's had neither).

**ONE FAIL → test-pass-fixes: 2bAA-8's out-of-range refund does not refund, nondeterministically.** From inv 4 it ended at **2** (paid, never refunded); from inv 3 it ended at **4** (gained 1). `edhaRefundCost` (~L9595) read-modify-writes an ABSOLUTE clamped value that **races the cosmere system's own absolute consume write**. **29 call sites share the helper.** Everything else in 2bAA-8 passed.

**TWO NEW DEFECTS, both found by running the window:** (1) **the dissipates card double-posts with two GMs connected** — two cards 1 ms apart authored by `Bench` and `Gamemaster`, because the `updateActor` branch at ~L9226 is guarded only by `isGM` and not by `activeGM`; the isolating control is the recast break card, which IS activeGM-guarded and posted once in the same session. (2) **deleting a summon/phantom ACTOR leaves its TOKEN behind** — reproduced **three times**, directly contradicting the engine's own comment; the leftover **combatant** then made Advanced Encounters throw from its `initiative` getter on every subsequent combatant add, i.e. one dead summon can wedge the tracker mid-combat.

**GM summon relay = PARTIAL, and the blocker is a world PERMISSION, not code:** `game.user.can("ACTOR_CREATE")` is **true** for the PLAYER role here, so the `summon-actor` relay branch is unreachable and `edhaSummon` ran the direct path on the player's own client. The player DID get a real, owned, movable Construct whose Slam rolled a genuine Athletics test. `actsAfterCaster` is separately unreadable: with Advanced Encounters live, **no** combatant stores a raw initiative. ⚑ Ben's call. **Pointer 8 of the player-client window (Unnerving Approach push relay) was STALE** — its home section no longer exists — and is deleted. World hygiene: **zero** drift in actors, combats, tokens, walls, templates, items, macros, folders and flags; the only effect delta was two Guardian Stance AEs re-minted with new ids by the talent re-sync. Ownership granted to `PlayerBench` on 16 bench PCs and **fully restored**. Nothing to deploy. DOCS-ONLY.) Prior: **2026-07-27o** (BENCH RUN 12 — FIX PASS A VERIFIED IN BOTH DIRECTIONS, AND 10 ROWS RETIRED ON LIVE EVIDENCE, NOTHING FAILED: **Shockwave Slam** now fires off a WEAPON hit ("pushed 10 ft", note off the document, token `_source` moved directly away from the caster — twice, on two axes), and the load-bearing NEGATIVE holds — **Cheap Shot** still does not ride a weapon hit yet still Stuns on its own, proven on the same victim in the same round; Dark Investiture unchanged. Retired: 2bT-19 (cost charged once, H1 captured the roll, exactly one dieStep entry per use), Withering Ray (`2d20kh + 5 + 1d8[Predatory Patience]` — no garble), 2bAC-2 (measured: 660×447, labels and controls on the same row), 2bC-8, and ALL FOUR runnable Order rows — 2bV-2’s forced-slide negative finally SEPARATED from the once-per-round gate, 2bV-6’s three halves, 2bV-8 with both blocks, and the Bear Witness quiet cases. **2bC-8 was never a roster gap** — Probability Net is an ADVERSARY ability, so the standing “grant it in bench-setup-console.js” fix could not have worked. World hygiene: zero drift in ids, flags and EFFECTS across 87 actors. Nothing to deploy. DOCS-ONLY.) Prior: **2026-07-27n** (FIX PASS A of bench marathon 2 — TWO DEFECTS FIXED, INCLUDING THE MARATHON-1 CARRY-OVER: **Sharp Eye** could never roll the test it gates (`activation` was `utility` with no skill, so `use()`'s `rollRequired` was false and H1's queued contest simply expired — verified in system source, DATA → a FIFTH `foundry-build heroic` + ⟳ Sync); and **Shockwave Slam**, open and un-root-caused since bench run 1, had its weapon-hit surface killed by an `itemSpecific` gate that read its COLLISION formula as "this is my own attack" (ENGINE-ONLY → ⟳ sync + F5). Both family sweeps reported: the def-test family is one talent but the SECOND surface of a shape 07-26j already fixed in the builder, so `lint-refs` **pass 14** now gates both; the on-hit family is TWO, and the second (**Volatile Strike**) is left alone as Ben's ruling, settleable from the Events tab via the new `whenDealer` field. Plus a **RETRACTION**: run 11's "the dialog exposes no advantage control at all" is wrong — it is the d20 icon itself, left/right-click, pre-seeded as a CSS class; only the PREVIEW LINE is blind. 306 tests green.) Prior: **2026-07-27m** (BENCH RUN 11 — THE DEPLOY LANDED AND THE BACKLOG CLEARED: the served engine hashed identical to HEAD and all five rebuilt packs were confirmed carrying their fixes, then **12 rows retired on live evidence** — Flamestance's first-ever pass, Confident Command's three enforced skills, Feinting Strike's number, Set at Odds + Synchronized Assault, quarry auto-advantage on **both** the fast-forward AND dialog paths, 2bX-17, Fault Line's Constructs ×3, both 2bAD contest rows, and Flame Surge on both bosses; plus Mender's card + range gate and Herding Antlers. ONE FAIL: Sharp Eye is still a silent no-op and the dead skill key was NOT the whole cause — its `activation` is `utility` with no `activation.skill`, a one-talent authored fix. World hygiene clean: zero effect drift across 87 actors, 46 adversaries synced with none lost. DOCS-ONLY.) Prior: **2026-07-27l** (BENCH RUN 10'S ONE DEFECT FIXED —
...
write-race — three groups, then two-in-one-tick; Pack Hunting's double-dip — one card, one roll).
ONE new engine FAIL: quarry auto-advantage has never applied, root-caused to the single site that
writes `advantageMode = 1` instead of `"advantage"` and skips the `configureDialog` wrapper. ONE
suspected defect RETRACTED after measurement — Stonestance is fine; `deflect.derived` is the wrong
field, read `.value`. A full skill-key audit of all 62 Heroic talents found no dead keys beyond the
8 already fixed. Cleanup exactly clean. Nothing to deploy from this pass.) Prior: **2026-07-27j** (BENCH RUN 9'S
...
*deploy-blocked, not untested*. NOTHING TO DEPLOY FROM THIS PASS (bench only).**

**Why Heroic still is not "done", precisely.** Runs 9 and 10 between them have now driven every
Heroic row the current deploy allows. **16 rows remain and ZERO are runnable today.** The dominant
cause is a single missing step: **8 of the 16 are BLOCKED-ON-DEPLOY on `foundry-build heroic` +
⟳ Sync Talents** — the dead-cosmere-skill-key family fixed in `83c04ea` but never built into the
pack. A fresh console read this run confirmed the live pack still carries every stale key
(`Sharp Eye.skill: "per"`, `Flamestance.whenSkill: "itm"`, `Feinting Strike @skills.itm.rank`,
`Confident Command.skill: "itm, lea, per"`, `Set at Odds`/`Synchronized Assault` `skill: "ldr"`,
Rallying Shout's `@skills.ldr.rank`), so none of those rows were failed against it. **Rebuilding the
heroic pack unblocks 8 rows in one action.** The other 8: 4 are ⚑ design calls that are Ben's alone
(every factual half is now proven), 1 is the new FAIL below, 1 is blocked behind that FAIL (2bX-17),
1 needs a roster change rather than a test (2bC-8 — no bench PC owns Probability Net), and 1 is a
Blue row parked in the section (Probability Cascade).

**Deploy verified BY HASH before anything was driven** — the served `register-skills.js` normalised
CRLF→LF hashed to `35bc9315…f7cad`, byte-identical (1 436 805 B) to `HEAD:module-src/scripts/
register-skills.js`. So the four 07-27j engine fixes were genuinely live, and **two of them are now
table-verified for the first time**:
- **The CAE grant write-race is FIXED (2bE-4).** A combat start with Foresight AND Sidestep owned
  wrote **all three** groups — `base`, `Edha: Foresight`, `Edha: Sidestep (Dodge only)`. Two Through
  the Fray uses fired in one tick wrote **two** groups. Run 9's probe produced two cards and one
  group; the queue keyed on combatant uuid + flag key holds. Later in the run **four** Edha action
  groups coexisted on one combatant (Backstep 1, Quick Analysis 2, Trickster's Hand 2, Cautious
  Advance 2), which is the same guarantee under more pressure.
- **Pack Hunting's double-dip is FIXED (2bO-7).** One banked use, ally Strike on the quarry → attack
  `1d20 + 4 + 3[Pack Hunting]`, damage `1d6 + 4` **clean**, exactly **one** card, flag consumed. The
  `edhaNextModClaimOk` guard was also shown inert for count-1 `test`-only mods four separate times
  (Demonstrative, Shrewd, Overwhelm with Details, Decisive Command all applied normally).
  ⛔ Its count-2 non-regression (Blue's Probability Cascade) could **not** be driven — that chain needs
  an Opportunity plus 1 Investiture, which cannot be forced. Parked as a Blue row.

**⚠️ NEW FAIL — quarry auto-advantage has NEVER applied (ENGINE, one site, named fix).** Seek Quarry's
own card promises "attacks are auto-advantaged". **Four separate attacks on a correctly-marked quarry
all rolled a plain `1d20 + 4`** — never `2d20kh` — with the ledger holding the victim and the `Quarry`
icon on it. Root-caused by probe rather than by reading: a hook probe shows `cosmere-rpg.preAttackRoll`
DOES fire and the roll carries `advantageMode: 1` at both `preAttack` and `attack`, but its `terms`
stay `[Die:1d20, +, 4]` — one d20, no `kh`. `edhaQuarryAdvPreRoll` (~5442–5451) is the **only**
advantage site in the engine that writes the **number `1`** instead of the string `"advantage"`, and
the only one with **no `configureDialog` wrapper** — while the engine's own comment at ~line 314 says
"Dialog rolls: configureDialog OVERWRITES options.advantageMode". Every other site (~476, 528, 3619,
5038, 14434, 15236) does both. Same retired-code smell run 9 named for Flamestance, in the one branch
never converted. → **test-pass-fixes**; blast radius is Seek Quarry, Tagging Shot's placed quarry, and
2bX-17's advantage half.

**⚠️ A SUSPECTED DEFECT RETRACTED AFTER MEASUREMENT (the run-9 discipline, deliberately repeated).**
Stonestance first read as a silent no-op because `system.deflect.**derived**` stayed 0 with the stance
active, even though the effect applied cleanly (`deflect.bonus = 1`, in `appliedEffects`, in
`actor.overrides`). It is **not** a defect: `system.deflect.**value**` goes 0 → **1**, and a control
measurement had 10 impact damage cost **10** HP without the stance and **9** with it. `derived` is the
armour-only sub-field and never folds in `bonus`; the engine reads `.value` (`edhaDeflectOf`). **New
operating rule: read `system.deflect.value`, never `.derived`.** Recorded because the first read would
have shipped a false FAIL into the fix queue.

**A WIDER AUDIT WORTH KEEPING.** Every skill key referenced by all 62 Heroic talents was validated
against `CONFIG.COSMERE.skills`: exactly **9 dead-key sites across 7 talents**, precisely the set
07-27j already fixed. **No new ones**, and `lor` (Overwhelm with Details) is valid. The pending heroic
rebuild closes the entire family at once — this is the measurement that makes "8 rows, one action" a
fact rather than a hope.

**Retired on evidence (12 rows):** 2bE-4 · 2bE-5 (Chain armour, `deflect.value` 2 → **only** Foresight
reached the tracker, Sidestep silent, with Foresight as the positive control in the same combat start)
· 2bO-7 · 2bF-13 ("24 vs SPI 14 — SUCCESS" → Disoriented asserted on the document AND the disadvantage
flag written, no click) · 2bZ-8 (all three halves, including editing `reduceFormula` on the Events tab
and watching the next card change from "reduced by 2" to "reduced by 1") · 2bZ-7's negative
(Resuscitation deleted → a success healed with **no** upsell line) · 2bC-7 (Emotional Overload wrote
to the TARGET, caster's flag null) · 2bN-3's last half (ally's token deleted mid-fight → its Determined
cleared at combat end while a pre-existing one survived) · 2bZ-11 (Cold Eyes end to end on an
adversary-type quarry: note, ledger emptied, icon gone, +1 focus) · the **CAE cluster spot** · the
**Envoy cluster spot** · the **orphan-token combat guard** (the run-1 orphan `Combat Construct`,
resolved **by id**, refused with the exact named toast; combat stayed started). The **Leader command
spot** went 10-of-11 and the **Warrior stances spot** 5-of-6, each now hanging on one blocked item.

**Bench hygiene.** Setup script re-run as a real step: zero ⚠ lines, no creations (idempotent), and
the ranged fixture asserted rather than trusted — `Shortbow.system.attack.type === "ranged"`, range 80.
Cleanup ended **exactly clean**: zero document adds/removes (actors, items, scenes, combats, macros,
folders, tokens, walls, templates), zero effect drift (8 effects, the same 8 as the start snapshot),
zero flag drift, zero resource drift, the moved token returned to its home square, and only Ben's own
combat `BerbNeuXp4iKduef` left standing. **One knowingly-accepted residue:** Resuscitation and Calm
Appeal were deleted and re-created from the pack to drive their two negatives, so those two items carry
new ids on Bench — Heroic; names and the 62-talent count are unchanged. Two restore passes were needed
because the first raced with in-flight engine writes — worth knowing.

**Two new harness lessons.** (1) The browser pane opened at 0×0, so Foundry **never initialised the
canvas** and `canvas.initialize()` then threw a PIXI plugin conflict; `resize_window` to desktop
followed by a **page reload** fixed it — but the reload wipes any in-page snapshot, so take the
snapshot *after*. (2) Under v13 with Advanced Encounters live, `tokDoc.update({x,y})` and
`tokDoc.move()` **both silently no-op** for a combatant with no movement left — they return without
error and the token does not move. `scene.updateEmbeddedDocuments("Token", [...], {teleport: true})`
lands.
