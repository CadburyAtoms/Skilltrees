...
- **Three pins were deliberately FLIPPED once R-72 was answered** (item 47) — do not "restore" them. **Three heal paths still write `hea` outside `edhaHealCutGate`** (regen tick, decay lifesteal, burst heal — which must stay ungated for Raise Dead) and closing that needs a ruling (item 68).
