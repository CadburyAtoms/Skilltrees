...
Backing detail (every session's notes) lives in agent memory `edha-foundry-module-build.md` + `edha-aoe-bursts.md`; this doc is the curated summary. Last update: **2026-07-14o** (PHANTOM DOUBLE / THE SEEMING REWORK — the illusion belief loop; ENGINE + data (blue authored + source + adversaries); **engine synced; pack rebuild PENDING (`foundry-build.js leyline` + `adversaries`) — Foundry open at session end, Ben closes → rebuild → relaunch → re-drag the Mistheron.** Ben's bench verdict: the manual Perception-note version 'needs a rework', spec given in chat; three design calls batch-ruled (caster's COGNITIVE defense — supersedes the card's 'Blue defense'; advantage rider DROPPED — the visibility swap is the payoff; direction-aware visibility approximation ACCEPTED). (1) **The loop** (`edhaCastPhantomDouble` — Phantom Double and The Seeming share it): copy defaults to the caster, targets an ally instead if one is targeted; spawns ADJACENT to the original (`edhaSummon` gains `anchorTok` + `disposition` specs — the Mistheron's copy is hostile-side now, not hardcoded FRIENDLY), 1 HP, max 1, dies on any hit (existing HP-sync). (2) **Belief, engine-rolled** (`edhaPhantomBeliefSweep`, runs on the ACTIVE GM's client via `createToken` — summons can materialize through the GM relay): every enemy token that CAN SEE the copy (edhaCanSee) rolls Perception vs the caster's Cognitive defense (baked into the copy's flags at cast). Fooled = only the copy is real to them; saw = it's empty air. Per-observer results persist on the copy's `phantomBelief` flag; the GM gets the full accounting card with a **re-test button** (`edha-illusion-retest` — rolls only not-yet-tested viewers, for late arrivals); player-owned observers are whispered their own truth; the public card carries counts only. (3) **THE CLIENT VEIL** (Ben upgraded the ask mid-session: one PC per computer, GM on his own machine → per-viewer = per-client): a `Token#isVisible` getter wrap (proto-chain descriptor walk, fail-open) filters each PLAYER client's canvas through the belief flag — a fooled player's client does not render the ORIGINAL, a seer's client does not render the COPY, the GM renders everything; NO token document is ever hidden, so nothing can desync (the veil lives and dies with the copy's flags; belief writes/copy deletion trigger `canvas.perception.update` on every client). Pure decision `edhaPhantomVeilHides` pinned in tests (34 green). Also: the copy's TOKEN label was "<name> (Illusion)" — a giveaway to fooled players; `edhaSummon` gains `tokenName` and the copy's token now wears the original's plain name (the actor keeps "(Illusion)" for the GM directory). (4) **The break**: copy death OR deletion (deleteActor/deleteToken, restore-guard set) un-hides the original and announces it. (5) **Data all three layers**: Phantom Double card (authored leyline-blue ×3 fields + leyline.json prose; tags drop 'advantage'), The Seeming trait→**1-Action item** running the same loop (self-only, ruling-40 adaptation framing kept), Spearing Beak rider re-worded ('taken in by the seeming'), Fade's stale 're-hiding restores the seeming' + a mojibake em-dash fixed. ⚑ Bench (after rebuild): PC Phantom Double (copy adjacent, GM accounting card + re-test button, no hiding), Mistheron Seeming vs the party (all-fail hides the bird, mixed whispers, copy dies → bird reappears), ally-targeted double, max-1 recast.)
