...
- [ ] **2. edha-items mirror** (gate: the #0 paste). Copy the shipped gear into
...
- [ ] **3. Country-of-origin culture items** (gate: the #0 schemas + a lore-forge-gated
  content walk — Ben 2026-07-18: ancestry is REPLACED by country-of-origin). One `culture`-type
  item per nation (native system type, confirmed in itemTypes): a per-nation **expertise list
  to pull from**, flavor text, and the primer's "who you might be". Expertise lists are
  invented content → derive from §5b, walk with Ben, THEN build. Also settle whether the sheet
  demands an ancestry item anyway (a trivial Human default if so — bench/dump will tell).
- [ ] **4. Heroic starting-kit wiring** (gate: #2 — kits grant OUR items). Per-path kit
...
- [ ] **5. Character-creation menu design** (DESIGN-GATED — questions first, then one coherent
  build; the open questions incl. guided-dialog vs one-button GM flow, key-talent auto-grant
  vs manual, nation flavor as primer-text vs actor flag). Scope candidates: country pick
  (#3 culture item) → path pick → key talent + kit grant (#4) → leyline/deity attunement →
  talent-budget spend (`L+3+floor((L-1)/5)`, §9g) → purse form + naming (primer). Build LAST,
  on top of #1–#4.
