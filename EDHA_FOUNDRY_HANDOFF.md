...
**2026-07-17c** (HEROIC SCHEMA SYNC — the 17b initiative executed: data/authored/heroic-*.json + 2 card texts; **pack rebuild `heroic` + relaunch + ⟳ Sync** for PCs carrying heroic talents. Ben ran `heroic-schema-dump-console.js` (cosmere-rpg 2.1.0, committed to `source-materials/system-schemas/`); the dump reframed the coverage claim — its "133/133 found" counted our OWN edha packs as finds; the REAL base coverage is **86/133 in `cosmere-rpg.heroic-paths`** (the other 47 are edha-homebrew per-path additions with no base source — they keep their current state, wiring them stays a from-scratch job). Verified before merging: (a) the base system's talents carry **ZERO event automation** (0 non-empty `events` across all 86 — its wiring is activation config + damage formulas + marker AEs + rich text), so this sync makes talents mechanically OFFICIAL, it does not make them auto-apply; (b) all 9 same-name multi-path base docs (Hardy, Mighty, Composed…) are mechanically identical, so any copy serves every path file; (c) base effects are pure MARKERS (empty `changes`, `isStackable` only) — same convention as our stance AEs; our edha AEs with real changes (Composed — Focus, Collected — Defenses…) are STRICTLY AHEAD and were left untouched. The merge was semantic-only (a first naive dict-compare produced 114 diffs, ~90% key-order/default-field churn — reverted; defaults-normalized compare left **23 real changes across 17 talents**): per-scene use limits Galvanize/Resilient Hero/Turning Point (`uses 1/per_scene` — native counters we lacked entirely); Cheap Shot plotDie:true + `@scalar.damage.unarmed` impact damage; heal formulas Field Medicine/Swift Healer (`@skills.med.rank`) + Applied Medicine (`@skills.lor.rank`); tier-scaling damage Devastating Blow `(2+max(@tier-2,0))d8` and Wit's End `(4+max((@tier-2)*2,0))d6` (card texts aligned: "+1d8/+2d6 per tier above 2"); activation corrections Calm Appeal (none→utility), Strategize (cost 2 + skill ded), Anatomical Insight (the missing 1-Focus consume); marker AEs copied for Hardy/Surefooted/Vinestance/Know Your Moment/Withering Retort. PRESERVED over base: the 4 edha skill_test auto-rolls the base lacks (Feinting Strike, Sharp Eye, Startling Blow, Subtle Takedown), the 3 edha events (Mighty, Know Your Moment, Swift Healer), all descriptions (edha condensed standard, Ben's lowest-churn ruling) and imgs. Gates green incl. 38 engine tests; audit.py doesn't cover heroic (leyline+deity scope). ⚑ bench rows in the checklist; the CI scratch-pack build validates the pack side.)
...
The whole "trusted action-economy" class is one initiative, not per-talent patches: Reaction-per-round
