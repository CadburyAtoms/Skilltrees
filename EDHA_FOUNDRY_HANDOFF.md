...
Backing detail (every session's notes) lives in agent memory `edha-foundry-module-build.md` + `edha-aoe-bursts.md`; this doc is the curated summary. Last update: **2026-06-11c** (Weakened mechanic: disadvantage on next physical test, then ends — engine-only, F5; see its delta). Prior: 2026-06-11b (V3 ENGINE PASS — state mechanics, custom statuses, derivations, adversary-effects bake; built + pack-validated, **NOT yet live-verified** — see the 06-11b delta block below). Prior: 2026-06-11 (playtest-PC manual-talent triage pass-1 + `TRIAGE_PLAYTEST_PC_MANUALS.md`), 2026-06-10b (playtest-1 prep — §8b), 2026-06-09 (RE-REFACTOR: behavior re-homed onto talents; both historic blockers fixed), 2026-06-08b (Demolisher playtest), 2026-06-08 (Event-System refactor).

**NEXT SESSION: live-verify the v3 pass (checklist in the 06-11b delta below — relaunch, ⟳ Sync, `edha.migrateDerivations()`, per-PC spot checks), then continue coverage tree-by-tree (§9 first bullet).**
...
## 2026-06-11c DELTA — WEAKENED MECHANIC (engine-only; F5 to load; NOT yet live-verified)
...
### Build & packs — ⚠ PACKS MOVED TO `packs/v3/`
...
- **NEXT SESSION — live-verify the 06-11b v3 pass** (checklist in the delta), then **broaden coverage**: author remaining `talent-*.json` entries tree-by-tree (36 of 365 talents now carry rules). Loop per batch: author table entries → `node foundry-build.js <scope>` (Foundry closed; NOTE packs now live in `packs/v3/` — see delta) → relaunch → `⟳ Sync` → live-verify. NOTE: 28 talent names collide across trees — sync matches `atlas|group|name`, but TABLES are name-keyed (colliding names all get the entry).
