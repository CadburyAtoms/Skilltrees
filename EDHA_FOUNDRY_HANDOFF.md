...
Backing detail (every session's notes) lives in agent memory `edha-foundry-module-build.md` + `edha-aoe-bursts.md`; this doc is the curated summary. Last update: **2026-07-15d** (HANDOUT-FORGE SKILL + SESSION-ZERO SHEET — docs + skill only; NO engine change, NO data change, NO pack rebuild, nothing for the bench. (1) NEW **`.claude/skills/handout-forge/`** — the player-handout workflow distilled from this session's session-zero sheet build: frame the audience + the reader's deliverable FIRST → query the talent data for every mechanic and flavor line (`leyline.json`/`cosmere.json`/`domain.json`; free-invented flavor got bounced — specialty names are the fallback) → the house design identity (moss/lamp-gold palette, Palatino, numbered zones, tier pills, ONE inverted block = the reader's deliverable, last and loudest) → HTML built with Write/Edit ONLY (PS 5.1 Get/Set-Content mangles UTF-8) → Chrome-headless render → verify EVERY page's last element each cycle (fixed-height sheets clip silently; pypdf races the flush — sleep first; extraction checks trip on uppercase transforms and the fi ligature; LOOK at the tall screenshot instead of estimating heights) → deliver to the repo folder + SendUserFile → iterate on Ben's design bar. (2) NEW handout sources committed: **`EDHA_CAMPAIGN_ONE_PAGER.html`** — the two-page A4 session-zero sheet (world pitch + go-back-well hook / heroic + leyline day-one choices / deity paths + ten-nation table + the green BRING-TO-SESSION-ZERO checklist block) — and **`EDHA_PLAYER_PRIMER.html`** — the styled printable twin of the primer. Their PDFs are regenerable (`chrome --headless --print-to-pdf`) and stay untracked per the 07-06 `*.pdf` gitignore; the deployed `EDHA_CAMPAIGN_ONE_PAGER.pdf` sits beside the source in the folder. (3) Ben's standing handout rulings live in the skill: data-derived flavor only; leylines get equal billing with heroic paths; legible color pips/swatches; countries in a ruled table; session-zero framing = ONE talent budget, bring a nation + a reason you left + the character fantasy, leylines "pick up to one", deities ~level 4 (2 ranks in each of the god's colors); headline claims literally true. CLAUDE.md trigger paragraph + doc table gained handout-forge.)
