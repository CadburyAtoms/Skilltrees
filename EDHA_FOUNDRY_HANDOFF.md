...
Backing detail (every session's notes) lives in agent memory `edha-foundry-module-build.md` + `edha-aoe-bursts.md`; this doc is the curated summary. Last update: **2026-07-15** (EQUIPMENT, MONEY & ITEMS INITIATIVE OPENED — Ben's report: sessions + Foundry tools have **no equipment, currency, or item layer** (adversary "weapons" are hand-shaped `action` items; armor and money untouched on every layer; the run-sheet had no loot section; canon has no coinage — session 1's "payment is food" was ad-hoc). Audit run, then FOUR direction picks (Ben, menu): **currency = full lore-forge pass FIRST** (NEW TODO_WORLDBUILDING **W25** — per-nation coinage, exchange, famine pricing, and **starting money/gear for the imminent character creation**; NO placeholder coin names anywhere until it lands — the provisional-flag rule); **item source = system pack + a small edha-items pack** (Edha-specific objects only; mundane gear keeps coming from `cosmere-rpg.items`, the Outlaw-Longsword precedent); **adversaries = MIGRATE attacks to real weapon-type items**; **money = engine-tracked**. Everything Foundry-side is gated on the **SCHEMA DUMP**: the system source is unreachable from repo sessions (the proxy 403s the public GitHub repo; add_repo is same-owner-only), so NEW **`scripts/schema-dump-console.js`** (paste into the console once, GM, read-only, creates nothing) captures the weapon/armor/equipment/loot DataModels + probe defaults, `CONFIG.COSMERE.currencies` + any register-currency API, sample Longsword/armor/loot docs from `cosmere-rpg.items`, and the weapon/armor registries → downloads `edha-schema-dump.json`; commit it to `source-materials/system-schemas/cosmere-rpg-<version>-dump.json`. Shipped this pass: (1) ⚑⚑ **WEAPON PIPE-CLEANER** (build + data → **pack rebuild `adversaries` + relaunch, NO ⟳ Sync**): `kind:"weapon"` in `advItemDoc` — the Activatable/Damaging fields stay byte-identical to the action shape (same skill_test + flat `modifierFormula`, so the PDF attack numbers hold at ANY skill rank), best-guess weapon fields (`equipped`, `range`, `type`+`weaponId` both carrying the registry id — one sticks, one strips) degrade to cosmetics if wrong (DataModels drop unknown fields; and the W23 invisibility failure CANNOT recur — `weapon` IS a rendered sheet section). Migrated: the **Corvaine Raider's Shortsword ONLY**. `validate.js` allows the new kind (+ a weaponId-without-kind gate); `validate-adversaries.js` prints `weapon:<id>` tags + a ⚑ stripped-activation diagnostic that decides the fleet-migration shape. (2) **SESSION TOOLING** (docs-only): run-sheet template §0 gains **The pay** (every job names its pay at frame time) + a **loot & payment ledger** under the clue ledger (loot is a clue *delivery* channel — a skipped fight must re-deliver its loot-borne clue; worth stays descriptive until W25); session-forge Phases 1/5/7 aligned (statblocks list strippable gear as `kind:"weapon"`; Phase 7 reconciles against party inventory); session-debrief extraction grid gains **row 10 — loot & money** (+ Phase 3); state doc gains **§1a party inventory & wealth** (wealth / notable items / owed — mundane kit lives on the Foundry sheets, §1a only tracks what a forge run must know). (3) **Backlog §9h** NEW — fleet migration, edha-items pack, engine currency primitive (double-gated: dump + W25), armor reality-check — each bullet names its gate. (4) Checklist section **"Equipment & items initiative"** (dump / pipe-cleaner / armor check / currency glance) + regenerated bench sheet. All gates green. Nothing invented lore-side — W25 is the gate.)
...
- **edha-items pack** (gate: schema dump — which of loot/equipment item types exist). New
  `data/items.json` → `edha-items` Item compendium via foundry-build, ONLY for Edha-specific
  objects; first entry: the **Malcurr-stamped blade** (session-1 clue object — description
  derived from the script/canon, no new lore needed). Mundane gear keeps coming from
  `cosmere-rpg.items` (the Outlaw Longsword precedent).
- **Engine currency primitive** (DOUBLE gate: schema dump for native `CONFIG.COSMERE.currencies`/
  register-API support + **W25** for denominations — NO placeholder coin names before W25, per
  the provisional-flag rule). Shape TBD by what's native: prefer registering Edha currency into
  the system's own field over a bespoke flag ledger (iron rule 2).
