...
- [ ] **Engine currency primitive** — **BOTH GATES CLEARED, buildable now** (schema dump committed
  07-17c confirms the public `registerCurrency` API; **W25 landed 2026-07-18** — canon §5d +
  rulings 54–59). Register ONE Edha currency: denominations **copper (base, rate 1) / silver
  (10) / gold (100)**, labels Copper/Silver/Gold with `c`/`s`/`g` units, actor-sheet display
  **gold → silver → copper** (Ben: big → normal → small, readable — flavor names stroke/seal/
  charter stay in descriptions only). Prefer the system's own field over a bespoke flag ledger
  (iron rule 2). ⚑ three bench/wiring questions (ruling 54): whether the sheet orders
  denominations by array position or conversionRate; whether the Roshar "spheres" row can be
  hidden/replaced; whether mundane `cosmere-rpg.items` prices (e.g. longsword 60 mk) get
  re-priced in c/s/g at the item level or a published conversion line (the ≤ 2-gold starting
  weapon cap ≈ the longsword tier either way, ruling 59).
