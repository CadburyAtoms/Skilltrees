...
- [x] **edha-items pack** — DONE 07-18d. `data/items.json` → `edha-items` compendium; first entry
  ...
  enforces).
- [ ] **Loot containers on the battlemap** (NEW 07-18, Ben: "I can paint a chest on the battlemap
  — how do we make it accessible by the players?"). Design menu presented (loot-cache token +
  cross-client Take chat-cards composed from the create-item GM relay, vs. the Item Piles module,
  vs. plain shared-ownership chest actor) — **GATED on Ben's pick**; the downed-adversary loot
  flow (search a body → its weapon items) is the natural second consumer of the same shape.
