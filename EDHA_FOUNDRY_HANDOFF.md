...
Backing detail (every session's notes) lives in agent memory `edha-foundry-module-build.md` + `edha-aoe-bursts.md`; this doc is the curated summary. Last update: **2026-07-14l** (MORRATH WORSHIP PASS — W11/W12/W13 Morrath slices + W16's Withervale payoff; docs-only, NO engine/data/pack change, NO rebuild, nothing for the bench. Process: four section-1 forks batch-approved, six §3a items walked one at a time in plain chat, assembly shown whole before commit. (1) **NEW CANON — rulings 44–48 (Ben):** **44 — sacred geography** (W12's rule ratified): worship reaches a god from anywhere (field model — a shrine works in any village); nexus-siting buys PRESENCE, not reception — high temples sit on pair co-peaks. **45 — the Passing chose the hour; rites never gate**: the mercy was universal (§1a holds for the godless), her rite was a proper asking (consent from the dying alone, no one may ask for another) that she always honored — and the general principle for every future faith block: rites petition and attune, never gate the machinery. **46 — the Black Altar is a door, not a house**: the keepers never built there and kept people off it (the §5b border folklore is that discipline six generations downstream); the altar is older/other than the faith; §8.8 untouched. **47 — the dead-line palette** (W13 first entries): sealed = a knock on a shut door with weight behind it (Morrath) vs banished = open sky (Razkael) — investigable, and sealed ≠ gone is the campaign's hope. **48 — the block index**: keepers are the continent's death-registrars (ruling 42's parish bells are SHRINE bells — the Lesser Tolling strikes keepers' rolls by keepers' hands), the no-high-temple doctrine, the crook-through-wars custom, the sheaf-ash loop, the name at the lamp (the one unbroken observance — never petitionary), carried grave-earth, the harvest-by-hand whisper (ones-and-twos, never a faction; Withervale untouched), the rolls as seal forensics (three districts side by side date the seal to a month and prove everywhere-at-once), and the lamps-feeding-the-sealed-god question deliberately open with §8.3. (2) **Canon §3a "Lived faith" is NEW** — section frame (rites as attention sustained; fills one god per pass) + Morrath's full block: the Passing / the Giving-Back / the Standing Sheaf / the Keepers / offerings + the name at the lamp / the broken case / the dead-line palette. Nine gods remain; W14/W15 stay ⚑ open (neither bites a dead line — deliberately dodged by doing Morrath first). (3) **Primer**: NEW "The quiet faith" section (the rites as lived culture everyone grew up with; the silence carried at whisper level — "the rites don't take like they did"). (4) **Session-1 script + opening doc**: Harrow's rite named (the Passing), NEW optional shrine-roll clue + clue-ledger row (the two-year discontinuity in Harrow's roll — plant tonight, compare districts in act 1). (5) Alignments: §3 Morrath cross-ref, §5b Lesser Tolling mechanism clause + giving-back cross-ref, §8.8 ruling-46 note, §10 NEW ⚑ **Death-tree pronoun drift** (domain.json flavor "He/His" vs canon She — decide at the next Death-tree DATA pass, not here); TODO W11/W12/W13/W16 logs updated. Still open in section B: nine gods' W11 blocks, W14 ⚑, W15 ⚑, W16 remainder.)
