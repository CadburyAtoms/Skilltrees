...
Backing detail (every session's notes) lives in agent memory `edha-foundry-module-build.md` + `edha-aoe-bursts.md`; this doc is the curated summary. Last update: **2026-07-14p** (BENCH SHEET — the human-facing test deliverable (Ben: the checklist MD is “very hard to parse as part of the workflow”; Excel floated as a starting point, better suggestion invited); REPO-SIDE ONLY → no engine change, no pack rebuild, nothing to deploy — the next `git pull` gives Ben `EDHA_FOUNDRY_TEST_SHEET.html` to test from. (1) NEW `scripts/build-test-sheet.js` — deterministic generator, `EDHA_FOUNDRY_TEST_CHECKLIST.md` → self-contained dark `EDHA_FOUNDRY_TEST_SHEET.html` (no dependencies, opens from disk): Pass/Fail/Partial/Skip buttons + a note box per row (503 rows / 30 sections parsed at build), per-section deploy chips scanned from the header/prose keywords (pack rebuild / ⟳ Sync / relaunch / F5 / engine-only), progress bar + per-section counts, filters (open / marked / fail+partial / ⚑ / free-text search), section nav rail, [x] rows rendered as retired. Marks live in browser localStorage keyed by a hash of the row TEXT — they survive pulls and regenerations for unchanged rows; a reworded row deliberately resets (changed test = fresh test). (2) **Copy results for Claude** emits the Phase-0-ready worklist (status + section + bold-name label + Ben's quoted note + the checklist `@hash` stamp — the stamp doubles as deploy-state evidence for Phase 1); **Copy TSV** keeps the Excel path alive as an option, no longer the default. (3) Sync ENFORCED: `--check` wired into CI (`validate.yml` step + path filter) and `scripts/pre-commit` — a checklist edit without the regenerated sheet fails both; the MD stays the agents' single source of truth, the HTML is generated and never hand-edited. (4) Workflow wiring: test-pass-fixes SKILL.md — Phase 0 parses the pasted “EDHA bench results” block (+ frontmatter trigger), Phase 7 step 2 regenerates the sheet and states the authoring contract (first bold run = the row's label in Ben's report; keyword-driven deploy chips; reword-resets-marks), new self-check row; checklist header re-pointed (Ben tests from the sheet); CLAUDE.md map row; scripts/README. (5) Self-verified headless (Chromium + Playwright, no Foundry needed): render, marking, notes, persistence across reload AND regeneration, all filters, search, both exports, zero console errors. ⚑ Only bench ergonomics remain for Ben — the four-row “The bench sheet itself” checklist section (marks persist, copy-paste report reads right, deploy chips honest, readability verdict).)
