...
- **Stays manual:** movement/positioning/ally-count triggers (Momentum's Edge, Coordinated Hunt), declared-prohibition violation calls (Order's Edict of the Fallen-style declarations — as of 07-03 the Order tree's violations are watcher-PROMPTED and button-fired, the declaration itself stays human; Snare/Bastion enter-triggers since got real Region behaviors), Fault Line ray template, Crown of Thorns (above); **Reserve SPENDING + Double Dip's HP-substitution** (Scope-A ruling, 06-13b — readout helps, but no auto cost-substitution).
