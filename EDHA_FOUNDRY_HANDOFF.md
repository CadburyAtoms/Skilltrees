...
Backing detail (every session's notes) lives in agent memory `edha-foundry-module-build.md` + `edha-aoe-bursts.md`; this doc is the curated summary. Last update: **2026-07-14m** (W23 PIPE-CLEANER FAILED → ACTION-TYPED-TWIN FALLBACK SHIPPED + DEPLOYED — build + engine + validator + pinned tests; **already live on Ben's machine this session: engine synced, adversaries pack rebuilt + validated — just RELAUNCH Foundry**, NO ⟳ Sync (none exists for adversaries); re-drag any world-placed Session-1 actors (snapshots). (1) **Bench result (Ben):** the Line-Caller sheet showed neither Guiding Signal nor Ordered Advance. **Root cause — the render layer, not the pack**: the deployed pack was verified correct first (both `talent`-type embeds present, LevelDB read, ✓ 0 issues); the system's `AdversaryActionsListComponent` renders exactly three item sections — trait / weapon / action, each filtered `item.type === type` — so ANY talent-type item on an adversary is silently invisible. 14j's ⚑ fallback implemented as designed. (2) **Build** (`foundry-build.js`): adversary `talents` refs now embed as **action-typed twins** — name/img/description/activation/damage/`system.events`/effects copied from the built talent (the action DataModel carries the same Activatable/Damaging/Modality/Events mixins, so nothing is stripped), `system.type:"basic"`, talent-only fields (prereqs/path) dropped by design, flags `edha-content.{adversaryTalent:true, talent:<name>}`. (3) **Engine**: NEW predicate **`edhaIsTalent(item)`** = `type === "talent"` OR the adversaryTalent flag; `edhaOwnsTalent` + 13 owner/caster item-by-name/rule lookups retrofitted (rally, ritual-hp-cost, heal/Bastion/Pinpoint-Charge lookups, the five per-deity `edhaXTalent` helpers, the Draw-Mana owned-keys set); **`edhaCountTalents` stays type-strict on purpose** — twins never count toward a PC talent budget. Pinned in `tests/engine-helpers.test.js` (3 new cases; 29 green). (4) **Validator** (`validate-adversaries.js`): `[TALENT]` tag keys off the flag; NEW hard-fail if a talent embed ships talent-TYPED (the exact regression that made the items vanish); notes embeds carrying no events/effects (name-based automation only — true for both White embeds). (5) Docs: ENGINE_INDEX 'Talents on adversaries' corrected (14j's 'embed the real talent-type doc' guidance is DEAD — twins are canonical), adversaries.json `_comment` updated, checklist W23 re-pointed. (6) **⚑ Bench (relaunch only):** Line-Caller sheet shows Guiding Signal + Ordered Advance under ACTIONS and the GM can use them; Guiding Signal end-to-end (inv 2→1, grant card lists RAIDER tokens, Plot Die arms, Raise the Stakes injected); Ordered Advance posts the round note + deducts 1 inv; 14j's other rows (folders, skills ranks, Roek, Mistheron) unchanged and still due.)
