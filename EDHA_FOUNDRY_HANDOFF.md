...
Backing detail (every session's notes) lives in agent memory `edha-foundry-module-build.md` + `edha-aoe-bursts.md`; this doc is the curated summary. Last update: **2026-07-14n** (W23 ROUND 2 — Ben's twin-bench results: designate shape, movement window, token numbers, RULING 49; ENGINE + build + data-comment + canon; **engine already synced to the live module — pack rebuild BLOCKED by Foundry's LevelDB lock: close Foundry, run `node scripts/foundry-build.js adversaries` + `node scripts/validate-adversaries.js`, relaunch.** (1) **Twins render + fire** — the 14m pipe-cleaner re-test PASSED (talents under ACTIONS, useItem fires, ownership gates pass). Three round-2 rows: (2) **Guiding Signal listed ALLIES to click — engine drift, the card text was canon** ("designate a character; the next ally who tests against it this round raises the stakes"): the 14j primitive skipped the designation and granted an ally directly. NEW Tool A2 — **`edhaPostDesignateCard`** lists OPPOSING tokens in Attunement Range; click stores a round-scoped **`plotDieMark`** on the designator (`edhaSetEdhaFlag` generic GM-relay write); the plot-die pre-roll/consume pair injects for the first same-side roller whose user-targets include the marked token (`edhaFindMarkGrant`), then clears the mark. Empty sweeps now EXPLAIN themselves (**`edhaSweepEmptyNote`**: no-token-on-scene vs nearest-candidate+distance — the 07-12b transparency rule; the old bare "No allies in Attunement Range" was undiagnosable). (3) **Ordered Advance was a bare GM-narrated note — re-litigated (Ben: movement-triggered ally card):** use now arms an **`orderedAdvance`** `{round, combatId}` window (**`edhaRoundWindowValid`** — out-of-combat marks stay open, in-combat = same round only); the `updateToken` watcher (initiating client only, `edhaForcedMove` skipped) posts the allies-within-10-ft card with each ally's **`edhaHalfSpeed`** (PC `.value` / adversary `.override`, 2.5-ft floor), or accounts for nobody qualifying. Provoke itself stays un-enforced — no reaction system exists to suppress. (4) **All multi-drops named "(1)" — Foundry core, not the build:** core `appendNumber` counts scene tokens by WORLD actorId (client actor.mjs) and every compendium→canvas drop creates a fresh world actor (client tokens.mjs), so the count is always 0. NEW `preCreateToken` renumber hook + pure **`edhaNextTokenName`** — on name collision, re-number by NAME pattern (lowest free). (5) **NEW CANON — ruling 49 (Ben, verbatim intent): adversaries run the FULL leyline economy** ("no reason for the adversaries not to have the same system as the players — that's the whole point"; extends ruling 40): foundry-build now auto-embeds each `leylines` color's **Leyline Attunement Key** (twin) + the universal **Draw Mana** action on every attuned adversary — `edhaDrawMana` is name-triggered and disposition-based so the riders run unchanged (White heals its own side; Blue arms next-Cognitive advantage). Line-Caller/Roek gain the White Key + Draw Mana, Mistheron the Blue; attuned blocks now DEFAULT to the PC inv derivation 2 + max(awa,pre) = 2 (Roek/Mistheron stale pre-49 `inv:0` removed; explicit `inv` wins). Canon §9 ruling 49 recorded. (6) Tests: 3 new pinned cases (32 green). ⚑ Bench after the rebuild: designate flow end-to-end (card lists PCs, click designates, Raider targeting the PC gets the injected Plot Die, mark clears), Ordered Advance move card (allies + half-Speeds listed), re-drop Mistherons number (1)/(2)/(3), Draw Mana on the Line-Caller (inv 1→2, White pulse heals raiders in 15 ft), Blue Draw Mana on the Mistheron.)
