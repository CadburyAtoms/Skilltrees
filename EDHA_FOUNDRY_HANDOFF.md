...
### 9j. Heroic-path & character-creation initiative (opened 2026-07-18e — Ben's ask; character creation is imminent)

- [ ] **Heroic talent copy-in** (UNBLOCKED — Ben's heroic schema dump is committed,
  `source-materials/edha-heroic-dump.json`, 133/133 found / 0 missing, 87 duplicated across
  packs). Copy each talent's real system schema (activation/damage/effects/flags) into
  `data/authored/heroic-*.json` per the 07-17b plan. Needs ONE ruling first: the
  **pack-preference rule** for the 87 duplicates (which source pack wins). Then pack rebuild +
  ⟳ Sync.
- [ ] **Heroic starting-kit wiring** (gate: ruling 59 kits — LANDED; item source = mundane
  `cosmere-rpg.items` + the weapon migration's corrected fields). Make "new PC gets their kit"
  one action instead of a manual drag-hunt: per-path kit manifests (Agent/Envoy/Hunter/Leader/
  Scholar/Warrior per the primer) + a GM grant flow (button or console `edha.grantStartingKit
  (actor, path)`) that adds the kit items, the 5-silver purse (10× silver? — purse writes need
  the currency field shape, ⚑ bench first), and the path's trade-token as a described item.
  Compose with existing primitives (`create-item` GM relay); no bespoke subsystem.
- [ ] **Character-creation menu design** (DESIGN-GATED — a Ben session, like §9i: questions
  first, then one coherent build). What the table wants: guided flow vs checklist? Where it
  lives (a dialog in Foundry, a primer-driven paper step, or the dashboard)? Scope candidates:
  path pick → key talent + kit grant → leyline/deity attunement pick → talent-budget spend
  (`L+3+floor((L-1)/5)`, §9g) → nation flavor (naming table, purse form, primer handout).
  Decide how much is automated vs narrated BEFORE wiring; the kit-grant item above is the
  only piece buildable ahead of the design.
