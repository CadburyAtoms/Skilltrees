...
Backing detail (every session's notes) lives in agent memory `edha-foundry-module-build.md` + `edha-aoe-bursts.md`; this doc is the curated summary. Last update: **2026-07-18e** (LOOT — chest caches + body search, the §9h painted-chest design built on Ben's pick; see the **2026-07-18e** delta immediately below). Prior: **2026-07-18d** (FLEET WEAPON MIGRATION + THE edha-items PACK — §9h's two dump-gated buildables landed; see the **2026-07-18d** delta below). Prior: **2026-07-18c** (THE CHECKLIST CONSOLIDATION — Ben: "clean up the outdated items, consolidate into a refreshed list"; see the **2026-07-18c** delta below). Prior: **2026-07-18b** (ADVERSARY PACK SYNC — the per-deploy "re-drag every adversary" step is retired by a GM ⟳ Sync; see the **2026-07-18b** delta below). Prior: **2026-07-18** (the all-in-one dashboard — `EDHA_DASHBOARD.html` replaces the bench sheet; see the **2026-07-18** delta below). Prior: **2026-07-17c** (the bench-results pass — 9 fail/partial rows root-caused against the system source + Ben's world DB; deploy-to-foundry.bat + relaunch + re-drag all adversaries; see the bold **2026-07-17c** delta immediately below). Prior: **2026-07-16d** (DEPLOY-BLOCKER HOTFIX, build script only — NO engine/data/pack change, nothing to re-author. Ben's `deploy-to-foundry.bat` crashed at [4 of 5] on `ReferenceError: Cannot access 'events' before initialization` in `foundry-build.js` `advItemDoc`: the 16 adversary-events pass added a `const events = {}` block BELOW the `system` object that already referenced `events`, so the `const` TDZ threw the moment any adversary was built (leyline/deity/heroic packs had already written; adversaries never did). Fix = moved the `events` declaration ABOVE the `system` object — its deps (`activation`/`damage`/`descValue`/`spec`/`advName`) were all defined earlier, so a pure reorder. Verified: `node --check` + validate + lint-refs green, and a full `scope=adversaries` build into a scratch `EDHA_MODROOT` produced 13 adversaries / 55 embedded items with no error (live packs untouched). Fixing [4 of 5] then exposed a SECOND deploy blocker hiding behind it at **[5 of 5]** — `validate-adversaries.js` failed with **✗ 12 issues**, every one "talent embed is type \"trait\" — must be an action-typed twin". Root cause: the validator's regression guard (`validate-adversaries.js:78`) dates from 07-14m, when the ONLY `adversaryTalent`-flagged embeds were tree-talent twins (all action-typed), so it demanded `it.type === "action"`. The 16 pass added **bespoke trait/action abilities that are ALSO flagged `adversaryTalent`** (Combat Training, Cinder Coat, Pack Tactics, Veil, Glyph Pulse, Phase 2, Predictive Ward, Mutation Upgrade, …) — a bespoke *trait* renders fine on the sheet (the sheet renders trait/weapon/action) but tripped the too-strict guard. Same class as the TDZ: 16 updated the build, engine gates, and lint-refs but not this validator, because `validate-adversaries.js` only runs against Ben's compiled packs — skipped locally AND in CI, so it only fires at deploy step 5. Fix = the guard now rejects only a genuinely-invisible type (anything not in the rendered set `{trait, weapon, action}`), catching the real 07-14 failure mode (a stray `talent`-type embed) while admitting bespoke traits. Verified for real: built a scratch `scope=adversaries` pack and ran `validate-adversaries.js` against it (honors `EDHA_MODROOT`) — reproduced Ben's 12 failures on the old guard, **✓ 0 issues** + exit 0 on the new one. Ben re-ran `deploy-to-foundry.bat` (Foundry closed) — **all 5 steps passed**, deploy clean end to end. **Both fixes then GATED in CI so the class can't recur**: `validate.yml` gained a "Pack build + validate" step that builds every pack into a scratch `EDHA_MODROOT` (classic-level installed just-in-time via `npm install --no-save`, pinned to Foundry's `2.0.0` for pack-format parity) and runs `validate-packs.js` + `validate-adversaries.js` — the previously deploy-ONLY validators now run on every PR. This is the exact gap that let BOTH of today's bugs reach Ben's step 5: the whole `buildAdversaries` → `validate-adversaries.js` path needs compiled LevelDB packs, so it ran nowhere in CI. Verified locally: `foundry-build.js all` + both validators exit 0 on the fixed tree (and the old guard reproduced the 12 failures pre-fix). Finally, the checklist's stale **"DEPLOY FIRST"** section — still claiming the module was "frozen at the 06-16 Green build" — was rewritten as **"DEPLOY STATE"**: records module+packs current through 16c + the 16d fixes, states that PC tokens are LINKED (never re-dragged; only adversaries are unlinked snapshots) and PCs need no ⟳ Sync from the recent engine-only passes, and that the sole per-deploy manual step is the adversary re-drag. Test sheet regenerated. **Nothing else outstanding for the playtest to continue — re-drag the adversaries and play.**)

**2026-07-18e** (LOOT: CHEST CACHES + BODY SEARCH — the §9h loot-container design, built same-session on Ben's pick of the loot-cache shape ("that sounds good. build it") after establishing that players could NOT loot anything before this: adversaries ship `ownership.default 0` (players can't open the sheet — the GM-lore guarantee) and Foundry has no native item transfer anyway (drag = copy, nothing leaves the body). ENGINE-ONLY (`register-skills.js`) — reaches Ben via deploy bat / module-src sync + relaunch, NO pack rebuild, NO data change. ONE mechanism, TWO entry points: (1) **chest caches** — `edha.createLootCache(name)` (GM, new console API) mints an adversary-type actor flagged `edha-content.lootCache` in a "Loot Caches" folder with a LINKED chest token (`icons/svg/chest.svg` ⚑ unverified path); the GM stocks it by dragging items onto its sheet and places the token over the painted chest. (2) **body search** — any DEFEATED adversary (HP ≤ 0) is a searchable source, which is how session-1's Malcurr blade actually reaches a PC's hand ("loot makes it free"). Flow: a PLAYER double-clicking the token (new `Token#_onClickLeft2` wrap, walk-the-proto idiom shared with the phantom veil; GM clicks always fall through to the sheet — that's how caches get stocked) within a 5-ft edge-gap of an owned token gets a whispered contents card (player + GMs); each Take button carries its payload in `data-edha-*` (the 06-14 cross-client rule) and relays through a new **`loot-take` socket action** where the GM client is the SINGLE WRITER: re-verify the item still exists on the source (THE double-loot guard — the losing click turns into a GM whisper, nothing duplicates), delete from source, create on the taker (selected owned token's actor → assigned-character fallback) with adversary provenance flags shed + equipped/alwaysEquipped cleared, then a public "X takes Y from Z" card. What's lootable is the PURE pinned pair `edhaLootableItems` (weapon/equipment/loot types; a body KEEPS `alwaysEquipped` natural weapons — no looting the hound's Bite; traits/actions/talents never) + `edhaLootSourceKind` (cache flag wins; downed PCs are never lootable). Adversary ownership never opens; no CSS needed (`.edha-trigger-card button` already styles the Take buttons). 44 engine tests green (was 42). ⚑ all 7 bench rows (new checklist section "Loot"), incl. the chest-icon path. Design alternatives recorded for history: the Item Piles module (rejected: third-party dependency + unknown cosmere-rpg field config) and a shared-ownership chest actor (rejected: drag copies instead of takes, double-loot risk, no announcement).)

**2026-07-18d** (FLEET WEAPON MIGRATION + THE edha-items PACK — §9h's two schema-dump-gated buildables, landed the session after Ben committed the dump (`source-materials/edha-schema-dump.json`, 07-18). ENGINE + DATA + BUILD; **pack rebuild needed: adversaries + the NEW `edha-items`** — `deploy-to-foundry.bat` step 4 now also runs `foundry-build.js items` — then relaunch + **"⟳ Sync Adversaries from Pack"** (the weapon items land on world adversaries in place; no re-drag). Ben's rulings this session: classification table approved as proposed; **Frost Lance stays an action** ("a bespoke investiture talent, basically"); **the Malcurr blade is a weapon**. (1) **Fleet migration**: 11 attack items across the 13 statblocks flipped to `kind:"weapon"` — gear (Trooper Strike, Poleaxe, Stalker Crossbow, Soldier's Crossbow ×2, Line-Caller Shortsword, Issued Blade; the Raider Shortsword was the 07-15 pipe-cleaner) + natural weapons (Bite, Spearing Beak, Slam, Scalpel-Strike — all `alwaysEquipped: true`, a new build-supported field: part of the creature, cannot be disarmed). Maneuvers/reactions (Devastating Blow, Reactive Strike, Press the Line, Snatch and Wade) stay bespoke actions. Attack numbers preserved by construction — same `skill_test` + `modifierFormula` roll. (2) **The pre-ship catch of the pass — rider survival**: the passive-rule harvest loops (`edhaRiderParts`, `edhaLightSpecFor`) gated on `edhaIsTalent`, and weapons are DELIBERATELY unflagged (equipment, not talents) → flipping Spearing Beak / Bite / Scalpel-Strike would have SILENTLY killed their authored riders (the whenTargetFooled +1d6, the Kindle light, the whenTargetStatus +4) — the same silent-drop class as the 07-16 unreachable-case family, caught by tracing the harvest path BEFORE flipping any kind. New predicate **`edhaRuleBearer`** = `edhaIsTalent(i) || type === "weapon"` (ALL weapons qualify, so a rider Ben hand-authors on any weapon's Events tab harvests too); both loops retrofitted; name-keyed useItem automation stays `edhaIsTalent`-gated on purpose. Pinned in `tests/`. (3) **`edhaAttackKind` upgraded**: for weapons it now reads the native **`system.attack.type`** (dump-verified DataModel field, stamped on every built adversary/summon weapon) — the ⚑ "cosmere field shape unverified" range-guess is retired; a legacy range-shaped fallback remains for hand-made weapons. (4) **Summon attacks are weapon-type** (Ben's 07-17 "defer to the weapon migration" ruling landed): `edhaSummon`'s primary attack AND damage-bearing `extraItems` now build as weapons (same Athletics skill_test, `alwaysEquipped`) — Construct Slam and Siege Cannon get the native target + test-defense flow the 07-17 interim only approximated; the `requiresSummonEffect` Siege-Form gate is item-type-agnostic and still applies. (5) **The edha-items pack**: NEW `data/items.json` → `edha-items` Item compendium (foundry-build scope `items` + in `all`; module.json pack + Edha packFolder entries; validate.js now validates 5 files incl. a HARD no-price gate until W25; validate-packs.js checks the pack incl. PACK-NOT-FOUND — module.json declares it, so a missing dir would break the world load; buildEdhaItems throws on a missing/broken items.json instead of shipping an empty pack). First entry: **Malcurr-Stamped Shortsword** — weapon, `light_wpn`, 1d6 keen PC-SHAPED (the system adds the wielder's mod; validate warns on adversary-shaped flat +N), description derived from the session-1 script's clue text (Perception DC 12 / Crafting-or-Lore DC 13, mountain-forge work, not Corvaine issue) — no new lore. (6) **Skills/docs teach the new model** (Ben's ask — future sessions build adversaries on it): leyline-tree-authoring SKILL.md §Adversary abilities (the item-kind model + the edhaRuleBearer trap), session-forge Phase 5 stats bullet, `adversaries.json` `_README` (the ⚑⚑ pipe-cleaner caveat retired; `alwaysEquipped` documented), ENGINE_INDEX (edhaRuleBearer + edhaAttackKind + the weapon-kind model). Verified: full scratch pack build against the branch tree (**gotcha: foundry-build's `DATA` default points at Ben's MAIN checkout — set `EDHA_DATA` when building from a worktree or you silently build main's data**; buildEdhaItems' original silent-catch hid exactly that and was made a hard error), `validate-adversaries.js` ✓ 0 issues with all 11 items listed `[weapon:…]` and the stay-action set intact, `validate-packs.js` ✓ incl. `edha-items: 1 items`, 42 engine tests green (was 40). ⚑ everything table-facing — 9 bench rows in the new checklist section. STILL OPEN in §9h: the currency primitive (W25 lore gate; `registerCurrency` confirmed by the dump) and the NEW **loot-container design** (Ben 07-18: "I can paint a chest on the battlemap — how do we make it accessible?" — design menu presented, GATED on his pick).)
...
### 9h. Equipment, money & items initiative (opened 2026-07-15; schema dump committed 07-18 → the two dump-gated items BUILT 07-18d)

Ben's 2026-07-15 report: sessions and Foundry tools have **no equipment, currency, or item layer**.
Direction picks (same date): currency = **full lore-forge pass first** (W25); item source =
**system pack + small edha-items pack**; adversaries = **migrate attacks to real weapons**; money
tracking = **engine-tracked**. Unblocker (1), the schema dump, landed 07-18
(`source-materials/edha-schema-dump.json` — item DataModels, currency registry, the system item
inventory); unblocker (2), the W25 currency canon, is still open.

- [x] **Fleet weapon migration** — DONE 07-18d (Ben-approved table). Every gear attack + natural
  weapon across the 13 statblocks is `kind:"weapon"` (natural = `alwaysEquipped`); maneuvers/
  reactions + Frost Lance (bespoke investiture, Ben's ruling) stay actions; summon attacks
  (Construct Slam / Siege Cannon) build as weapons too — the 07-17 deferral landed. Riders
  survive via `edhaRuleBearer`; `edhaAttackKind` reads native `attack.type`. ⚑ bench section
  "Fleet weapon migration" (9 rows).
- [x] **edha-items pack** — DONE 07-18d. `data/items.json` → `edha-items` compendium; first entry
  the **Malcurr-Stamped Shortsword** (weapon, Ben's ruling; clue text from the session-1 script).
  Mundane gear keeps coming from `cosmere-rpg.items`. Prices ship 0/"none" until W25 (validate.js
  enforces).
- [x] **Loot containers on the battlemap** — DONE 07-18e (Ben picked the loot-cache shape from
  the menu). `edha.createLootCache(name)` + double-click contents card + GM-single-writer
  `loot-take` (the double-loot guard); DEFEATED adversaries are searchable bodies (natural
  weapons stay). Engine-only. ⚑ bench section "Loot" (7 rows, incl. the chest-icon path).
- [ ] **Engine currency primitive** (gate: **W25** only, now — the dump confirmed native
  `registerCurrency` + `CONFIG.COSMERE.currencies` and the actor currency block, so the shape is
  known: register an Edha currency into the system's own field, NO bespoke flag ledger. NO
  placeholder coin names before W25, per the provisional-flag rule; the system's "spheres"
  rename is recorded under W25).
- [x] **Armor** — RESOLVED (07-17 bench): system armor works natively (`cosmere-rpg.items` ships
  it; equipping sets Deflect). Nothing to build; adversary Deflect stays the number override.
