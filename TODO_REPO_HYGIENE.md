...
## 172. [x] (2026-09-15, PR #408) A triggered effect's resource-gain card prints the rule's declared gain even when the pool is already full — "Reeve-Owl regains 1 Focus" posts at 3 / 3 Focus (ENGINE-ONLY, F5) (2026-09-15)
...
## 173. [x] (2026-09-15, PR #408) An on-hit status rule gated on an Isolated target posts "Sapping Hex — no Isolated target to affect (target a token, then re-fire)." publicly on every hit against a creature that is not Isolated (ENGINE-ONLY, F5) (2026-09-15)
...
## 176. [x] (2026-09-15, PR #408) A `near-victim` damage splash that catches nobody still rolls its damage and posts "6 energy to (no target — target a token, then re-fire)." publicly — Chain Detonation fired on a rootling's death with no creature within 5 ft (ENGINE-ONLY, F5) (2026-09-15)
...
**Progress — 2026-09-15, the static compatibility check (DOCS-ONLY, PR #403, open):** `docs/analysis/cosmere-rpg-3.1.0-compatibility.md`. 3.1.0 — zip 69,276,436 bytes, sha256 `bdad787b1c282147c7b01ae6fc0c4e6048b4b739c2a2ebf9c74491347c0c2bc8`, downloaded to `%TEMP%` only — was read at its release tags against every Edha touchpoint: **5 blockers, 9 breaks a feature, 6 cosmetic, 21 touchpoints unchanged, 1 third-party module unverified.** The root is 3.0.0's Embedded Actions: talents and weapons lose `activation` / `damage`, `use()` runs only on the embedded action, `preUseItem` / `useItem` hand the engine that action instead of the talent carrying the rules, and the native `use` event fires only for actions (189 authored talent rules). Hook names, the dice classes, `applyDamage` and the registration API are unchanged. The 3.1.0 vocabulary was extracted statically (events 17 → 27, handlers 12 → 14; the control run reproduced the tracked 2.1.0 snapshot with 0 differences) without touching `data/native-vocabulary.json`, which regenerates at the flip. The plan is six themed PRs — engine work dual-mode on `main`, the builder behind a target flag, the flip in the live-upgrade window — at about 9 worker sessions; the live test runs on a separate Foundry data path with Ben's own license entry, and the final upgrade is a backed-up in-place upgrade with Ben present. Still open: all of "Done when" — no break is fixed, the snapshot is 2.1.0, and the live world runs 2.1.0.
...
**Amended 2026-09-19 (item 214):** the deity leg builds the AMENDED text of `docs/design/channel-deity.md`, not the struck text — **four** faces keep a 1-Investiture consume row (Place Omen's Blue, Tend's Green, Pyre, Forge Construct; R-160 (a)), **Widening V** (`capToChannelled`, a per-round budget on resource gains) is the refund cap R-161 (a) needs and is the leg's one new mechanic, the atlas has **nineteen** faces rather than twenty (Death is one-faced), and the bench list is **thirty-three** DS rows. `VoidSenseRvl0000` and `ExposeReact00000` each split in two first, so the non-refund half of each stays ungated.
...
**What to do:** the source is the handbook journal in the installed `cosmere-rpg-mistborn-handbook` module — 21 journals, 241 pages — read from a scratch copy with `docs/analysis/talent-comparison/dump-packs.js` (licensed text: cite chapter and section, quote at most a short phrase, commit none of it). **Phase 1, Chapter 3 Character Statistics** (attributes, skills and tests, advantages and disadvantages, opposed tests, working together, defenses, health / focus / Investiture, conditions, injuries — whatever the chapter's pages hold), against: the engine (`module-src/scripts/engine/` — the dice and advantage channel, derived stats per `docs/ACTOR_STAT_DERIVATION.md`, the statuses table in `01-shared-core.js`, the contest core, Draw Mana), the docs (`docs/analysis/talent-ecosystem/SYSTEM-PRIMER.md`, its README's "rules facts", `EDHA_FOUNDRY_HANDOFF.md` §10, `EDHA_TALENT_HANDBOOK.md`), the skills (`cosmere-canon-reference` — its glossary, conditions and standard patterns; `leyline-revision-guide`; `deity-revision-guide`; `talent-balance`; `phrasing-verifier`), the rulings whose premise is a rule fact (start with R-43, R-98, R-100, R-105, R-137 … R-140), and `~/.claude/projects/C--dev-Skilltrees/memory/`. **Phase 2, chapters 9 – 13 and the Glossary** (Adventuring, Combat, Conversations, Endeavors, Gamemastering) the same way. Output: `docs/analysis/rules-audit-2026-09.md` — one ledger row per contradiction: the rule (chapter / section), where we contradict it (file and line, or ruling id), severity (changes play / changes a card / wording), the fix class (DOCS-ONLY, ENGINE, DATA, RULING), and its disposition (fixed in this PR, filed as item N, re-filed as ruling R-N). DOCS fixes land in the audit PR; ENGINE and DATA fixes are filed as items with the ledger row as their "why"; a ruling whose premise fails is re-filed in §L with the rule cited. `cosmere-canon-reference` gains the handbook as its source of record beside the Starter Rules.

**Done when:** both phases' ledgers exist; every DOCS row is fixed; every ENGINE / DATA row has an item; every failed-premise ruling has a §L re-file; gates green.

**PM:** lane R · model opus · size L · deps none (R-155 settles the first engine row) · Filed 2026-09-16 by the PM from Ben's instruction.
...
**What to do (on R-155 (a)):** phase 1 — replace the fold with a harvest-and-net step at pre-roll over every live source (the 27 `edha-next-test-mod` rules, the condition riders, the Gain Advantage marker), cancellation one for one, and placement by fixed priority across the d20 (`advantageMode`), the plot die (`advantageModePlot`, using the system's `p` / `gmp` picker) and the largest damage die (`advantageModeDamage` on the attack dialog's result), with pinned tests for every net count from −3 to +3 and for a raised-stakes roll. Phase 2 — the dialog: inject "N to place / M to place" beside the system's per-die toggles on `RollConfigurationDialog` and `AttackConfigurationDialog`, refuse the roll until placed, relay the opponent's placement (GM whisper card on a player's roll; most-affected player on an adversary's roll; d20 by default when unanswered). Both phases 2.1.0-safe: the dialogs and the plot modifiers are identical at both tags. The three documents corrected in R-155's filing PR are re-read against the build. 🤖 rows per die kind and per chooser.

**Done when:** a roll with two advantages doubles two different dice; one advantage and one disadvantage roll a plain d20; an adversary's disadvantage reaches the affected player; the bench rows pass; gates green.
...
**Done when:** a SPD-4 character's sheet reads 30 ft, and 40 with a +10 ft effect; the engine writes no walk-rate override; the wizard preview matches the sheet; the ladder is pinned in `tests/`; `ACTOR_STAT_DERIVATION.md` no longer names movement as an Edha derivation; gates green.
...
**What to do:** in `edhaDefTestOutcome`, keep `>=` for `vs: "defense"` and `vs: "dc"` and use `>` for `vs: "skill"`, with the reason in the comment above it (the function already documents the three modes). Leave the fail-open on an unreadable comparison value exactly as it is. Add a pinned regression case in `tests/` covering all three modes at equal, above and below — the tie is the case that must flip. Read the three authored consumers' card text and make it say who wins a tie, or stay silent rather than be wrong; check the pass-through call site at `53-native-event-system.js:569` (it forwards `cfg.vs`, so it inherits the fix). Note in the delta that only the opposed-skill path changes.

**Done when:** an opposed skill test that ties resolves as a failure for the initiator; defense and flat-DC tests behave identically to before; the regression case fails against the old comparison and passes against the new; gates green. 🤖 one bench row: tie a Blue contest and confirm the defender keeps the status quo.
...
**What to do:** rename the Edha status id and label off the published name. The naming is Ben's to approve, so **propose in the PR rather than deciding alone** — a pair that keeps the symmetry with `exalted` ("Exalted Die" / "Lessened Die", or `exalted` / `abased`). The id is referenced by the Sovereignty rules in `data/authored/deity-sovereignty.json` and by `44-sovereignty.js`, so the rename is engine + data and needs a REBUILD + ⟳ Sync; `tests/status-labels.test.js` already guards labels and should gain a case that no Edha status label collides with a **published condition name** (the fifteen in `cosmere-canon-reference` §Conditions). Check `conditionImmunities` entries in `data/adversaries.json` for the old id. Cards naming "Diminished" in the Sovereignty tree need the new word too, so `data/domain.json`'s source prose and the authored overlay move together.

**Done when:** no Edha status uses a published condition's name; the Sovereignty step-down works under its new id on a fresh build; a test fails if a future status label collides with the published list; the card text and the source prose agree; gates green. 🤖 one bench row: apply the renamed status and confirm the die steps down.

**PM:** lane B · model sonnet · size S · deps none (naming confirmed in the PR) · Filed 2026-09-16 by item 201's audit.
...
## 214. [x] (2026-09-19) The deity atlas reviewed against the published Invested families — two amendments applied to `docs/design/channel-deity.md` (four costed faces, not two; the refund passives become upkeep) and four rulings filed, R-161 answered the same day (DESIGN + RULINGS, DOCS-ONLY; nothing to deploy) (2026-09-19)

**Why:** Ben asked for a read on the Channel model and the deity interactions from PRs #433/#434, then for a balance opinion against the currently implemented atlas, with the goal of following the Radiant, Allomancy and Feruchemy paths now that `docs/analysis/talent-comparison-mistborn-radiant.md` has the data. The leyline side already lands inside the published bands; the deity side overshoots on price in the other direction (80 % costed → 7 %, below the published 8 % floor) and stacks three refill sources where every published family has one.

**What was done:** seven AMENDED blocks on the house strike-and-restate convention — Place Omen's Blue face and Tend's Green face take 1 Investiture (R-160 (a); G-3 re-answered (b)), and the five refund passives take one upkeep tail (R-161 (a)). §5.2 carries the rule and the face-pricing line, §5.3's three walks are recomputed, §5.4's row 1 is rewritten and a row 5 added, §7.2's "no new ruling was filed" is struck, and **Widening V** — a per-round budget on resource gains — is the leg's one new mechanic. Three pre-existing errors corrected in the reference sections: the Investiture sum 125 → **121** (verified against `data/domain.json`; three of the pass's own component lists already agreed at 121), the face count twenty → **nineteen** (Death is one-faced), and §6.1's consume-row spec eighteen → **fifteen**. Gate-log occurrences are left as the record of the day and declared superseded. Also fixed: eleven option bullets orphaned in `EDHA_RULINGS.md` §L when R-156 – R-158 moved to §K.23, which had been parsing into R-145's phone card.

**ANSWERED the same evening (2026-09-19).** The rulings were put to Ben as full text in chat and he answered in one line — *"160 d defaults rest"* — then accepted R-145 separately. **R-160 (d):** the widening was taken, so Tend's Blue face is costed too and **five** faces carry a consume row; Tend is the only supply costed on both. Sum 15 → 16, Life 1 → 2, ten priced cards, §6.1 five/fourteen. **R-162 (b):** unchanged, the wear/build split stands. **R-163 (a):** `Predatory Patience` and `Predator's Due` take R-161's rule; `docs/design/channel-black.md` amended at both cards, §1.5's economy, both turn walks and bench row CK-14 (+3 → +1), with a new CK-17 for Predator's Due's cap; `Flashpoint` exempt. **R-145** accepted unchanged, no code change, filed §K.27 — and with it **`EDHA_RULINGS.md` §L is EMPTY for the first time since the doc was created**; the test is re-pinned 1 → 0.

**What was open before those answers:** **R-160** (the face-pricing currency; its default is recorded in the design doc and wants a veto, and (d) — Tend's Blue face — is an unanswered widening), **R-162** (whether a scene-long install should outlive its Channel; untaken because it reopens gates 2 – 4), and **R-163** (whether the two leyline Black refunds take R-161's rule, when capping them contradicts `channel-black.md` §1.5 and falsifies bench row CK-14). Two rule splits are also needed before Widening V can be wired: `VoidSenseRvl0000` and `ExposeReact00000` each fuse their refund with a half that must stay ungated.

**Done when:** merged — done. The build rides **item 198's deity leg**, whose brief must read the amended text, not the struck text: four faces keep a consume row, Widening V exists, nineteen faces, thirty-three DS rows.

**PM:** lane H · model fable · size M · deps none (the build is item 198) · Filed 2026-09-19 from the review itself.
