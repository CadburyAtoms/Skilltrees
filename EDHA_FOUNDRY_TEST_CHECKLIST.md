...
- [ ] 🤖 **OM-3 — Isolating Pressure on an UNMARKED target places one Omen and does not also remove it:** on Bench — Chaos, clear the caster's Omen ledger (`flags.edha-content.lists.omens`) and pick a target bearing no `omen`. Use Isolating Pressure and succeed — confirm the target becomes Isolated, chat shows *"📋 Isolating Pressure : &lt;target&gt; bears your **Omen** (1/3)"* with the **placement** note *"no Omen to shatter — one is placed instead"*, the target ends the activation still carrying `omen`, the owner's ledger holds exactly one entry for it, and **no vital damage card is posted** (the card's "also" clause rides the shatter only). The failure this replaces: the release's `false` used to halt the batch, so nothing after the Isolated status ever ran.
- [ ] 🤖 **OM-4 — on a MARKED target it shatters exactly as before:** same caster, a target already bearing your Omen. Use Isolating Pressure and succeed — confirm the Omen is removed (status gone, ledger entry gone), the release card posts, the **[Tier][Die] + Awareness vital** damage lands, and **no placement card** is posted. This is the pin that proves the branches stay mutually exclusive: the damage rule's gate now reads the statuses the target bore when the activation *started*, not what the release left behind.
- [ ] 🤖 **OM-5 — a release card's denominator reads the tier + 1 cap:** on Bench — Chaos (tier 2, so the cap is **3**), place two or three Omens with Entropy Strike, then release one with Cascade Collapse (or Unweaving / Isolating Ruin / Unravel Everything) — confirm the release card reads *"… is spent (**2/3** left)"* / *"(1/3 left)"*, never `/2`. Bench run 47 measured `(2/2 left)` and `(1/2 left)` at a live cap of 3.
- [ ] 🤖 **PW-2 — a GM-applied Frightened survives the end of a Power-flavoured combat:** apply `frightened` to a creature by hand (it is a GM marker; no talent applies it since item 135), run a combat that a Power owner is in, and delete the combat — confirm the creature is **still Frightened** afterwards, while Power's own arming markers (`crowned`, `warlord`, `momentum`, `fury`, `unstoppable`, `mantled`) and `compelled` are cleared as before. ENGINE-ONLY: F5 is enough, no rebuild.
...
- [ ] 🤖 **GATE-1 — Bulwark Ground scales on White rank, not tier:** on Bench — Fate (tier 2, White rank 3 stock), stand an ally on one of your Ordained Ground squares at the start of a round: confirm the temporary HP granted is 3 (your White rank), not 2 (your tier). Off the rebuilt pack, the card reads "gains temporary HP equal to your White rank," not "equal to your tier."
- [ ] 🤖 **GATE-2 — Cascading Failure's multi-catch bonus scales on Blue rank, not Red:** Bench — Destruction is built with every colour at rank 3, so Blue and Red read identically at stock — temporarily set Red to a different rank (e.g. 1) so the two diverge, or inspect the owned item's `system.events.CascadeFailDet00.handler.doubleCaughtFormula` via the console. Set 2+ Charges and detonate them together with Cascading Failure so one target is caught in both blasts: confirm the "additional [Tier][Die] energy damage" bonus is sized off Blue rank (e.g. 2×3+2 = d8 at stock), not Red — the card text itself is unchanged (it never named a colour, only "[Tier][Die]"). Restore Red afterward if you changed it.
- [ ] 🤖 **GATE-3 — Vital Diagnosis's bonus vital damage scales on Blue rank, not tier:** on Bench — Life (tier 2, Blue rank 3 stock), Diagnose a target and have anyone deal damage to it: confirm the auto-applied bonus is 3 vital (your Blue rank), not 2 (your tier). Off the rebuilt pack, the card reads "deal additional Vital damage equal to your Blue rank," not "equal to your Tier."
