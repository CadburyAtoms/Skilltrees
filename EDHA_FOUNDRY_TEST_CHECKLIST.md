...
- [ ] ⚑ **Melee discriminator (`edhaAttackKind`) — the misread WAS confirmed 2026-07-26k (bench
      run 3)**: the cosmere 2.1.0 weapon schema has **no `system.range` field at all** (it is
      stripped by the DataModel) — range and kind live at **`system.attack.type`**
      ("melee"/"ranged") + `system.attack.range`. A weapon set to `attack.type: "ranged"` still
      fired Warlord's Advance's melee-only rider and consumed the arm, so today EVERY weapon
      reads as melee and no rangedOnly/meleeOnly gate can ever stand a rider down. Zero current
      exposure (no ranged weapon ships in the compendia) but the fix is one field read.
      → test-pass-fixes. The original row (Spurs/Venom stand-down cards on a ranged hit) is
      re-testable only after that fix.
...
- [ ] **2bI-1 — Whispered Doubt (Black) ⚠️⚠️ — PARTIAL 2026-07-26k (bench run 3)** — the MECHANICS all pass: extra loss applied (in and out of combat), once per round per enemy held on a same-round second spend, re-armed the next round, and the `scope: scene` watch is alive. **FAIL on the card clause: the extra loss posts NO card, ever** — the only visible trace is Coercive Pressure's arm card firing on the same trigger; Wary's reduction card even names an "involuntary focus loss" no card announced. → test-pass-fixes. **Out-of-combat scope, observed richly this run (ruling is Ben's):** the watch fires out of combat; a GM console/bookkeeping focus edit counts as a "spend" (dropping a fixture 4→0 armed Coercive AND credited Predatory Insight); campaign tokens' watches fire scene-wide (The Outlaw armed Coercive on a bench fixture from ~45 ft with no one playing it; a Dirgehound's own 1-Focus ability cost was taxed by Bench — Black's watches from 45 ft).
...
- [ ] **2bJ-8 — Puppeteer (Black) ⚠️⚠️ — PARTIAL 2026-07-26k (bench run 3)** — the first `turn-start` watch is ALIVE: the whispered offer posts on a 0-focus enemy turn start, accepting spends exactly 2 focus + 1 Investiture and posts the public "chooses one of Bench Target — Floater's actions" note (correctly named). **FAIL on the naming clause: the whispered OFFER prints a literal `{name}` placeholder** ("🎭 Puppeteer — {name} starts its turn at 0 focus…") — the template variable is never substituted. → test-pass-fixes. (A triple-post scare was reproduced as driver artifact — one turn-change event → exactly one card.)
...
- [ ] **Black spot-checks (like-for-like) — PARTIAL 2026-07-26k (bench run 3)** — Cruel Step on a clean lane slides the full **10 ft toward the Isolated target** ("ignoring Reactions") and stops at **0 ft** when the target square is adjacent/occupied — both correct. **FAIL: when the mover's own square straddles a map wall** (the Playtest Map has one at x=5156 crossing the lower-left room), the slide stops after ~3 ft with "(stopped at an obstacle)" although the move backend reports the lane ahead CLEAR — the engine-move ray appears to collide with a wall the token is standing on (corner-vs-center ray origin). → test-pass-fixes. *(The row's "two 16-char rule ids" is STALE text: the document carries ONE rule (`CruelStepMove001`, 16 chars) and `data/authored/leyline-black.json` also authors exactly one — no data loss.)*
...
- [ ] **Green spot-checks (like-for-like) — ROOT-CAUSED 2026-07-26k (bench run 3)** — **Mender's Instinct: run 1's report decomposes into TWO defects, both reproduced deliberately.** (1) **No ally gate**: the `edha-hp-threshold` handler's only scoping is `includeSelf` — a HOSTILE crossing half HP drew the owner's heal offer, and clicking it HEALED the hostile 10 HP for 1 Inv. (2) **The "double post" is two different OWNERS**: the second card each time came from **"The Vivisectionist" — a campaign actor with no token on the Playtest Map** — the watch fires for every rule-owner world-wide, with no scene-presence or range gate; every damage event in the world produces one offer per owner. Also confirmed: the card prints the full description, not the tight one-liner; Green's own `oncePerRound` DID hold (its offer went silent while the Vivisectionist's kept posting). **Plus a third defect found by clicking**: the Mender heal path IGNORES the "Withering Touch — No Healing" block — it healed a blocked target 10 HP (the block guards the applyDamage path only). All → test-pass-fixes. **Herding Antlers on the Fellstag (2bF-10): FAIL — the ability is DEAD on the REBUILT pack**: fresh import reads `utility`, no `activation.skill`, and `system.events` is EMPTY, so 2 Inv is charged and the promised Drive-the-Prey contest never runs — it was missed by the 07-26j `advItemDoc` sweep. → test-pass-fixes. Vital Surge's offer posts normally (below-half heal → THP offer; not re-tested for the dice family per the ⚑). **Still unrun**: Spreading Roots (2bS-4) · Pack Hunter (2bS-6) · Scent the Weak (2bS-7, though its advantage was seen arming incidentally) · Resurgent Growth (2bS-12) · Natural Recovery (2bS-14) · Reknit Form (2bS-15).
...
- [ ] **2bW-1 — Withering Touch — PARTIAL 2026-07-26k (bench run 3, dice re-test)** — the restored dice PASSED: arm card + "Withering Touch (next melee hit)" icon, re-use refused **pre-cost** ("already active — nothing spent"), and the melee hit added **"+12 vital strike"** ([T][D black]+Wil = 2d8+4) in the SAME application ("takes 18. Damage Calculation: 6 - 0 + 12"), icon consumed, "No Healing" AE + card landed on the target. **FAIL on the heal-block**: a Mender's Instinct heal-offer click healed the blocked target 10 HP — the block guards the applyDamage path but not `edha-hp-threshold`'s programmatic heal. → test-pass-fixes. (Temp-HP-still-lands and the turn-start expiry not driven this run.)
...
- [ ] **2bV-15 — Tempered Edge / Magnum Opus — PARTIAL 2026-07-26k (bench run 3, dice re-test)** — Tempered Edge's restored dice PASSED: a Construct Slam on a deflect-2 foe printed **"+9 energy and the hit ignores … deflect (+2)"**. **⚠️ Discrepancy on the ignore-deflect claim**: the application calc still read "takes 17. Damage Calculation: **19 - 2**" — the deflect WAS subtracted from the aggregate despite the card's claim (either the ignore applies to the energy part only and the card overclaims, or the ignore didn't wire) → test-pass-fixes to adjudicate. Still open: Siege Cannon adds NEITHER; all of Magnum Opus (once/scene pre-cost, +2×[T][D white] HP, +2 defenses, Foundation +1→+2, Colossus splash + Agility-vs-Red-or-Prone).
...
- [ ] **2bU-9 — Warlord's Advance — PARTIAL 2026-07-26k (bench run 3, dice re-test)** — the restored dice PASSED: melee hits added **"+6/+9/+10 impact strike"** ([T][D red], fresh dice each arm) in the same application; a kill granted **2 Temp HP** (tier) + the 10 ft free-move whisper; a survivor granted the target-bound Presence-advantage card. **FAIL on the ranged skip — but it is an ENGINE-WIDE bug, not this talent's**: a weapon updated to the system's own `system.attack.type: "ranged"` (+ range values) still fired the melee-only rider and consumed the arm — `edhaAttackKind` doesn't read `system.attack.type` (the cosmere 2.1.0 weapon schema has NO `system.range`; range lives under `system.attack`). Blast radius = every meleeOnly/rangedOnly gate (this row, 2bW-2, the Spurs/Venom stand-downs). Zero current table exposure — no ranged weapon ships in the compendia — but → test-pass-fixes. (Re-use-while-armed refusal not driven for this talent; the same pre-cost veto passed 3× elsewhere this run.)
