...
**Agent-run deploy 2026-09-14T06:09:43.066Z from main @ 8f0c843: packs 2026-09-14T06:09:43.066Z, engine 7fc3ce74 = HEAD, validators PASS**
...
✅ **The fix-pass-2 pack rebuild filed 2026-09-05 (Reeve-Owl / Sovereign of Solitude's rejected `edha-triggered-effect target` enum) is RETIRED** — Ben rebuilt the same day (16:15) and bench run 27 retired the row on evidence (*"Reeve-Owl / Sovereign of Solitude — the contest half — RETIRED on evidence 2026-09-05, bench run 27"*, White section); every agent-run deploy since has rebuilt all five packs. (Corrected by bench run 49a, 2026-09-15.)
...
*(✅ **111-2 RETIRED on evidence, bench run 49a (2026-09-15)** — engine hash-verified `5e9833a9…` = `HEAD` from both sides (item 146 live). **The owned path refreshes:** Bench — Black's owned `Black` path and `Draw Mana` were staled by hand (*"STALE bench-49a …"*), then its sheet's own ⟳ Sync Talents → *"Edha: synced 27 item(s) on Bench — Black (25 talents, 1 path, 1 action)."* and both came back byte-equal to the pack (the path's 1857 chars, *"Black leyline mages are predators, and the leyline feeds on them…"*; Draw Mana *"Recover Investiture equal to your highest leyline rank…"*). **The heroic half:** Bench — Heroic still owns no path item (run 47), so it was read on the three players' actors right after their PM-R17 refresh — all nine owned path cards (heroic Scholar ×2 and Envoy; Green / White / Blue; Knowledge / Fate / Chaos) byte-equal to the packs, where run 47 measured 24 of 24 owned path items stale. **The starting-skill parse:** `edhaParseStartingSkill` + `edhaSkillIdFromLabel` lifted from the SERVED engine over the six heroic pack cards → Scholar→Lore (lor), Leader→Leadership (lea), Hunter→Perception (prc), Warrior→Athletics (ath), Envoy→Discipline (dis), Agent→Insight (ins).)*
...
*(✅ **KM-2 RETIRED on evidence, bench run 49a (2026-09-15)** — items 141 + 148 live. `system.damage.formula` is `null` on all three docIds, owned and in the pack. **No damage button on any of the three:** Predatory Strike's item card carries no button at all (stored content and rendered card); Killing Blow's and The Final Study's messages carry only the engine damage card's `expandRoll` dice toggle — where a talent that still has a formula (Entropy Strike) renders the system's `apply-damage` ×4 and `reduce-focus` buttons (TODO item 169). **The bonus still lands:** Predatory Strike armed, then a Sidesword hit (1 keen) applied honestly to Bench Target — Undefended → *"🐺 Predatory Strike (Bench — Knowledge): +17 vital strike."* (inside `2d8 + 2 × max(0, 1)` = [4, 18]), HP 30 → 12, then *"📖 Predatory Strike : 1 Insight placed on Bench Target — Undefended (now 1)."* **Killing Blow** at 3 staged Insight (the `system.stacks` field the engine writes), 13 vs PHY 12 SUCCESS → *"⚡ Killing Blow (Bench — Knowledge) — 24 vital to Bench Target — Undefended. (2d8) * 3"*, HP 30 → 6, *"all 3 Insight removed"*. **The Final Study** at 2 Insight, 24 vs PHY 12 SUCCESS → *"⚡ The Final Study (Bench — Knowledge) — 20 vital … (2d8) * 2"*, HP 30 → 10, *"all 2 Insight removed"*, and the free-Strike roster listed the allies inside Green rank 3's 60 ft.)*

*(✅ **KM-3 RETIRED on evidence, bench run 49a (2026-09-15)** — Predatory Strike's arm card on Bench — Knowledge, verbatim: *"📖 Predatory Strike: make a melee or ranged weapon attack — the next hit auto-adds [Tier][Die] plus your Tier per Insight on the target (min 1), then places 1 Insight on it. Don't also roll the card's damage by hand."*)*
...
*(✅ **BUILT 2026-09-14 (items 142 + 150, PR #383)** — the paragraph below is why it was left open, kept as the record of the constraint. H3 grew the dial it named: a non-halting `release` (`onMissing: "continue"`) plus `whenTargetStatus` / `unlessTargetStatus` read against an ENTRY SNAPSHOT of the target's statuses. The rows are in the next section.)*
...
## Chaos — Isolating Pressure places an Omen; release caps; Power's reset list — items 142 + 143 + 150 (2026-09-14; **ENGINE-ONLY → F5 + REBUILD deity + ⟳ Sync Talents**)

*(✅ **OM-3 RETIRED on evidence, bench run 49a (2026-09-15)** — Bench — Chaos (tier 2, cap 3) with its Omen ledger unset, against a fresh Trooper bearing nothing. Isolating Pressure, 20 vs PHY 13 SUCCESS (the first take failed, 10) → *"Isolating Pressure — Trooper is Isolated."* + *"📋 Isolating Pressure : Trooper bears your Omen (1/3). no Omen to shatter — one is placed instead"*; no damage card (HP 14 → 14); the Trooper ends with `isolated` and `omen` and `markedBy.omen`, and the ledger holds exactly one entry for it. ⚠️ The system's own use message rolls a `DamageRoll 2d8 + 2 + 5` on every take, pass or fail, which the engine never applies — TODO item 169.)*

*(✅ **OM-4 RETIRED on evidence, bench run 49a (2026-09-15)** — the same Trooper, now bearing the Omen. 18 vs PHY 13 SUCCESS (the first take failed, 12) → *"📋 Isolating Pressure : Trooper's Omen is spent (0/3 left). shattered — the damage below rides this"* + *"⚡ Isolating Pressure (Bench — Chaos) — 6 vital to Trooper."* (`2d8 + 2` = 1 + 3 + AWA 2), HP 14 → 8; no placement card; `omen` and its mark gone, the ledger `[]`.)*

*(✅ **OM-5 RETIRED on evidence, bench run 49a (2026-09-15)** — Entropy Strike placed three Omens (*"(1/3)"*, *"(2/3)"*, *"(3/3)"* on two Troopers and Bench Target — Isolated); Cascade Collapse, one shared roll of 18, affected all three → *"📋 Cascade Collapse : Trooper's Omen is spent (2/3 left)."*, *"… (1/3 left)."*, *"📋 Cascade Collapse : Bench Target — Isolated's Omen is spent (0/3 left)."* — never `/2` — each followed by its spirit damage and *"… is Disoriented."*; the ledger `[]` after.)*

*(✅ **PW-2 RETIRED on evidence, bench run 49a (2026-09-15)** — a STARTED bench combat (round 2) holding Bench — Power and Bench Ally — Two. A GM-applied `frightened` on Ally Two; Power's own `crowned` and `fury` on Bench — Power; a Trooper outside the combat carrying Kneel's `compelled`. `combat.delete()` → Ally Two **still `frightened`**; Bench — Power's statuses `[]`; the Trooper's `compelled` cleared (its `disoriented`, a Chaos status, stayed). The only other output was *"Edha: cleared Kindle light from 0 token(s)."*)*
...
*(✅ **GATE-1 RETIRED on evidence, bench run 49a (2026-09-15)** — Bench — Fate (tier 2, White rank 3); the owned Bulwark Ground card reads *"…gains temporary HP equal to your White rank."* Ordained Ground placed under Bench Ally — One with a real click → *"✦ Ordained Ground set (1/2). Allies beginning their turn on it gain +1 all defenses and Temp HP = 3 (Bulwark Ground), and may Aid at up to 30 ft."* In a started combat, at Ally One's turn start → *"✦ Ordained Ground (Bench — Fate): Bench Ally — One begins its turn ordained — +1 all defenses, Temp HP 3 (Bulwark Ground), and may take the Aid action at up to 30 ft (execute by hand)."* and `flags.edha-content.tempHp = {value: 3, source: "Bulwark Ground"}` — 3, the White rank, not 2.)*

*(✅ **GATE-2 RETIRED on evidence, bench run 49a (2026-09-15)** — Bench — Destruction (tier 2, Blue 3); the owned `CascadeFailDet00.handler.doubleCaughtFormula` reads `(@tier)d(2 * @skills.blue.rank + 2)`. Two Charges set with real clicks 5 ft from one Trooper (both store the raw `(@tier)d(2 * @skills.red.rank + 2)`), then Red dropped 3 → 1 so the two colours diverge. Cascading Failure → *"💥 Cascading Failure — 2 Charge(s): Trooper (1): 2 energy · Trooper (1): 7 energy · Trooper (1): +8 energy (caught in 2 blasts)"* with rolls `2d4 = 2`, `2d4 = 7`, **`2d8 = 8`** — the blasts sized off Red 1, the multi-catch bonus off Blue 3. Red restored to 3.)*

*(✅ **GATE-3 RETIRED on evidence, bench run 49a (2026-09-15)** — Bench — Life (tier 2, Blue rank 3; it owns exactly one `edha-apply-status` rule, so the owner-rule lookup cannot pick the wrong one); the owned card reads *"…deal additional Vital damage equal to your Blue rank."* A Trooper (deflect 1) Diagnosed; 4 keen from Bench — Life → *"🎯 Vital Diagnosis (Bench — Life): +3 vital vs the Diagnosed target."*, HP 14 → 8 = (4 − 1) + 3; 2 keen from the ally Bench — Destruction → the same +3, HP 8 → 4 = (2 − 1) + 3. ⚠️ The Diagnosed card's own note still says "+Tier vital" beside its "+3 vital (auto-applied)" — TODO item 170.)*

## Blue — False Premise denies Reactions (2026-09-14; **REBUILD leyline + ⟳ Sync Talents**)

*(✅ **FP-1 RETIRED on evidence, bench run 50 (2026-09-15)** — R-141 (a) and item 171 live (engine `80042b86…`). Bench — Blue vs Bench Target — Adjacent A (COG 14) in a started combat on Bench Arena, turn order Blue 0 · Adjacent B 1 · Bench — Black 2 · Adjacent A 3, both talents used on Blue's turn. **Pattern Recognition, unchanged:** its whispered card *"🔮 Pattern Recognition — if you succeeded on a Cognitive test against this character, spend 1 Investiture to give its next test this round disadvantage."* → its own *Impose disadvantage* button → *"🎲 Pattern Recognition : Bench Target — Adjacent A's next test — at disadvantage ."*, one `nextTestMod` entry `{mode: "disadvantage", count: 1, round: 1}`. **False Premise** (Blue's Investiture 4 → 2 across the two talents): `2d20kh + 5` = 21 vs COG 14 SUCCESS (the advantage is Blue's own Calculated Patience — a slow-turn first test) → *"False Premise — Bench Target — Adjacent A is No Reactions ."*, `noreactions` on Adjacent A with its token HUD icon active, stamped `expireAfter {round: 1, turn: 3}` — Adjacent A's own turn, not Blue's (turn 0) — and `nextTestMod` still the one entry, no second disadvantage stack. **The expiry, stepped one turn at a time:** still on at Adjacent A's turn start (round 1, turn 3); cleared on the next turn change with *"💢 No Reactions on Bench Target — Adjacent A ends (end of its turn)."* **The card:** the owned False Premise, synced by the roster run and equal to the pack document, reads *"… On a success, that character cannot take Reactions until the end of its next turn."* in its value, chat and short forms. (Bench run 49a's PARTIAL — the status cleared at the END while the old card said the start — raised R-141; Ben answered (a) and fix pass 13 moved the card.))*
...
*(✅ **IMM-1 RETIRED on evidence, bench run 49a (2026-09-15)** — item 149's card half live. Bench — Death raised a Risen Servant (*"💀 Risen Servant : a Harvested Remain is consumed — 0 left."*; `system.immunities.condition` compelled / disoriented / frightened true, COG 11). Bench — Power's **Kneel**, 25 vs COG 11 SUCCESS → *"🛡️ Kneel : Risen Servant (Bench — Death) is immune to Compelled — no status applied."* — no "is Compelled", no "movement ENFORCED"; the servant's statuses `[]`, no effects, `markedBy` unset; the cost paid (Investiture 3 → 2). **Control:** Kneel on a fresh Trooper (unlinked, COG 11), 22 vs 11 SUCCESS → *"🎯 Kneel : Trooper is Compelled (by Bench — Power). Next action: move toward the compeller or do nothing — movement ENFORCED (only distance-closing moves pass)."*, with `compelled` and `markedBy.compelled {actorId: <Bench — Power>, talent: "Kneel"}` written on the token's actor. Row-text correction: R-127 is not open any more — Ben answered (a) on 2026-09-14 (a PRE-COST immunity gate); that half is unbuilt (TODO item 149 stays open), so paying the cost is still today's behaviour.)*

*(✅ **INC-1 RETIRED on evidence, bench run 49a (2026-09-15)** — every call a dry run, with four pack adversaries freshly imported into `Bench Targets`. `edha.syncAllAdversaries({folder: "Edha Bench", scenes: [<Playtest Map>]})` → `{dryRun: true, actors: ["Briar-Gone Grove", "Trooper", "Trooper", "Trooper"], missing: ["Bench Target — Undefended"]}` — the roster reached through the child folder; `folder: "Bench Targets"` → the same four; `folder: "No Such Folder"` → the warning toast (and console line) *"Edha: adversary sync — the folder "No Such Folder" filter matched ZERO adversary actors, so nothing will be synced. Check the folder name (it matches a folder or any of its subfolders) or pass actorIds."* and `actors: []`. The import's `_stats.modifiedTime` did not move across the three calls. (The zero-match call also posts a second info toast, *"DRY RUN — would sync 0 adversaries"*, after the warning — noise, not a defect.))*

*(✅ **SYNC-1 RETIRED on evidence, bench run 49a (2026-09-15)** — the sheet's own button on Bench — Black: *"Edha: synced 27 item(s) on Bench — Black (25 talents, 1 path, 1 action)."*, the count including the path and Draw Mana it had just restored (see 111-2). **The negative half:** `edha.syncActorTalents(<a fresh Briar-Gone Grove import>)` → `{updated: 0, missing: [], byType: {}}`; its embedded `Draw Mana` (flags `{adversary: "Briar-Gone Grove", drawMana: true}`) kept the same description and the same `_stats.modifiedTime`, and is not listed missing. ⚠️ **Found alongside — TODO item 168:** on the three players' actors the same button says *"Edha: synced 8 item(s) on Tem parinaem (4 talents, 3 paths, 1 action) — 19 not found in packs (see console)."* — the nineteen are the cosmere system's own basic actions (Dodge … Disengage), swept in by item 146's `action` widening; the bench PCs own none, which is why this row reads clean.)*

## Fix pass 13 — bench runs 48 + 49a's defects (2026-09-15; **ENGINE-ONLY → relaunch / F5** for items 161 / 162 / 163 / 164 / 168 / 171; **REBUILD deity + ⟳ Sync Talents** for item 170's rule texts; R-141's card is **REBUILD leyline + ⟳ Sync Talents**, re-driven by row FP-1 above)

*(Items 161–164 came from bench run 48's yardstick fights, 168 / 170 / 171 from bench run 49a. Every root
cause is pinned headless in `tests/`; these rows are the live half. R-141 (a)'s card text is not a new row —
FP-1 above was reworded in place to the END of the target's next turn plus a card re-read.)*

*(✅ **CUE-161 RETIRED on evidence, bench run 50 (2026-09-15)** — item 161 live. A started combat on Bench Arena holding Bench — Red and two fresh Rootling Swarm imports, each diagonally adjacent to Red. **Control:** at Red's round-1 turn start BOTH whispered *"⏰ Territorial Instinct (Rootling Swarm): Territorial Instinct is ready this round (Reaction, 1 Focus): when an enemy within 30 ft declares a Disengage, target it and use the item — Green vs. Survival zeroes its movement. (Bench — Red's turn starts in range.)"* — one card per rootling token (`bji70k7N6IvMKJIl`, `EkYGOcq3dxvJH0QL`). **The dropped copy:** Rootling Swarm (1) taken 6 → 0 by an honest `applyDamage` — the system's `dead` status and "Dead" effect landed — and at Red's round-2 turn start only Rootling Swarm (2) whispered. **The tracker-defeated copy:** Rootling Swarm (2)'s own combat-tracker skull (`[data-action="toggleDefeated"]`, which the Advanced Encounters row renders) → `combatant.defeated: true` with its HP still 6; at Red's round-3 turn start neither rootling posted anything.)*
*(✅ **CUE-161b RETIRED on evidence, bench run 50 (2026-09-15)** — out of combat, fresh imports, every card attributed by its speaker token. **`damaged`:** a living Mistheron damaged 20 → 18 whispered *"⏰ Fade (Mistheron): Reaction, 1 Focus — the bird may move 10 ft without provoking, into fog if possible; once unseen it may raise The Seeming again."* (the control); a second Mistheron already at 0 (its `dead` marker on) struck again for 5 posted only the system's blank damage message — no Fade card and no bloodied card. **`ally-drops`:** three Corvaine Raiders, Raider (1) already at 0; dropping Raider (2) 10 → 0 posted Raider (2)'s own below-half Break card and, from Raider (3) alone, *"⏰ Break (Corvaine Raider): Reaction — an ally dropped: the Raider may immediately Disengage and flee. (Corvaine Raider dropped.)"*; Raider (1) posted nothing.)*
*(✅ **THORN-162 RETIRED on evidence, bench run 50 (2026-09-15)** — item 162 live. **Bench — Green's own square:** Draw Mana clicked beside its own token (the SNAP-164 placement below) → Region `(1000, 800) 200×200` whose Thorn Field hazard carries `exemptActorUuid: "Actor.KYTl8CYAycyeNR11"` (Bench — Green) and `floor((2d8) / 2)`. **Placement:** no card for Bench — Green (57 → 57), while Bench Target — Adjacent A, also inside, took *"🔥 Bench Target — Adjacent A takes 5 keen from dangerous terrain (Thorn Field — Bench — Green)."* **A hostile moved in:** Adjacent B walked into the square (`move(…, {action: "walk"})` → true) → *"… takes 4 keen …"*. **Turn start:** in a started combat Bench — Green's round-2 and round-4 turn starts inside the square posted nothing (57 → 57), while Adjacent B's same-round turn start took 3 keen (the control). **The Grove repeat (YARD-3):** a fresh Briar-Gone Grove (2×2 at (1500, 1200)) Drew Mana, clicked at world (1610, 1290) → Region `(1500, 1200) 200×200`, exactly its own footprint, hazard exempt its token actor, `floor((1d8) / 2)` — no keen card on placement (60 → 60) and none at its own turn start. **R5, unchanged on purpose:** Bench — Death's Bone Garden under its own token (Region `(700, 1500) 200×200`, `turnEndDamage {formula: "2d8", type: "keen"}`, the scene-freebie Harvested Remain consumed) took nothing at its turn start and *"🦴 Bench — Death ends its turn in the Bone Garden — Bench — Death — takes 14 keen."* at its turn end (42 → 28).)*
*(✅ **PACK-163 RETIRED on evidence, bench run 50 (2026-09-15)** — item 163 live. Bench — Green at (1000, 600), a fresh Trooper (1) east of it, Bench Ally — One east of that Trooper, Trooper (2) south of Green. Pack Hunter targeting Trooper (1) (Investiture 3 → 2) → *"🐾 Pack Hunter (Bench — Green): 2 hunter(s) gain advantage on their next attack against Trooper (1)."*; both actors' `advAttackNext` read `{source: "Pack Hunter", targetUuid: "Scene.rtek97ETZ7jKHcmJ.Token.utAp89OefM8kZaLA"}` — that Trooper's token. **A different enemy first:** Green's Sidesword on Trooper (2) rolled `1d20 + 4` (`advantageMode: "none"`), posted no spend card, and both flags remained. **Then the marked Trooper:** `2d20kh + 4` (dice 20, 10 discarded) and *"🐾 Pack Hunter — advantage spent on this attack."*; Green's flag cleared, the ally's kept.)*
*(✅ **PACK-163b RETIRED on evidence, bench run 50 (2026-09-15)** — a fresh Briar-Gone Grove (2×2) at (1500, 1000); Bench Ally — One touching its left edge and Bench — Green its right edge (each 1.5 squares from its centre — outside the old centre-to-centre rule), Bench Ally — Two at 0 HP touching its bottom edge. Pack Hunter from Green targeting the Grove (Investiture 2 → 1) → *"🐾 Pack Hunter (Bench — Green): 2 hunter(s) gain advantage on their next attack against Briar-Gone Grove."*; Green's and Ally One's `advAttackNext` name the Grove's token (`…Token.sa2i0eEI0ufQa1hx`), and the downed Ally Two has none. (The old centre rule would not count Ally One; without the defeated skip the count would read 3.) Ally Two's 41 HP was restored afterwards.)*
*(✅ **SNAP-164 RETIRED on evidence, bench run 50 (2026-09-15)** — item 164 live; every pick hovered before its click, and `canvas.mousePosition` read each world point back exactly. **Green Draw Mana (Green rank 3, 10 ft):** Bench — Green at (1000, 800) beside Adjacent A at (1100, 800); clicked world (1110, 890) — inside Adjacent A's cell, 10 px from their shared vertex — → Region `(1000, 800) 200×200`, centre (1100, 900) = that vertex, covering both tokens' cells (by item 164's own arithmetic the old centre snap would have laid `(1100, 800)`, Adjacent A's cell only). **Lay Foundation (10 ft):** clicked (1710, 490) → Foundation drawing `(1600, 400) 200×200`, centre (1700, 500). **Sudden Growth:** clicked (1390, 290) → burst template at (1400, 300); after its Detonate button, Region `(1300, 200) 200×200`, centre (1400, 300). **5 ft markers still land on the clicked cell:** Ordained Ground clicked (1930, 740) → marker template at (1950, 750); Snare clicked (1830, 740) → template at (1850, 750) and snare Region `(1800, 700) 100×100`. The THORN-162 Grove and Bone Garden placements snapped the same way.)*
*(✅ **SYNC-168 RETIRED on evidence, bench run 50 (2026-09-15)** — item 168 live, driven as the PM-R17 refresh of the three players' actors, each through its own sheet's ⟳ Sync Talents button: *"Edha: synced 8 item(s) on Tem parinaem (4 talents, 3 paths, 1 action)."* · *"Edha: synced 8 item(s) on Soggy Bottom (4 talents, 3 paths, 1 action)."* · *"Edha: synced 8 item(s) on Ishee (4 talents, 3 paths, 1 action)."* — no "not found in packs" suffix and no "items not found in any Edha pack" console line (only Foundry's own deprecation notices from rendering the sheets); each actor owns 20 `action` items, and Draw Mana is the only one carrying the `edha-content` scope — the one action the toast counts.)*
...
- [x] 🤖 **I10b2-1 — H6 offer card (`edhaPickCandidates`)**: with an `edha-prompt-pick` rule at
...
- [x] 🤖 **I10b2-2 — sweep empty-note (`edhaSweepEmptyNote`)**: PlotGrant with no ally in range and
  ...
  disposition set — allies and targets cannot be told apart, so nothing is in range."*
      ...
      at create and update); the fail-closed behaviour is pinned in `tests/disposition-failclosed.test.js`
      (23 cases), which is the standing record.
- [x] 🤖 **I10b2-3 — movement-window card (Ordered Advance)**: arm the window, move next to a real
  ...
  stopped (its token has no disposition set, so allies could not be told apart)."*
      ...
      at create and update); the fail-closed behaviour is pinned in `tests/disposition-failclosed.test.js`
      (23 cases), which is the standing record.
- [x] 🤖 **I10b2-4 — Edict `<select>` + Beacon card**: `edhaPickProhibition`'s "Attack a chosen
...
*(✅ **156-1 … 156-9 RETIRED on evidence, bench run 49b (2026-09-15)** — the adversaries pack of the agent-run deploy 2026-09-15T11:19:34Z (main @ 7bd01c4; served engine `5e9833a9…` = `HEAD`, hashed from both sides). One `pack.getDocuments()` pass read the seven blocks: every old skill total from the attribute alone with no rank written (Roek ath 2 / dis 2, Mistheron stl 2, Tollbird prc 3, Surecat prc 4 / stl 3; the Well-Warden's dec 4 = PRE 2 + rank 2), defenses 10 + the attribute pair with no override in `_source`, prototype `sight.range` 10 / 10 / 10 / 20 / 20 / 20 / 10 = the ladder. Fresh imports into `Bench Targets`, driven on `Bench Arena`: the Raider 10 → 4 posted *"⏰ Break (Corvaine Raider): Reaction — below half HP …"*; Guiding Signal's card listed exactly the two PCs within 15 ft (the one at 20 ft absent); the Line-Caller 10 → 0 posted *"⏰ The Line Falls Apart …"*; Roek 24 → 7 posted *"⏰ Not a Bandit …"* and Press the Line's applied hit *"… One allied Raider may immediately make a crossbow shot as a Reaction. (hit Bench — Chaos.)"*; the Spearing Beak against a fooled PC read `1d6 + (1d6)[Spearing Beak] + 2` with `Graze 1d6` (against a seer `1d6 + 2`), and the Seeming's use card renders the R-136 sentence; Whispered Doubt drained a spender inside a combat (a 2-Focus Counterspell took Bench — Blue 6 → 3, *"🧠 Whispered Doubt : Bench — Blue loses 1 focus."*); an Isolated hit landed *"Sapping Hex — Bench — Chaos is Weakened"*; Redirect Momentum rolled `1d20 + 2` vs Athletics; Intercept's confirm click queued `{mode: disadvantage, source: Intercept}` on Bench — Chaos; Hollow Command rolled `1d20 + 4` and a success stamped *"Cannot Act (Hollow Command)"* to the target's next turn (`expireAfter {round 3, turn 1}`); the Well-Warden's Draw Mana Weakened the Isolated PC (*"swept 1 · newly Weakened 1 — of 1 enemy you can see within 30 ft (Isolated)"*) and Dread Presence then refused its move toward an ally (*"🚫 Dread Presence : Bench — Fate is Weakened and cannot willingly move closer to Bench — Chaos."*); 22 → 10 posted The Well Comes First. Bench — Blue's Phantom Double, synced by the roster run at 17:58Z, ends its description, chat and short forms with the R-136 sentence, and a fooled Bench — Heroic's Steadfast Challenge on the copy read *"15 vs Bench — Blue (Illusion) (Bench — Blue)'s SPI 0 — SUCCESS"*, Disoriented and the disadvantage on the copy only. Per-row evidence: the 2026-09-15 bench run 49b delta.)*
...
*(✅ **156-10 … 156-22 RETIRED on evidence, bench run 49b (2026-09-15)** — same deploy, same pack, fresh imports on `Bench Arena`. Every attack rolled through the item's own play button (`item.use({configurable: false})`), formulas read off the cards' `.dice-formula` nodes: Raider Shortsword and Crossbow `1d20 + 2` / `1d6 + 2` / `Graze 1d6`; the Line-Caller the same on both; Roek's Issued Blade and Press the Line `1d20 + 3` / `1d8 + 3` / `Graze 1d8`; the Mistheron's Beak `1d20 + 2` / `1d6 + 2`, Snatch and Wade `1d20 + 2` with no damage roll, its belief card *"belief vs DC 14"*; the Tollbird's Mob the Straggler `1d20 + 2` / `1d4 + 2` / `Graze 1d4`; the Surecat's Pounce `1d20 + 3` / `1d8 + 3` / `Graze 1d8` with the on-hit +1d4 cue; the Well-Warden's Hook-Pole `1d20 + 1` / `1d6 + 1`; the Rootling's Whip and Root `1d20 + 2` / `1d6 + 2` and Grasping Vines `1d20 + 2` vs Physical; the Grove's Bough `1d20 + 4` / `1d8 + 4` / `Graze 1d8`; the Crownox's Horn Toss `1d20 + 3` / `1d8 + 3`; the Reeve-Owl's Stoop `1d20 + 3` / `1d8 + 3`; the Preacher's Staff `1d20 + 2` / `1d6 + 2` — each equal to its card's printed Attack and Hit, every attack `modifierFormula ""` with dice-only damage, every defense derived with no override. A rootling 6 → 3 posted *"⏰ The Heart's Runners … scatter"*, and Territorial Instinct posted once per living rootling at the first PC turn start in range each round (none for a PC 85 ft away, none for a second PC the same round); with three PCs inside the Grove's own Draw Mana briar the Bough rolled `2d20kh + 4`, with two `1d20 + 4`; three edge-adjacent oxen carried no effect and Deflect 1; the Reeve-Owl's Focus went 1 → 2 on a hit against a Weakened creature with *"regains 1 Focus"*, stayed 2 on an un-Weakened one and 3 at 3; Censure rolled `1d20 + 4` and stepped Bench — Life's Sidesword damage from `1d6 + 4` to `1d4 + 4`; the Preacher 20 → 9 posted *"⏰ The Crown Lowered …"*. Two card-honesty defects seen on the way are TODO items 172 and 173. Per-row evidence: the 2026-09-15 bench run 49b delta.)*
...
*(✅ **123-1 RETIRED on evidence, bench run 49a (2026-09-15)** — items 147 + 151 live (engine `5e9833a9…` = `HEAD`). Four pack adversaries imported fresh into `Bench Targets`; one Briar-Gone Grove token on the Playtest Map (in scope — staged `displayName` 40 and `sight.range` 45, its actor's Draw Mana card staled) and one on the **Bench Arena** (out of scope — pack values, sight 30, `displayName` 20). ⚠️ **Ben's started combat no longer exists** (the world held zero combats at 16:19Z), so the out-of-scope started combat was a bench-owned one on the Arena (round 1, turn 0) holding the Arena Briar token. **Dry run** `{folder: "Edha Bench", scenes: [<Playtest Map>]}` → the four actors, `sceneTokens {Playtest Map: 1}`, *"DRY RUN — would sync 4 adversaries, stamping tokens on 1 scene(s) — nothing written"*; the staged token and the stale card unchanged. **The refusal:** the same call scoped to the Arena (still a dry run) → *"Edha: adversary sync REFUSED — Briar-Gone Grove's token is in the STARTED combat dr61RctVGlg37XaX on scene "Bench Arena" (token W7khAc3WQ9GJOtRy). Nothing was written."* **Real run** (`dryRun: false`, Playtest Map only) → *"Briar-Gone Grove (11 items, 1 token)"* + three Troopers; the in-scope token restamped to **sight 30** (the pack's bespoke Senses, not the AWA ladder's 10) and `displayName` 20; the stale card restored. **The item 147 half:** full token signatures (id / x / y / size / texture / disposition / sight / bars) byte-identical on Palewater Ford, Playtest Map (Copy) and the Arena — the Arena Briar token still sight 30 — the started combat's document hash unchanged, and on the Playtest Map only the synced token changed. No unscoped call was made; everything created was deleted.)*
...
*(**✅ FOUR OF THE FIVE FIX-PASS-9 ROWS RETIRED on evidence 2026-09-06, bench run 41**, on the
hash-verified `f2fb3e2da75057ce…` engine (`main` @ `53dd0bc`), one started combat on `Bench — Black`
against a target with two living allies within 10 ft.*

- ***`once` budget (the dotted-key defect) — FIXED.** Three uses in one round: clicks 1 and 2 pushed
  (each token exactly **10 ft** away from the anchor), the **third click refused** —
  `ui.notifications` read verbatim *"Unnerving Approach was already used this round."*, button
  disabled, coordinates **unchanged**, no push card, Investiture unmoved. The stored shape is **one
  FLAT key**, `coordRound["Actor_2vSISUi8NZ66KM9B_Item_Lt4bBgGwLZx7TyiZ"] = {_pick: 1}` — **no
  `Actor` branch anywhere**; `nextRound()` restored exactly one use and re-stamped `{_pick: 2}`, same
  shape. **NEG:** Puppeteer's turn-start offer stamped its own flat key, and the SECOND 0-focus
  turn-start in the same round was refused with *"Puppeteer was already used this round."* — focus
  4 → 4, Investiture 1 → 1. (The ambush-belief ledger's key is flat by the same helper:
  `Scene_<id>_Token_<id>`.)*
- ***2bJ-10 (reopened) — PASSES WHOLE, run 40's failing last clause included.** Investiture 4 → 3 on
  use; **Decline (refund)** restored **3 → 4**, relabelled the first button *"Declined — cost
  refunded"*, disabled **every** button and stamped `cardResolved`; `coordRound` stayed `null` so the
  round's use was NOT spent; the same round's second use fired a real Push; the third refused.*
- ***R-84 — the offer that CANNOT be made refunds.** Unnerving Approach on a target whose nearest
  living hostile was **42.7 ft** away: *"😨 **Unnerving Approach**: no living ally of your target
  within 10 ft to push (it may already be Isolated). — cost refunded"*, Investiture **3 → 3** net,
  **zero buttons** (no Decline), and the pick budget **not** spent — proven by a later valid use in
  the same round that offered normally.*
  *⚠️ **The NEG was UNRUNNABLE AS WRITTEN.** Puppeteer's pick is `source: "confirm"`, and the
  empty-candidate branch exists only under `source === "creatures"`, so Puppeteer can never reach it;
  a survey of all ten `edha-prompt-pick` rules in `data/authored/` finds **none** with
  `source: "creatures"` on a non-`use` event, which is what `refundable === false` + a consuming item
  requires. Substituted control, **staged and labelled**: a clone of Puppeteer (watch → pick,
  `costs: "foc:2, inv:1"`) with its pick switched to `source: "creatures"`, `rangeFt: 5` so the list
  is empty → *"🎭 **B41 Puppeteer Probe**: … — **no cost was spent**"*, focus 4 → 4, Investiture
  3 → 3. Clone deleted. Un-staged half of the same claim: Puppeteer's real offers carry
  `refund: false` on the message flag, and neither a refusal nor a round-change sweep credited
  anything.*
- ***R-85 — a "this round" rider granted by a NON-combatant expires.** `Bench — Blue`, not in the
  tracker, cast Pattern Recognition on a combatant: the bearer's `nextTestMod` carried **`round: 3`**
  — the BEARER's round, not `null` — and after `nextRound()` the flag read **`null`** and the next
  test rolled a plain `1d20 + 4`. **NEG (a):** granter IN the tracker → the stamp is still the
...
❌ **That same take exposed a NEW defect — the card announced the UNGATED amount. Fixed by item 68
(fix pass 8) and CONFIRMED LIVE below.**)*

*(**✅ ALL FOUR ITEM-68 ROWS RETIRED on evidence 2026-09-06, bench run 41**, on the hash-verified
`f2fb3e2da75057ce…` engine. **Both marks were produced FOR REAL, not staged:** `Bench — Death` armed
Withering Touch and landed a melee Sidesword hit → `healCut {fraction: 0, byName: "Withering
Touch"}`; `Bench — Black` landed Withering Ray → `healCut {fraction: 0.5, byName: "Necrotic Grasp"}`.*

- ***Withering (0):** HP **4 → 4**, gate card *"🩸 … cannot regain HP (Withering Touch)"*, talent card
  *"⚕️ **Field Medicine**: … cannot regain HP (Withering Touch) — no healing lands."* — **no number
  anywhere in it**.*
- ***Halved (0.5) — the case run 39 predicted and never drove:** HP **4 → 6**, gate card *"… has their
  healing halved (Necrotic Grasp)"*, talent card *"heals **2**"*, not 5.*
- ***The group heal:** `White Leyline Attunement`'s `edha-pulse` over exactly two allies in range
  (5 ft each; every other ally ≥ 66.7 ft, measured) plus self, one withered and one halved, all three
  below max → *"🕊️ **White Leyline Attunement**: healed 2 of 3 ally(ies) for **3** HP within 60 ft
  (visible) — no healing landed on Bench Ally — One."* The HP bears it out exactly: self 52 → 54
  (+2 = tier), halved ally 18 → 19 (+1), withered ally 23 → 23 (+0, and named). ⚠️ The row named
  "Mending Aura" as the shape, but Mending Aura is an `edha-burst`; the shipped `edha-pulse` heal is
  **White Leyline Attunement** (on `edha-draw-mana`) — the only one in `data/authored/`.*
- ***The Investiture clamp:** `Reaper's Harvest` on a qualifying defeat with the pool **1 short**
  (3 of 4) → *"✨ **Reaper's Harvest**: Bench — Death recovers **1** Investiture."*; at a **FULL**
  pool the next defeat posted the Remain card and **no Investiture card at all**, 4 → 4.*
  *⚠️ **Staging note, cost this run three takes:** the `defeat` watch is gated on
  `victim.type !== "character"` (Ben R2, the `updateActor` defeat watcher), and the bench TARGET
  dummies are `character`-typed — killing one harvests **nothing** and reads exactly like a dead
  watch. Use an **adversary**-typed victim (a fresh pack import, or `Bench Target — Undefended`), in
...
*(✅ **169-DTH RETIRED on evidence, bench run 50 (2026-09-15)** — item 169's narrowed fix live: `system.damage.formula` is `null` on Bench — Death's owned Withering Touch (`d2IDgKhRwYyxZy6n`), equal to the pack. The arm (Investiture 4 → 3, `withernext`) posted the system's use message and *"🥀 Withering Touch : next melee weapon hit withers — the talent's [Tier][Die]+Willpower vital is added automatically …"* — neither carries a roll, an `apply-damage` or a `reduce-focus` button. A Sidesword hit on a fresh Trooper (24 vs PHY 13, `1d6 + 4` = 8 keen) applied with `originatingItem` → *"🐺 Withering Touch (Bench — Death): +10 vital strike."* (inside `2d8 + 2`) and *"🥀 Withering Touch : Trooper cannot regain HP until the end of Bench — Death's next turn."*, HP 14 → 0, the `Withering Touch — No Healing` effect on the Trooper, the arm consumed. Necrotic Cascade stays held on R-142, as the row said.)*
...
*(✅ **169-PWR RETIRED on evidence, bench run 50 (2026-09-15)** — item 169's narrowed fix live: `system.damage.formula` is `null` on Bench — Power's owned Momentum of Victory and Warlord's Advance, equal to the pack. **Momentum of Victory:** the arm (Investiture 4 → 3, `momentum`) posted the system's use message and *"👑 Momentum of Victory : Free — 1 Investiture + an Opportunity (not auto-deducted): move up to 15 ft and make a free melee Strike … The Strike's hit gains +2 impact automatically."* — neither carries a roll, an `apply-damage` or a `reduce-focus` button; a Sidesword hit on a fresh Trooper (18 vs PHY 13, `1d6 + 4` = 9 keen) applied with `originatingItem` → *"🐺 Momentum of Victory (Bench — Power): +2 impact strike."*, HP 14 → 4, the arm consumed. **Warlord's Advance:** the arm (Investiture 3 → 2, `warlord`) the same — no roll, no button; six Strikes missed PHY 13 and the arm stayed up; the seventh hit (22, 6 keen) → *"🐺 Warlord's Advance (Bench — Power): +7 impact strike."* (inside 2d8), HP 14 → 2, and the survivor rider whispered *"👑 Warlord's Advance : Trooper survives. Advantage on your next Presence test against Trooper (target-bound — fires only with Trooper targeted; until the start of your next turn, trusted)."* with `nextTestMod [{attr: "pre", mode: "advantage", source: "Warlord's Advance", targetUuid: <that Trooper's token actor>}]`. Unstoppable Advance stays held on R-142, as the row said.)*
...
*(**✅ R-71 / item 69 — RETIRED on evidence 2026-09-06, bench run 41**, on the hash-verified
`f2fb3e2da75057ce…` engine. `Bench — Order`'s **Verdict**, whose document formula is STILL
`(@tier)d(2 * @skills.blue.rank + 2)`, rolled through `CosmereItem#rollDamage` — the SYSTEM's own
path — printed **`2d8 + 5`**, read off the card's own `.dice-formula` node rather than the Roll
object, at tier 2 / blue rank 3. The build-time fold's known limit is CLOSED at runtime and the
document is untouched, which is exactly item 69's claim. **Rider case:** with a `1d6` `either` rider
armed the same card read **`2d8 + 1d6 + 5`** — the rider joined onto the **folded** base, and was
consumed with its own card. (The row wrote the example as `2d8 + 5 + 1d6`; `edhaJoinRiderTerm` puts
the die beside the dice, before the numeric modifier — a shape difference, not a defect.) The fold
was observed on three more talents in passing: Withering Ray `4d8 + 5`, Pyre's hazard *"2d8 energy"*,
and Forge Construct's baked *"Construct Slam 2d8 impact"*.)*
...
✅ **THIS SECTION IS CLEARED — all 16 rows RETIRED on evidence at bench run 41 (2026-09-06)**, on the
hash-verified `f2fb3e2da75057ce…` engine (`main` @ `53dd0bc`). Item 24 (PR #244) turned the
~2,500-line run of 102 sequential `registerItemEventType` / `registerItemEventHandlerType` calls into
two tables (`EDHA_EVENT_TYPES` = 15 rows, `EDHA_HANDLER_TYPES` = 87 rows) registered by ONE loop; the
repo-side proof was the byte-identical registry snapshot
(`tests/fixtures/handler-registry.snapshot.json`), and these rows were the live smoke.

*(**The picker.** Console: `edha.EDHA_HANDLER_TYPES.length` → **87**, `edha.EDHA_EVENT_TYPES.length`
→ **15**, `Object.keys(CONFIG.COSMERE.items.events.handlers).filter(k => k.startsWith("edha-")).length`
→ **87** (99 handlers total = 87 + the system's 12; 32 event types = 15 + the system's 17). The real
Events-tab picker, opened on `Bench — Order`'s Verdict → Events → `+`: the Handler dropdown holds
**100** entries — `None` + 12 native + **87 `Edha:`-labelled** — and the Event dropdown **29** —
`None` + 13 native + **15 `Edha:`**; the picker's `edha-` option values are identical, IN ORDER, to
`EDHA_HANDLER_TYPES`, so the loop bound them in table order. Boot log carries
`Edha Content | native event system registered` once, with no `registration failed`. The probe rule
was deleted and Verdict's events restored.*

***The 15 family rows,** one talent each, all driven end to end: `edha-cae-grant` **Fast Talker** ·
`edha-def-test` → `edha-test-success` **Cascade Collapse** (both success payloads: the owner-list
release emptied the `omens` ledger and the two triggered effects landed `2d8` spirit + `disoriented`)
· `edha-watch` readers **Guardian Stance** (`edha-aura`, on the ally at 5 ft and NOT on the one at
85.6 ft) and **Resilient Hero** (`edha-hp-floor` held 10 HP − 40 damage at **5** = `max(1,
@skills.ath.mod)`) · `edha-damage-rider` **Prognosis** (`2d8 + 2 + ((2)d(2 * 3 + 2))[Prognosis]` vs a
condition-bearing target; NEG `2d8 + 2`) · `edha-test-rider` **Kneel** (`2d20kh + 4` vs a Weakened
target; NEG `1d20 + 4`) · `edha-burst` **Sudden Growth** (template on the clicked square, Detonate
named all three captured tokens with their damage) · `edha-move`/`edha-push` **Cruel Step** (exactly
10 ft toward the Isolated target) · `edha-apply-status` **Vital Diagnosis** · `edha-pulse` on
`edha-draw-mana` **Black Leyline Attunement** (*"swept 2 · newly Weakened 2 — of 6 enemies …"*) ·
`edha-hp-threshold`/`edha-multi-hit` **Mender's Instinct** + **Flashpoint** (both prompts posted and
both buttons resolved) · `edha-zone`/`edha-place-hazard` **Bastion** (`edha-content.enemy-cost` +
`edha-content.fortified`) and **Pyre** (`edha-content.hazard`) · `edha-summon` **Forge Construct**
(actor + token in `Edha Summons` with the baked Construct Slam) ·
`edha-prompt-pick`/`edha-owner-list`/`edha-marker-command` **Unweaving** (a button per borne effect;
the click deleted it) and **Foreknown Strike** (Spring Snare consumed the ledger entry and deleted
the Region) · `edha-focus`/`edha-temp-hp` **Sovereign's Favor** (temp HP 8 on the flag AND on the
sheet) and **Reaper's Harvest**. Evidence per row in the 2026-09-06 bench-run-41 delta.*

*⚠️ **One factual correction, made here rather than left to bite the next run:** the gated-test row
asked for "the FAIL rule on a miss", but **Cascade Collapse carries no `edha-test-fail` rule** — its
three rules are `edha-def-test` plus two `edha-test-success` payloads. The observable on a miss is
that **no** payload fires, measured twice on Spreading Omen's failures (no ledger entry, no status,
no damage).)*
