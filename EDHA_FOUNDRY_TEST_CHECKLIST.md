...
- [ ] 🤖 **FP-1 — False Premise denies a Reaction instead of duplicating Pattern Recognition's disadvantage:** on Bench — Blue, owning both `Pattern Recognition` and `False Premise`, have a target within Attunement Range succeed a Cognitive test: `Pattern Recognition`'s own `edha-next-test-mod` disadvantage still writes and posts its own card exactly as before — confirm it fires unchanged. Separately, react with `False Premise`, spend 1 Investiture, and test Blue vs. that target's Cognitive defense; on a success confirm the target now carries the `noreactions` status (icon on the token HUD) and cannot take a Reaction until the END of ITS OWN next turn — still on when that turn starts, cleared as that turn ends — not a second disadvantage stack, and not tied to your (the owner's) turn. **After REBUILD leyline + ⟳ Sync Talents,** re-read the owned False Premise card: it says the target *"cannot take Reactions until the end of its next turn"* (R-141 (a), fix pass 13).
      *(⚠️ **PARTIAL, bench run 49a (2026-09-15) — every clause PASSES except WHEN the status clears; the row stays open on R-141.** Bench — Blue vs Bench Target — Adjacent A (COG 14) in a started combat (turn order Adjacent A 0, Blue 1). **Pattern Recognition, unchanged:** its whispered card *"🔮 Pattern Recognition — if you succeeded on a Cognitive test against this character, spend 1 Investiture to give its next test this round disadvantage."* → *Impose disadvantage* → *"🎲 Pattern Recognition : Bench Target — Adjacent A's next test — at disadvantage."*, one `nextTestMod` entry `{mode: "disadvantage", count: 1, round: 1}`. **False Premise:** 17 vs COG 14 SUCCESS on Blue's turn → *"False Premise — Bench Target — Adjacent A is No Reactions (Extract Thought)."* — `noreactions` applied, stamped `expireAfter {round: 2, turn: 0}` (Adjacent A's own next turn, not Blue's `{2, 1}`), and `nextTestMod` still ONE entry — no second disadvantage stack. ❌ **The expiry clause:** at the START of Adjacent A's round-2 turn the status was still on; it cleared at the next turn change with *"💢 No Reactions (Extract Thought) on Bench Target — Adjacent A ends (end of its turn)."* — the END of its next turn, where this row and the card (R-115 (a)'s approved text) say the start. That is the engine's timed-status convention (`edhaExpireTimedStatuses` deletes a status only once the turn sequence has passed its `expireAfter`), which R-28 (a) kept for Withering Touch while fixing the prose — so which side moves is **R-141**. The "(Extract Thought)" label is TODO item 171.)*
      *(**R-141 ANSWERED (a) — Ben, chat 2026-09-15: "R-141, yeah fix the card text."** The END of the target's next turn is canon; fix pass 13 moved the card text (authored forms + source prose) and item 171 dropped the talent name from the status label, so the row above now asks for the END plus a card re-read after REBUILD leyline + ⟳ Sync Talents. The PARTIAL above is the evidence that raised the ruling, kept as history.)*
...
- [ ] 🤖 **CUE-161 — a defeated creature's enemy-turn-start cue stays silent:** in a started combat on Bench Arena, place two copies of an adversary whose ability carries an `enemy-turn-start` cue (the Rootling Swarm's Territorial Instinct — the YARD-1 case) adjacent to a bench PC. Drop ONE copy to 0 HP (confirm the system's dead marker lands) and advance to the PC's turn: only the living copy whispers *"⏰ … turn starts in range"*; the dead one posts nothing. Then mark the living copy **defeated** from the combat tracker with its HP still above 0 and advance again: it goes silent too. Control: with both copies alive, both cue.
- [ ] 🤖 **CUE-161b — a corpse struck again, and a fallen pack-mate, post no cue:** apply damage to an adversary already at 0 HP whose ability carries a `damaged` cue (the census lists them, `docs/analysis/bestiary/CENSUS.md` §5): no cue card. Then, with one member of a group carrying an `ally-drops` cue already down, drop a second member: only the standing members whisper the ally-drops cue; the one already down stays silent.
- [ ] 🤖 **THORN-162 — a creature's own Green terrain never burns it:** a bench character owning Green Leyline Attunement and Thorn Field Draws Mana and places the terrain square under ITS OWN token: no *"takes N keen from dangerous terrain"* card on placement, and none when its turn starts inside the square. A hostile token moved into the same square still takes the half [Tier][Die] keen. Repeat once with the Briar-Gone Grove placing its square under its own 2×2 token (the YARD-3 case). Unchanged on purpose: a turn-END hazard (Bone Garden) still catches its owner (R5).
- [ ] 🤖 **PACK-163 — Pack Hunter's advantage is spent only on its target:** a bench character owning Pack Hunter moves adjacent to a Trooper that an ally is also adjacent to, targets it and uses Pack Hunter: the card reads *"2 hunter(s) gain advantage on their next attack against Trooper"* and both actors' `flags.edha-content.advAttackNext` read `{source: "Pack Hunter", targetUuid: <that token's uuid>}`. Attack a DIFFERENT enemy first: that roll is NOT at advantage and both flags remain. Then attack the marked Trooper: 2d20kh, the *"🐾 Pack Hunter — advantage spent on this attack"* card, and the attacker's flag cleared.
- [ ] 🤖 **PACK-163b — a downed ally is not a hunter; a Medium hunter beside a Large enemy is:** with the Briar-Gone Grove (2×2) in the arena, put one ally orthogonally touching its edge and a second ally at 0 HP also touching it; Pack Hunter from a third PC touching it reads *"2 hunter(s) … against Briar-Gone Grove"*, and the downed ally gets no `advAttackNext`. (`edhaAdjacent` now reads token footprints, and every adjacency consumer shares it.)
- [ ] 🤖 **SNAP-164 — an even terrain square is centred on the grid vertex nearest the click:** with a Green rank-3 caster (a 10 ft square), HOVER the pointer over the grid vertex shared by two adjacent tokens first (a coordinate click sends no pointer-move, so `canvas.mousePosition` would be stale — bench run 49a), then click at the *"Click where the 10 ft difficult-terrain square grows"* prompt: the Region is 200×200 with its CENTRE on that vertex and covers both tokens' cells. Record the clicked world point and the Region's rect. Repeat once with Lay Foundation (10 ft) and once with a Green terrain burst (Sudden Growth): each square is centred on the snapped vertex. A 5 ft marker (Ordained Ground, a Snare) still lands on the clicked cell.
- [ ] 🤖 **SYNC-168 — ⟳ Sync Talents no longer lists the system's basic actions as not found:** on the three players' actors (the PM-R17 refresh — their own sheets' ⟳ Sync Talents button) or on a bench character given the cosmere system's basic actions, click ⟳ Sync Talents: the toast reads *"Edha: synced N item(s) on … (…)."* with NO *"— N not found in packs"* suffix, the console lists no missing items for the basic actions, and Draw Mana still counts as the one action.
- [ ] 🤖 **LBL-171 — the No Reactions status names no talent:** land False Premise from Bench — Blue (or Extract Thought from Bench — Black) on a target: the success card reads *"… is No Reactions."* with no "(Extract Thought)", the token's status tooltip and the effect's name read "No Reactions", and the expiry card reads *"💢 No Reactions on … ends"*. Hollow Command's marker reads "Cannot Act".
- [ ] 🤖 **VD-170 — the Diagnosed card's note agrees with its own number:** after **REBUILD deity + ⟳ Sync Talents** on Bench — Life, use Vital Diagnosis on a Trooper: the card's *"+3 vital (auto-applied)"* (Blue rank 3) is followed by a note saying *"additional vital damage equal to your Blue rank"* — no "+Tier vital" anywhere on the card. On Bench — Fate's Events tab, Bulwark Ground's rule description reads "Temp HP = your White rank".
