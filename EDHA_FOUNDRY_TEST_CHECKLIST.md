...
- [ ] **The counter economy stores a real number now — the run-9 gate row (FIXED 2026-07-27h; ENGINE-ONLY, relaunch / F5)** — if this row fails, every 2bT row below it is meaningless, so run it FIRST. Place Studied Mark on a bearer, then read the EFFECT, not the card: the `Insight` effect on the bearer must be **named `Insight [2]`** (the system re-derives the name from the stored stacks), `effect.system.stacks` must be **2**, and `effect.system` must contain **no `count` key at all**. Then the sheet handshake: open the bearer's sheet → Conditions → cycle Insight **up** by one → the engine's next read must see **3** (Killing Blow's ×count, Accumulate's cap, `@counter` — all one field now); cycle it **down to 0** → the status must disappear entirely. ⚑ The byte-check on the served engine (`edhaEffectStacks` ×2, `"system.stacks"` ×2, no `system.count` in code) is in DEPLOY STATE.
...
- [ ] **2bT-3 — Killing Blow — PARTIAL 2026-07-27g: the flow is right, the count is 0 (see the FAIL above)** — ✅ retired-in-place halves: no-bearer → refused pre-cost, nothing spent ("Edha: you have no creature bearing your Insight for Killing Blow. Nothing spent.", Investiture 4→4); YOU roll the Red test on the card; **ONE** `2d8` roll auto-applied to the bearer as vital on both branches ("⚡ Killing Blow (Bench — Knowledge) — 9 vital to Bench Target — Adjacent A"), the card telling you not to hand-apply; SUCCESS clears the bearer and the pointer, FAILURE decrements. ⛔ **RE-OPENED for run 9 (07-27h fix; ENGINE-ONLY, relaunch / F5).** With the bearer at **3** Insight: the SUCCESS branch must roll `2d8` and multiply it by **3** (compare against a 1-Insight bearer — the two must differ, which they never have), then clear the bearer and the pointer; the FAILURE branch must roll ×1 and leave the bearer at **2**, with the card saying "(now 2)" and not "(now 0)". Both numbers come from the same store, so a wrong one on either branch means the read, not the branch.
- [ ] **2bT-1 — Studied Mark — PARTIAL 2026-07-27g** — ✅ retired-in-place halves: the place card ("Bench Target — Floater bears 2 Insight (any prior bearer is cleared). Insight fades at the end of the scene."), the whispered snapshot **WITHOUT Cognitive defense** ("HP 32/33; conditions: Exhausted, Insight; defenses — Physical 14, Spiritual 14."), the documented two-card cosmetic drift, and **all three refusals with nothing spent** — no target ("target the creature first (nothing spent)"), self ("target a creature other than yourself. Nothing spent."), out of range ("Bench Target — Isolated is outside your Attunement Range (Green). Nothing spent."), Investiture 4→4 each time. ⛔ **RE-OPENED for run 9 (07-27h fix; ENGINE-ONLY, relaunch / F5).** The card said "bears 2 Insight" while the store held 0 — it printed the *intended* value without reading back, so it was truthful about intent and lying about state. Now the two must agree: after the place, the card's number, the effect's name (`Insight [2]`), and `effect.system.stacks` must all read **2**. Re-check the transfer case too (2bT-2's shape): marking a second creature must leave the new bearer at 2 and the old one with no marker at all.
- [ ] **2bT-6 — Hunter's Discipline — PARTIAL 2026-07-27g** — ✅ the own-hit half PASSES and is correctly bearer-gated: every own hit on the bearer posted "🐺 Hunter's Discipline (Bench — Knowledge): **+2 vital strike**" (+Tier), and a weapon hit on a creature that was NOT the bearer at that moment got nothing. ⛔ **RE-OPENED for run 9 (07-27h fix; ENGINE-ONLY, relaunch / F5).** The on-kill whispered `floor(count/2)` transfer offer never posted, because `floor(0/2) = 0` fails the `amt > 0` gate. Kill a bearer holding **4** Insight: the whisper must offer **2** and the candidate list must be the Green-range creatures. ⚠️ Note the true-negative: at 1 Insight `floor(1/2) = 0` and **no card is correct** — that gate is not the bug, the 0 was.
- [ ] **2bT-7 — Death Mark — PARTIAL 2026-07-27g** — ✅ the PUBLIC per-ally burst card PASSES: killing the bearer posted "📖 Death Mark: each ally in Attunement Range (… fifteen named …) deals [Tier][Die] Vital (**Bench — Knowledge's dice**) to any enemy of their choice", one button per ally, and clicking one with an enemy targeted resolved as "📖 Death Mark: Bench Ally — One deals **8** vital to Bench Target — Undefended" (`(2)d(2*3+2)`, HP 30→22) with the button flipping to "✓ Bench Ally — One → Bench Target — Undefended". R9 confirmed at sweep level: BOTH transfer rules ran on the same kill. ⛔ **RE-OPENED for run 9 (07-27h fix; ENGINE-ONLY, relaunch / F5).** The FULL-count transfer card never posted (count 0 → the `amt > 0` gate). Kill a bearer holding **3** Insight: alongside the ally-burst card that already works, a second card must offer the whole **3** to a new bearer, and clicking it must put 3 on the chosen creature (`Insight [3]`) — R9's "last click wins" then decides against Hunter's Discipline's 1, so click them in both orders once.
- [ ] **2bT-8 — Accumulate — PARTIAL 2026-07-27g** — ✅ retired-in-place halves: the turn-start tick fires **in range** ("📖 Accumulate: +1 Insight on Bench Target — Floater"), posts **nothing** with the bearer moved out of Green range, and fires again when the bearer is moved back — a proper negative *and* a back-in-range control, so the non-fire is the range gate and not a once-per-round guard. The damage→1 Investiture clause also fired unprompted ("🔮 Accumulate: the Insight creature took damage — Bench — Knowledge recovers 1 Investiture"). ⚠️ **Operating note: Accumulate's turn-start watch fires on `combat.update({turn})`, NOT on the `flags.cosmere-rpg.activated` flip** — the opposite of run 7's Foundation/Civ finding; driving the activation flag produced nothing. ⛔ **RE-OPENED for run 9 (07-27h fix; ENGINE-ONLY, relaunch / F5).** The cap-5 clamp was unreachable at count 0. Tick the bearer up turn by turn: the count must climb 1→2→3→4→5 (each card printing "(now N)" and the effect renaming to `Insight [N]`), and at 5 the sixth tick must post **nothing at all** — the point simply doesn't land. Then confirm the cap is the RULE's, not a constant: edit `capFormula` on Accumulate's Events tab and watch the ceiling move.
- [ ] **2bT-10 — The Pack — PARTIAL 2026-07-27g** — ✅ arm PASSES (`Pack Mind (pack strikes as one)` icon + the card naming the live-count rider), and the "**at 0 Insight the Pack line simply doesn't post**" negative PASSES — an ally hit with The Pack armed alongside Pack Share posted only Pack Share's +2. ⛔ **RE-OPENED for run 9 (07-27h fix; ENGINE-ONLY, relaunch / F5).** R10 and R11 were both unverifiable at count 0. With the bearer at **3**: an ally's hit must post BOTH riders — Pack Share's `+2` (+Tier) and The Pack's `+3` (`@counter`), **additively, as two separate cards** (R10) — and each must place its own 1 Insight once per round, tracked independently (R11): the second ally hit in the same round posts the bonuses again but places nothing, and a new round re-arms both placements. **Ruling default applied 07-27h (Ben can veto):** the card is canon, so the placement no longer sits behind `amt > 0` — The Pack now places on a qualifying ally hit even when its bonus is 0, which is the state a bearer lands in when the marker is cleared from the token HUD or cycled to 0 on the sheet while the owner's pointer survives. Drive that case once: clear the Insight effect by hand (leaving the pointer), have an ally hit the ex-bearer, and the placement must still land.
...
- [ ] **2bX-16 — Tagging Shot (heroic pack) — needs an actual RANGED weapon on the PC first (07-27h: the bench setup script never gave one out)** — use (arm); ranged weapon hit; separately arm then MELEE hit → Use: `tagged` icon on YOU (expires end of your next turn). Ranged weapon hit: arm consumed, victim gains the `quarry` icon + ledger card. Melee hit: rule stands down, arm SURVIVES. ⚑ a 0-damage graze is owner-judged. ⚠️ **Prerequisite, new 07-27h:** `bench-setup-console.js` chose the roster's weapons off `system.range`, which is not a cosmere field — so it always read undefined, every weapon classified as melee, and `rangedW` was never assigned for eight runs. The script now reads `system.attack.type`; **re-run it, or drag a bow on by hand and confirm `weapon.system.attack.type === "ranged"`** before believing this row or any other ranged/melee discrimination (Warlord's Advance's `meleeOnly` stand-down, Withering Touch's, `edhaAttackKind` generally).
