...
*(Balance review batch, 2026-09-13 — **R-120 … R-125**, filed from `docs/analysis/talent-ecosystem/BALANCE-REVIEW.md`, the review run on the honest intent after item 111. Six asks, one batch; each carries a recommended default, the alternatives, and the deploy class of every option. The arithmetic behind every number is `docs/analysis/talent-ecosystem/balance-turns.js`.)*
...
**R-124. deity/Civilization's `Tempered Edge` reads as if the Construct's whole attack ignores Deflect; every other Deflect bypass in the atlas is a damage type or a named die. Narrow it to the energy die?** `Forge Construct` (one Action, one Investiture, level 1) plus `Tempered Edge` (free Passive: "Your Combat Construct's melee attacks deal an additional [Tier][Die] energy damage and ignore deflect") and `Arsenal` (two attacks a turn) give the disciple an attacker on its own initiative doing **14 damage a round at levels 3–5 and 36 at level 7, free after about four Investiture of setup**, with the disciple's own three Actions untouched (67 a turn at level 7 with plain Strikes) — the second-highest sustained line in the game and the highest that costs nothing per round. Deity power is Radiant-tier by design and the Construct also absorbs attacks, so the size may be intended; the one clause doing more than its sentence suggests is the Deflect bypass, which read plainly covers the impact die too. *Recommended default: **(a) clarify `Tempered Edge` to "the energy damage ignores deflect"** (the impact die is still turned by armor) and accept the numbers otherwise. DATA (card text + the authored rule's scope) and, if the engine applies the flag to the whole attack today, a one-line scope change in the construct attack handler (ENGINE-ONLY, F5), REBUILD deity.* (b) accept as written — the whole attack bypasses (DOCS-ONLY: say so in the guide). (c) `Arsenal`'s second attack only on the round after a kill — a bigger change than the evidence asks for (DATA, REBUILD deity). *(Balance review, 2026-09-13.)*

Ask: Tempered Edge's "ignore deflect" — narrow it to the energy die (a), accept the whole-attack bypass (b), or throttle Arsenal instead (c)?

**R-125. deity/Power's two entries read the Frightened condition, and nothing in the game applies it — replace the dead word, delete it, or build a source?** `Kneel` ("You have an advantage on attack tests against any Compelled, **Frightened**, or Weakened character in Attunement Range") and `Absolute Authority` ("choose a Compelled, **Frightened**, or Weakened character") both read Frightened; across all 365 talents no talent applies it (`Risen Servant` is merely immune to it). The canon reference lists Frightened among the homebrew conditions the deity reviews promoted — Compelled was built, Frightened was not — so two of Power's nine cards carry a permanently dead clause. *Recommended default: **(a) replace Frightened with Disoriented in both** — a condition Chaos, Order, Red, White, Blue, Envoy and Leader all apply, which hands the one deity tree without an income (R-121) a little cross-path synergy for free. DATA (two card texts + the two authored condition lists), REBUILD deity.* (b) delete the dead word (honest cards, a hair weaker; DATA, REBUILD deity). (c) keep it as a hook and build a Frightened source later (no change now). *(Balance review, 2026-09-13.)*

Ask: Kneel and Absolute Authority read Frightened, which nothing applies — swap it for Disoriented (a), delete it (b), or keep it as a hook (c)?
