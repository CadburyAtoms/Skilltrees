...
*Recommended default: **(a) keep the doubled dice and the health cost, and add once per turn** — everything the surviving defence protects stays (Black pays in blood, and one cast is still twice `Searing Bolt`'s dice), and the three-cast turn goes. Card text + authored rule; pack rebuild.* (b) keep it exactly as it is, and write it into the guides as the leyline damage ceiling every comparison is measured against. (c) halve it to `[Tier][Die]` and keep it free of Investiture. *(Talent ecosystem review, filed 2026-09-12.)*

Ask: `Withering Ray` is the game's only doubled-tier damage, costs health instead of Investiture and can be cast three times a turn — add once per turn and keep the rest (a), keep it as the documented leyline ceiling (b), or halve its dice (c)?
...
*Recommended default: **(a) reword both cards to what the rules deliver** — `Fatal Thrust` gains one advantage if the weapon is Discreet; `Defensive Position` drops its "two disadvantages" clause and keeps "allies can Brace behind your shield". Card text in the authored overlay and the source prose; pack rebuild; no engine change.* (b) keep "two" and give each second grant a different, real payload. (c) move both to the summing channel, behind the bench check and cap R-98 (b) describes. *(Talent ecosystem review, filed 2026-09-12.)*

Ask: `Fatal Thrust` and `Defensive Position` promise a second advantage or disadvantage the binary rules cannot give — reword both to one (a), give each second grant a real payload (b), or move them to the summing channel behind a bench check and a cap (c)?
