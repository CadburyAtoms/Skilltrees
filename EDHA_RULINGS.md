...
> **Second read, 2026-09-14 — Ben questioned the finding's validity:** *"This is taking every talent in the Knowledge tree, right? The highest possible amount of Insight? … What's the damage curve per-talent-taken?"* **Answer: one talent.** `Predatory Strike` alone adds an Insight on every hit, so it reaches the cap of five by turn 2–3 with no other pick; with Draw Mana at rank (R-126) a Draw plus two strikes is sustainable from level 1 — **52 a turn at tier 1 and 111 at tier 2 from the first pick**, against Black's first pick (Withering Ray) at 33 / 72 paid in health, Warrior's second pick (Mighty) at 31 / 40, Civilization's fourth pick (Arsenal) at 39 / 67. The per-talent curve is `docs/analysis/talent-ecosystem/balance-per-talent.js` and `BALANCE-REVIEW.md` §2 finding 1. Under (b) the first pick lands at 34 / 59 — parity with Black — and the cash-outs keep the multiplier as the reward for deeper investment. **Item 130 waits for Ben's re-read of that table before it ships.**
