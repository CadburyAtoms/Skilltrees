...
**R-95. `Momentum's Edge` reads a derived-field OBJECT, so it probably does nothing — and if it worked it would be ten times every other damage rider in the game.** leyline/Red's `Momentum's Edge` (depth 2, **level 3**, Passive, no cost, no resource) carries `bonusFormula: "@movement.walk.rate"`. That field is a **DerivedValueField object** (`{value, override, derived}`), not a number — the engine says so itself at `19-red-momentum-frenzy.js:224`, documenting the *same mistake shipping once before* on 2026-07-27, when a raw `Number()` on that field made every `edha-move {byHalfSpeed}` move 0 ft. That path was repaired with `edhaDerivedNum`; the damage-rider path was not, and goes through `Roll.replaceFormulaData`, which does `String(value)`. So the rider very likely contributes nothing, and may error the damage roll — **a 🤖 bench row settles which.** Separately, the design intent was clearly the movement rate (the engine's own field hint reads "Bonus = your Speed via `@movement.walk.rate`"), and walk rate is `20 + 5·SPD` = **30 at SPD 2**, 40 with Surefooted. Every other damage rider in the game is worth 1–4: `Mighty` `(1 + @tier)`, `Hexmark` `@tier`, `The Unmooring` `@attr.int`, `Kindle` `@skills.red.mod`, `Burning Drive` half a rank die, `Predatory Patience`/`Prognosis` `1d6`. *Recommended default: **(a) `@attr.spd`** — the SPD attribute (0–5), which is what the card's "damage equal to your Speed" reads as (the system's own lang file names that attribute "Speed"), resolves cleanly as a flat roll-data shorthand, and lands in family at +2 to +4.* (b) keep the movement-rate intent and teach `edha-damage-rider` to resolve derived fields through `edhaDerivedNum` — Red then has a free level-3 Passive worth +30, which is a deliberate balance choice, not a fix. (c) something between: half the movement rate, or the rate capped at `[Tier][Die]`. *(Talent ecosystem review, 2026-09-09; `formula-audit.js` is the re-runnable check and finds no other instance in all 365 talents.)*

Ask: `Momentum's Edge` says "bonus impact damage equal to your Speed" — should that be the Speed ATTRIBUTE, +2 to +4 (a), or your movement rate in feet, +30 to +40 (b)?
...
**R-96. Should Blue and/or White get damage? — the question that started the review.** The measured answer is **no for both, and the reason is not "no damage is fine" but "the hole is somewhere else"**. On the 15-axis capability budget (21 trees, each profiled and adversarially re-checked twice), within leyline **Blue scores 28 and White 27 against Red's 25** — the two no-damage trees are *broader* than the tree that does all the damage, so the no-damage identity is genuinely paid for. What each actually lacks is specific and is not damage: **White is 52% trigger-gated with seven Reactions competing for one Reaction per round, and 22 of its 25 talents are inert unless a second body is in position** — its problem is *permission to act on its own turn*, not output. **Blue's three real teeth (`Counterspell`, `Ghostly Walls`, `Absolute Stillness`) all sit behind Blue 3+, i.e. level 6** — its problem is that the kit does not assemble until L6, which is also exactly when a Blue mage's Investiture problem is worst (15 Investiture-costing talents, zero bonus regen). *Recommended default: **(a) no new damage in either; fix the two specific problems instead** — give White one or two things to do on its own initiative at depth 0–1, and pull one of Blue's three teeth below the rank-3 gate.* (b) give White damage only — it already has the seam (`Retributive Guard` is spirit and its one damage talent). (c) give Blue damage only. (d) give both damage — cheapest to write, but it spends the clearest identity contrast in the leyline set and the budget numbers say it is not needed. *(Talent ecosystem review, 2026-09-09.)*

Ask: Blue and White measure as BROADER than Red on the capability budget, so the no-damage identity looks paid for — do we leave damage alone and instead fix White's action economy and Blue's level-6 assembly (a), or add damage to one or both anyway (b/c/d)?
...
**R-105. Heroic damage is flat from level 1 to level 10; leyline damage doubles at level 6. Designed, or an accident?** Heroic damage talents carry fixed formulas: `Devastating Blow` is `(2 + max(@tier − 2, 0))d8`, which is **2d8 at level 2 and still 2d8 at level 10**; `Wit's End` is 4d6 at both tiers; `Fatal Thrust` is a flat `4d4` forever; `Deadly Trap` a flat `2d4`. Leyline and deity damage is `(@tier)d(2 × rank + 2)`, which goes `1d6` (3.5) at Tier 1 rank 2 to `2d8` (9) at Tier 2 rank 3 — **because level 6 raises the tier AND the rank cap in the same step**. So a heroic character is ahead at levels 1–5, level at 6, and behind after; and the leyline curve has a cliff at exactly the level the heroic curve does not. Compounding it, every heroic tree is 100% available by level 5 while leyline trees are 72–84% (Blue worst), so heroic front-loads its options as well as its damage. The design guide's stated target is that "Leyline mages are mortal. Comparable to Heroic path characters" — which is true on average across levels 1–10 and false at every individual level. *Recommended default: **(c) accept the trade but document it** — a leyline character trading levels 1–5 for levels 6+ is a legitimate design, and session one starts at low level where heroic is ahead, which is the safer direction. Write it into the guides and the session-zero material so a player choosing leyline knows they are buying a late-game character.* (a) flatten leyline's L6 step by moving some rank-3 gates to rank 2. (b) give heroic damage a tier-2 step so it does not go nine levels without growing. *(Talent ecosystem review, 2026-09-09.)*

Ask: Heroic damage never grows between levels 1 and 10 while leyline damage doubles at level 6 — accept the trade and document it (c), soften leyline's level-6 cliff (a), or make heroic damage scale at tier 2 (b)?
