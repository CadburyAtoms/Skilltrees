...
**The phone card's default** (2026-09-15): the same card shows the entry's recommended default, and
`parseOpenRulings` reads it from exactly two shapes — an **italic** `*Recommended default: … *` span
(the colon is required, and `**(a)**` may sit inside it), or a bare italic `*Recommended*` marker
attached to a `**(x)**` option label. A **bold** `**Recommended.**` inside an option reads fine on
the page and parses as *"no default stated"*, which is what R-142 and R-143 showed on the phone
until they were answered in chat. File the italic form.
...
*(R-136 — a fooled PC's contest resolves against the phantom copy's defenses 0: as designed, with card text — ANSWERED 2026-09-14, moved to §K.)*

*(R-137 — adversaries follow the same rules as the PCs: attributes, skill ranks and talents; the PC attack model and its gate — ANSWERED 2026-09-14 in chat, filed in §K.17.)*

*(R-138 — the ¼-per-round line binds the encounter, not the card — ANSWERED 2026-09-15, moved to §K.)*

*(R-139 — on the PC model the defenses derive; HP, Focus and Investiture stay stated — ANSWERED 2026-09-15, moved to §K.)*

*(R-140 — R-134's per-hit rows read as expected damage per attack, grazes charged — ANSWERED 2026-09-15, moved to §K.)*
...
*(R-128 — blocks gain attributes; senses, speed and rolled tests derive — ANSWERED 2026-09-14, moved to §K.)*

*(R-129 — decision-free cues become effects in the nation passes, every rule labelled — ANSWERED 2026-09-14, moved to §K.)*

*(R-130 — the legacy nine sit in a `Legacy — Playtest Dungeon` folder — ANSWERED 2026-09-14, moved to §K.)*

*(R-131 — one invested human per act-1 nation, as-written kits from trees the party lacks — ANSWERED 2026-09-14, moved to §K.)*

*(R-132 — the census re-count closes the second balance pass; the mono-Blue moratorium lifts — ANSWERED 2026-09-14, moved to §K.)*

*(R-133 — three card rules: ring adjacency, shade for the glare, the swarm rule as written — ANSWERED 2026-09-14, moved to §K.)*

*(R-134 — party-relative per-role targets, verified by the yardstick fights before any retune — ANSWERED 2026-09-14, moved to §K.)*

*(R-135 — Corvaine + Riverlands → Thalendor → Malcurr → the rest; yardstick fights first — ANSWERED 2026-09-14, moved to §K.)*
...
> **Post-design, 2026-09-17 (item 212, `docs/design/channel-deity.md` §5, gate 5 J-2 (a)):** the Channel is the leyline agency work this ruling waited for, and the deity pass measured the result: the five modifier Reactions become Specials (0 → 5, ~7 %), and the three armed strikes became *faces* of their trees' supplies rather than Specials. **The target is restated to ~5 – 10 %**, reached by that rule alone; no standalone Action is retyped to move the column. The guide's table is replaced by the measured mix (J-1 (a)).
...
> **PR #383 (2026-09-14, items 142 + 150)** — the placement half ships, and R-122 (a) is now fully implemented. H3 `edha-owner-list` gained the ONE generic dial the gap named: `onMissing: "continue"` (a release that finds nothing stops doing nothing *and* stops halting — default `"halt"` unchanged) plus `whenTargetStatus` / `unlessTargetStatus`, both read against an ENTRY SNAPSHOT of the target's statuses taken before the batch's first rule runs, so the two branches cannot un-gate each other whatever order they sit in. `Isolating Pressure` now carries release → damage (`whenTargetStatus: omen`) → place (`unlessTargetStatus: omen`, `@tier + 1`, `evict: refuse`), and its card gains *"If the target bears no Omen, place one."* Item 150 rode along: every `op: release` rule in the tree now states `capFormula: "@tier + 1"` outright, so the release cards stop printing a denominator of 2 at a live cap of 3.
...
### K.13 — R-92, R-114 … R-119 (the design proposals, answered 2026-09-13 from the phone)

R-92 (bench run 44, 2026-09-09; relocated into §L on 2026-09-13 for lacking an Ask line) and R-114 … R-119 (item 127's six design proposals for items 101/106/107/108/109/114, filed 2026-09-13 into §L) were all seven answered by Ben from the mobile board's "Needs you" view at 21:57–21:59 ET on 2026-09-13, each by tapping the card's own (a) text — the PM saved the seven notes verbatim to `tmp/pm/inbox-2026-09-13/notes.json` (gitignored). Every answer is option (a), the recommended default.
...
> **ANSWERED (a) — Ben, phone board 2026-09-13 21:59 ET (the row's (a) text tapped): "add a generic drop cue."** → **item 144** (ENGINE-ONLY, F5).
...
> **ANSWERED (a) — Ben, phone board 2026-09-13 21:57 ET (the row's (a) text tapped): "gate it at the emitter."** → **item 101** (ENGINE-ONLY, F5).
...
> **ANSWERED (a) — Ben, phone board 2026-09-13 21:58 ET (the row's (a) text tapped): "the target cannot take Reactions until the start of its next turn."** → **item 107** (DATA-only, REBUILD leyline).

> **PR #374 (2026-09-14)** — False Premise's card text (`data/leyline.json` + `data/authored/leyline-blue.json`) ships R-115 (a)'s approved text verbatim, and its `FalsePremMod0000` rule swaps from `edha-next-test-mod` to the engine's existing `noreactions` timed status (`statusExpire: "target"`, the same shape as Hollow Command's `noactions`); Pattern Recognition untouched. Pack parity (scratch `edha-leyline`): of 136 leyline-pack documents, exactly one — False Premise — differs, zero roll-formula drift anywhere. Item 107 done.
...
> **ANSWERED (a) — Ben, phone board 2026-09-13 21:58 ET (the row's (a) text tapped): "replace the advantage grant with a free Reactive Strike against the violator."** → **item 109** (ENGINE-ONLY `edhaDecreeResolve` edit + card, REBUILD deity).
...
> **ANSWERED (a) — Ben, phone board 2026-09-13 21:58 ET (the row's (a) text tapped): "link a Foundation to a point, not to a second Foundation."** → **item 114** (ENGINE-ONLY `edhaZoneLink` point mode + card, REBUILD deity).
...
> **ANSWERED (a) — Ben, phone board 2026-09-13 21:58 ET (the row's (a) text tapped): "one small sizing change per tree, all DATA-only."** → **item 108** (DATA, REBUILD deity).

> **PR #375 (2026-09-14)** — all three dials ship: Fate's `Bulwark Ground` `thpFormula` and Life's `Vital Diagnosis` `bonusDamageFormula` move off `@tier` onto `@skills.white.rank`/`@skills.blue.rank` with their reworded card lines; Destruction's `Cascading Failure` resizes only its `doubleCaughtFormula` overlap-bonus clause onto `@skills.blue.rank`, leaving its own base `damage.formula` and every other Destruction formula on Red (its card never named a colour, so it is unchanged). Pack parity (scratch `edha-deity`): exactly three of 110 documents differ, each in one formula field plus its description copies where the card names a rank, zero other formula diffs anywhere. `deity-gate-audit.js`: Fate/white 0 → 1 talent, Destruction/blue 1 → 2, Life/blue 1 → 2. Item 108 done.
...
> **ANSWERED (a) — Ben, phone board 2026-09-13 21:58 ET (the row's (a) text tapped): "retrofit Decree of Ruin into the radius version."** → **item 106** (ENGINE H9 radius mode + DATA, REBUILD deity; the H9 mode may proceed on (a), but the card text still waits on Ben's own design text, chat 2026-09-13).
> **Card text APPROVED — Ben, phone board 2026-09-14 09:23 ET: "Go with your card text for the Sovereignty items."** → item 106's `Decree of Ruin` card ships as proposed above (the full text in this entry); nothing on item 106 waits on Ben any more — the H9 radius mode and the card go in one Opus dispatch.

*(TODO item 106, from R-97 (a), the talent ecosystem review, 2026-09-09/13; unblocks item 106 only.)*

### K.14 — R-127 (bench run 47, answered 2026-09-14 from the phone)

Filed 2026-09-14 from bench run 47's BR-1 row (the card half of the same finding is TODO item 149, not part of the ruling); answered (a) from the phone board at 09:20 ET the same day. Moved here verbatim by the bestiary close-out that evening — the move the 09:25 board note said would ride the next close-out.

**R-127. A talent spends its cost, passes its test, and then the target turns out to be IMMUNE to the condition it was buying. Does the caster get the cost back?** What is live: nothing gives it back, and nothing tells the caster either. Measured at **bench run 47 (2026-09-14)**: `Bench — Power` spent 1 Investiture on `Kneel`, rolled 25 against a Risen Servant's Cognitive 11 — a clean success — and the Compelled status was refused outright by the summon's condition immunity (`ui.notifications`: *"Risen Servant (Bench — Death) is immune to Compelled"*, the actor's `statuses` stayed empty). The Investiture was gone, the Action was gone, and the talent's own card cheerfully announced *"🎯 Kneel : Risen Servant (Bench — Death) is Compelled (by Bench — Power)"* for a condition that never landed. **The card half is a plain bug and is already filed as TODO item 149 — it will be fixed either way; this ruling is only about the economy.** Why it needs you: Edha already has a *precedent on each side*. The pre-cost gates (`requireTargetStatus`, the Black-range checks) refuse **before** cost and say so — *"…must be compelled / disoriented / weakened for Absolute Authority — nothing spent"* — so "the talent could never have worked on this target, so it costs nothing" is an established idiom. But the post-test failure path charges full price for nothing: `Absolute Authority` on a legal target that then **fails** its test still spends 2 Investiture and delivers only the consolation Weakened. Immunity sits between those two: like a gate in that the outcome was never possible, like a failed test in that the dice were already rolled. Which one it is decides whether a GM who drops an immune creature into a scene is taxing the Power player twice. *Recommended default: **(a) treat immunity as a pre-cost gate — refuse the activation before cost when EVERY status the talent would apply is one the target is immune to, with the same "nothing spent" card the existing gates use.** It matches the idiom the player already knows, it never silently eats a resource, and it is one predicate next to the `requireTargetStatus` check rather than a refund path (refunds are the hard case: the cost may have been paid from Reserve, from ritual HP, or from a pool that has since changed). A talent whose payload is only partly immune still fires and pays, and item 149's honest card then says which part landed.* (b) charge as now, but make the card tell the truth — item 149 alone, no economy change; simplest, and "the dice were rolled, the price is paid" is defensible, but a GM can then farm a Power player's pool with immune summons. (c) refund the resource after the fact when nothing landed, leaving the Action spent — kindest to the player, but it needs a refund path through every cost channel (Investiture, Focus, Reserve, ritual HP) and can double-credit if the pool moved in between. (d) leave it to the table — document it in the guides as a GM call (DOCS-ONLY). ENGINE-ONLY under (a), (b) or (c); F5 either way, no rebuild. *(Filed 2026-09-14 by bench run 47, from the BR-1 row.)*

> **ANSWERED (a) — Ben, phone board 2026-09-14 09:20 ET (the row's (a) text tapped): "treat immunity as a pre-cost gate — refuse the activation before cost when EVERY status the talent would apply is one the target is immune to, with the same 'nothing spent' card the existing gates use. … A talent whose payload is only partly immune still fires and pays, and item 149's honest card then says which part landed."** → **item 149's second half** (ENGINE-ONLY, F5): one predicate beside the `requireTargetStatus` pre-cost check, never a refund path. *(The marker closes it for the phone and the test pin; the move to a §K.14 stub rides the next rulings close-out.)*

### K.15 — R-128 … R-135 (the bestiary redo's menu, answered 2026-09-14 from the phone)

Filed at 22:1x ET by the session that scoped item 121 with Ben ("This looks good. Continue.") and answered from the mobile board's "Needs you" view between 23:19 and 23:23 ET the same night — seven by tapping the card's own (a) text, R-128 by (a) plus a gloss. The consequences: items 155–160 in `TODO_REPO_HYGIENE.md`, `.claude/skills/bestiary-forge/STANDARD.md`'s PENDING lines flipped to RULED, canon ruling 164, and the close-out PR's data + build changes. Each block below is verbatim as filed, with its answer line.

**R-128. Should adversary blocks carry attributes — so senses, speed and rolled tests derive the way a PC's do — or stay at attributes 0 with per-block `senses` / `movement` overrides and skill ranks standing in for attributes?** Every block has attributes 0: `advActorSystem` in `scripts/foundry-build.js` writes none. Three consequences, all measured in the census: **51 of 52 blocks see 5 ft in the dark** on sheet and token alike (the cosmere ladder `[5, 10, 20, 50, 100, ∞][ceil(AWA/2)]` at AWA 0 — R-56 final, item 83; only Briar-Gone Grove states `senses: 30`); 46 of 52 state `movement` and the rest inherit the 25 ft default; and **41 of 52 carry explicit skill ranks that do the work of attribute + rank** — Callthief `dec: 4`, The False Spring `stl: 4` and `dec: 4`, above any tier-1 PC's rank cap of 2. Canon ruling 40 said *"rolled color tests run soft for now (adversary attributes stay 0) — revisit if a future boss pulls a rolled attack talent"*; nine abilities now roll `edha-def-test` contests and seven carry test riders. Defenses, HP, Deflect and attack modifiers are overrides and would not move under any option. Your item-82 ask (2026-09-07) — *"the actor tokens for the adversaries will inherit the correct sight range, speed, etc."* — is exactly this question. *Recommended default: **(a) attributes on every block** — add `attributes: {str, spd, int, wil, awa, pre}` to the schema and the build; senses derive from AWA through the ladder (a wolf with AWA 3 sees 20 ft), speed from SPD where a block states no `movement`, and skill tests roll attribute + rank so the 41 ranked blocks can come down to PC-honest ranks in a re-derivation pass that keeps every total (rank 4 → attribute 2 + rank 2). `senses` stays as the override for a creature whose fiction beats the ladder (the eyeless grove at 30 ft), because the ladder is coarse. Cost: build + a pinned test + the schema note, then adversaries REBUILD + ⟳ Sync.* (b) stay at 0 and state `senses` and `movement` on every block — item 82's original shape, a 52-value batch you approve as one menu, no build change, ranks keep standing in for attributes; DATA, REBUILD. (c) attributes only where a block rolls or should see — AWA for sight, the rolling attribute for its contests, the rest 0; same build change as (a), smaller data pass. *(Bestiary redo, item 121; absorbs item 82.)*

Ask: Should blocks gain attributes so senses, speed and rolled tests derive (a), stay at 0 with explicit senses and movement on every block (b), or gain only the attributes they use (c)?
> **ANSWERED (a) — Ben, phone board 2026-09-14 23:21 ET, with a gloss, verbatim: "Default, but note how vision is set up on the PC actors. Senses range of 20 feet means it can sense things 20 feet away with its primary sense obscured. That is- in dim light. In a normally lit room it should see as far as the light goes."** Build support landed the same night (the close-out PR: `attributes` on blocks, AWA → the token's Senses Range on the system ladder, the pool from max(AWA, PRE); `tests/adversary-attributes.test.js`), the gloss is written into the schema note and STANDARD.md §2, and the VALUES land nation by nation behind the statblock gate (R-135), never as a bulk batch. Item 82 closed as absorbed; item 160 carries the build change.

**R-129. Seventy-four of the bestiary's 185 rules are whispered GM cues. Which cues should become effects, and which stay cards?** The census (§5) counts 28 `hp-below` cues, 17 `on-hit`, 12 `enemy-turn-start`, 7 `damaged`, 5 `ally-drops`, 3 `turn-end` and 2 `seeming-break` — every one an `edha-gm-cue`, against 111 rules that are effects and 64 items that roll natively. The 07-16 wiring standard set the cue as the FLOOR *"when the decision stays at the table"*, and it has been the ceiling too: no pass has asked which cue texts name no decision. Some plainly do (Heat of the Flats' glare needs a shade read; a morale withdrawal needs the GM to move the token); some plainly do not (a threshold that only says "it frenzies: +1 damage" could be an `edha-self-status` at `hp-below`). *Recommended default: **(a) a policy, applied in the nation-by-nation pass** — a cue whose text names no table decision becomes an effect where a hook exists, a cue that names one stays a card, and every rule's `description` is labelled `cue — table call: <what>` or `effect — decision-free`, so the next census can count the split and a lint can one day check it. Conversions ride each nation's REBUILD; no separate pass.* (b) keep every cue as it is — the whispered card is the designed floor and adversaries are GM-run by nature; docs only. (c) convert every threshold cue (`hp-below` frenzies and withdrawals) to an engine-applied status marker in one pass now; ENGINE-ONLY where the statuses exist, otherwise DATA + REBUILD. *(Bestiary redo, item 121.)*

Ask: Should decision-free cues become effects during the nation pass with every rule labelled cue-or-effect (a), stay cards (b), or should all threshold cues convert in one pass now (c)?
> **ANSWERED (a) — Ben, phone board 2026-09-14 23:21 ET (the card's (a) text tapped).** The policy is STANDARD.md §4; each nation pass (item 156) labels every rule `cue — table call: <what>` or `effect — decision-free` and converts the decision-free ones where a hook exists.

**R-130. The nine legacy playtest-dungeon blocks — retire them, shelve them in a legacy folder, or reskin them as Thyrcross creatures?** Trooper, The Living Lock, Stonebound Captain, Wrenchmaster, Cinderhound, Frostbinder, Stalker, Stitchmother and Mutated Thrall carry no `folder` (the build files them under the legacy "Playtest Adversaries" folder). They are the 6-room test dungeon the data file began as in July — constructs and aberrations from the old cheatsheet, not fauna canon §5c knows — and until 2026-09-14 the file's own purpose line still called them its reason to exist. They are not idle, though: six of them are the only users of `data/adversary-effects.json`'s baked effects, Cinderhound is the **sole consumer** of `edha-pack-advantage` and `edha-thorns` and Stalker of `edha-dark-veil` (census §6), and a Cinderhound sat at 6/14 in the live combat bench run 46 found on "Playtest Map (Copy)" (R-113) — they have been your test fixtures. Ten blocks are unattuned; these nine plus the Corvaine Raider. *Recommended default: **(a) a `Legacy — Playtest Dungeon` folder** — they keep building, the three sole-consumer primitives keep a live fixture, the campaign folders stop sharing a root with them, and the census keeps counting them apart (it already flags them). DATA, adversaries REBUILD.* (b) delete the nine and `adversary-effects.json`'s six entries, after re-pinning the three sole-consumer handler types on a bestiary block or a headless test fixture; DATA + tests, REBUILD. (c) reskin them one by one as Thyrcross creatures through the lore gate — a concept pass per block, the largest option. *(Bestiary redo, item 121.)*

Ask: The legacy nine — a legacy folder (a), deletion after re-pinning their sole-consumer primitives (b), or reskinning through the lore gate (c)?
> **ANSWERED (a) — Ben, phone board 2026-09-14 23:22 ET (the card's (a) text tapped).** Landed the same night (the close-out PR, item 157): the nine carry `folder: "Legacy — Playtest Dungeon"`, the build creates the old top-level fallback only when a block lands in it (none does), the census counts them apart by folder name. Adversaries REBUILD + ⟳ Sync owed.

**R-131. Invested-human adversaries were named W29's mechanical-balance lever, and exist on two of 52 blocks. How many, where, and built how?** W29's close-out (2026-07-20) says *"mechanical balance also arrives later via invested-human adversaries — the ecosystem balances first."* Canon ruling 40: humans use tree talents AS WRITTEN, the actual talent embedded, costs and all; R-94 (2026-09-09): no prerequisites or rank gates apply, and `python scripts/validate-build.py --adversaries` prints what a pick costs the players, informational only. Today the Corvaine Line-Caller carries White's Guiding Signal and Ordered Advance, the Sevenbrand Construct-Smith carries three Civilization talents, and Sergeant Roek runs "tree-kin, run natural" traits by note — five talents on two blocks. One design fact bears on the answer: the party is Blue / White / Green with no fighter, so an invested human is also the only way the players will SEE Red, Black or the deity trees they do not hold. *Recommended default: **(a) one invested human per act-1 nation the party reaches, at rival role, carrying a kit of two to four as-written talents from a tree the party does not hold**, drawn from canon §5b's named factions (the Sevenbrand smiths, the Kenmere cult, the Canticle Law-Singers, Corvaine's well-wardens) — proposed as concepts at the lore gate, statted through the statblock gate, one per nation pass; DATA, REBUILD per nation.* (b) build invested humans only as sessions need them — session-forge's cast pass — with no bestiary quota. (c) a full roster, one per leyline colour and one per deity path the party will meet, as its own pass after the nations. *(Bestiary redo, item 121.)*

Ask: Invested humans — one per act-1 nation from canon's named factions with as-written kits from trees the party lacks (a), only as sessions need them (b), or a full colour-and-deity roster (c)?
> **ANSWERED (a) — Ben, phone board 2026-09-14 23:22 ET (the card's (a) text tapped).** Item 158: one invested human per act-1 nation, rival role, a kit of two to four as-written talents from a tree the party does not hold, from canon §5b's named factions — concepts at the lore gate, blocks at the statblock gate, riding each nation pass.

**R-132. The statted colour ledger reads Blue 11 / Black 9 / Red 8 / White 7.5 / Green 6.5. Does the mono-Blue moratorium lift, and is the second balance pass a re-count or work?** Canon ruling 106 (a) put mono-Blue on moratorium *"until the ledger recovers"* when the count was Blue-heavy 2:1 and White had ONE statted block; ruling 109 queued *"a second balance pass after the Red countries"*. W29's ledger at 07-20 read Blue 8.5 / Green 6.5 / Black 4.5 / White 3 / Red 3.5 statted; Vorsk, Ashkar and Kettavar landed 07-20 → 07-22 and no re-count was ever recorded. The census (§3, pairs counting ½) reads it today: **Blue 11, Black 9, Red 8, White 7.5, Green 6.5**, with 10 unattuned blocks. Blue leads the median by three blocks; White has gone from 3 to 7.5. Your bar (ruling 106): *"the ratios don't need to be perfect, but it shouldn't be lopsided."* *Recommended default: **(a) record the census re-count AS the second balance pass, lift the mono-Blue moratorium, and steer new entries toward Green and White (the two lowest) in the nation passes** — DOCS-ONLY: a canon §9 ruling line and the standard's §1, and the census keeps the ledger live so no third pass needs scheduling.* (b) keep the moratorium until Blue sits at or below the median — three fewer mono-Blue blocks or six more elsewhere. (c) run a real second balance pass now, concepts and blocks, before any nation pass. *(Bestiary redo, item 121; canon rulings 106, 109.)*

Ask: Does the census re-count close the second balance pass and lift the mono-Blue moratorium, steering new entries Green and White (a), does the moratorium hold until Blue reaches the median (b), or does a full balance pass run first (c)?
> **ANSWERED (a) — Ben, phone board 2026-09-14 23:23 ET (the card's (a) text tapped).** Canon ruling 164 records the census re-count as the second balance pass and lifts the mono-Blue moratorium; STANDARD.md §1 steers new entries toward Green and White; the census keeps the ledger live. DOCS-ONLY.

**R-133. Three of the five shelved bestiary feel rows ask for a written rule, not a feel. What are the rules?** You shelved the five on 2026-09-13 (*"the bestiary 'how does this feel' items can all be shelved"*). Two are feel and need play — Cold-Fire Cinderbrock's PITIABLE read and the Dirgehounds' pack-or-mob — and stay shelved until a session plays them. Three are decisions a card can carry today: **(i) Crownox Ring** — the card says an ox pulled 10+ ft "loses the wall kit" and nothing measures it; the row asks whether that is a distance, a broken adjacency chain, or a GM eyeball. **(ii) Heat of the Flats** — the glare cue has no shade clause and no hook could give it one; the row asks what counts as shade (a wall? any cover? only a roofed square?). **(iii) Swarm bookkeeping** — half damage from single-target Strikes, scatters on AoE, GM-run under the Wake-Eel precedent (no nameable hook); the row asks for a sanity read of the rule itself. *Recommended default: **(a) three card rules — (i) adjacency: an ox keeps the wall kit while within 5 ft of at least one other ring ox (with R-52's 2.5 ft slack), and loses it the moment it is not; (ii) shade = cover from the glare source or a roofed square, the GM's read, stated on the card so the whispered cue can carry the words; (iii) the printed swarm rule stands as written, GM-run, with its `noHook` reason kept** — DATA card text, adversaries REBUILD + ⟳ Sync; the three rows retire on the text landing, the two feel rows re-open after play.* (b) different rules — write them in the answer and they go on the cards verbatim. (c) leave all five shelved until play. *(Bestiary redo, item 121; the rows shelved by item 119's PR.)*

Ask: For the three shelved rows that need a rule — adjacency for the Crownox ring, cover or a roof for shade, the printed swarm rule GM-run — take the defaults (a), write different ones (b), or leave all five shelved until play (c)?
> **ANSWERED (a) — Ben, phone board 2026-09-14 23:22 ET (the card's (a) text tapped).** Landed the same night (the close-out PR, item 159): (i) Crownox Ring / The Ring carries the adjacency rule; (ii) The False Spring / Heat of the Flats carries the shade clause on the card and on the whispered cue; (iii) the swarm rule stands as written with its `noHook`. The three rows retired; the two feel rows (Cold-Fire Cinderbrock, Dirgehounds) wait for play. Adversaries REBUILD + ⟳ Sync owed.

**R-134. Should the standard carry explicit per-role numeric targets against THIS party, so a new block's numbers are derived rather than eyeballed against its neighbours?** The census (§2) reads the bands the 52 blocks occupy: a minion averages 12 HP, attack +4 to +5 and 5 expected damage per hit; a rival 27.5 HP, +4 to +8 and 7; a boss 66 HP, +6 to +9 and 9.3. Against the party those are large: a level-1 PC has `10 + STR` HP (Tem, the highest Physical, 13) and Deflect 0 without armour, so **a minion's average hit is about 40% of a PC's HP, a rival's about 55%, a boss's about 70%**; a PC Strike with Mighty is about 10 (the ecosystem critique's per-Action arithmetic), so a minion drops in one or two hits, a rival in three, a boss in seven. Bench run 44 measured the ford at 18 damage to the party against 7 to the raiders over three rounds on already-softened numbers, for structural reasons (no ranged attack; grazes flooring damage) rather than the statblocks. The party has no fighter and no ranged attack until they shop. Nothing in the repo says what a minion, rival or boss SHOULD do to this table; the numbers were set block by block. *Recommended default: **(a) party-relative targets as ratios in STANDARD.md §3** — a minion's average hit at most ⅓ of a level-1 PC's HP (about 4), a rival's at most ½ (6 to 7), a boss's at most ⅔ (8 to 9); a minion drops to one average PC Action, a rival to two or three, a boss to six or more; an encounter's expected damage in per round at most ¼ of the party's HP pool at the "bruise, not wipe" tier. The current bands sit at or a little above those, so the yardstick fights (item 155) verify before any block is retuned — no retune ships on the arithmetic alone.* (b) adopt the measured bands as the targets — practice is policy — and retune nothing. (c) take tier-based targets from the published system's adversary guidance instead of party-relative ratios. *(Bestiary redo, item 121; R-101 (a) is the licence to read the numbers.)*

Ask: Should the standard carry party-relative per-role targets — a hit at most ⅓ / ½ / ⅔ of a PC's HP, one / two-to-three / six-plus PC Actions to drop — verified by the yardstick fights before any retune (a), adopt the measured bands as-is (b), or take tier-based targets from the system (c)?
> **ANSWERED (a) — Ben, phone board 2026-09-14 23:19 ET (the card's (a) text tapped).** The ratios are STANDARD.md §3's targets; the yardstick fights (item 155) verify against them before any block is retuned.

**R-135. In what order does the nation-by-nation data pass run, and does it wait for the yardstick fights?** Session 1 (built, unplayed) uses the Corvaine Raiders, the Line-Caller, Sergeant Roek and the Riverlands' Mistherons; session 2's briar grove is Thalendor (the Rootling Swarm and the Briar-Gone Grove are statted; the grove-heart is terrain-scale by ruling 40, a negotiation not a fight); the campaign state names Malcurr as the northern act-1 branch. Twelve bestiary folders exist; the Corvaine River-Plains folder holds one block (Surecat) and the Riverlands two; Sylvaneth has none and is gated on W20 (the Fae). Each nation's pass depends on R-128 (what a block states), R-129 (what a cue becomes) and R-134 (what the numbers aim at); the yardstick fights need no ruling. *Recommended default: **(a) Corvaine + the Riverlands → Thalendor → Malcurr → the remaining nations in canon order** — each nation waits only for the rulings it needs; the first yardstick set (a minion pack, a rival pair, a boss on copies of the actual PCs) runs BEFORE Corvaine's pass so the retune has numbers, and the standard's `movement` / `senses` requirement (or attributes, per R-128) lands in the same pass as each nation's content.* (b) one mechanical pass over all 52 first — attributes or overrides, cue labels, weapon kinds — then content nation by nation. (c) only what sessions need — session-forge pulls and fixes blocks as it goes; no standalone pass. *(Bestiary redo, item 121.)*

Ask: Order — act-1 nations first, each waiting only on its rulings, with the yardstick fights before the first (a); one mechanical pass over all 52 first (b); or only as sessions need (c)?
> **ANSWERED (a) — Ben, phone board 2026-09-14 23:19 ET (the card's (a) text tapped).** The order is item 156's plan: Corvaine + the Riverlands → Thalendor → Malcurr → the rest in canon order, each nation waiting only on its rulings (all eight answered), the yardstick fights (item 155) first.

---

### K.16 — R-136 (bench run 48, answered 2026-09-14 in chat)

Filed 2026-09-14 from the yardstick set's YARD-2 ledger (item 155); answered (a) by Ben in chat the same evening, in the message that approved the Corvaine + Riverlands statblock gate. Moved here by that session.
...
> **ANSWERED (a) — Ben, chat 2026-09-14, verbatim: "Default for R 136 as well."** → the sentence is on the Mistheron's The Seeming card and on Phantom Double's card (its authored value / chat / short forms and the source prose) — DATA: adversaries REBUILD + ⟳ Sync Adversaries, leyline REBUILD + PC ⟳ Sync Talents for Phantom Double's owners. No mechanic changes; the copy keeps HP 1 and defenses 0/0/0.

### K.17 — R-137 … R-140 (the attack-model rerun, answered 2026-09-14 and 2026-09-15 in chat)

Filed 2026-09-15 by the correctness rerun of the bestiary redo (item 156's Corvaine + Riverlands pass and the Thalendor pass; TODO item 166). R-137 was answered before it was filed — Ben's ruling in chat on 2026-09-14, when the finding was first raised — and is written here in the §K.16 shape with a §G stub; R-138 … R-140 were filed on the gate page (https://claude.ai/artifact/UciQTpc4TcAej9thP71tBQ) and answered in chat on 2026-09-15: *"you have my permission to continue with everything as presented and defaults on all rulings except what I mentioned above"* (the exception was the Thalendor preacher's colour — White, not Green — not a ruling).

**R-137. A stated Strength or Speed adds to an adversary's attack AND to its damage on top of the flat card numbers — do adversary attacks ignore attributes, or do adversaries roll like PCs?** The build wrote every adversary attack as a Heavy / Light Weaponry test with the block's whole attack bonus in `modifierFormula` and the whole damage inside the formula ("so PDF numbers hold at any skill rank"), which was right only while every block's attributes were 0. The cosmere system rolls an adversary's item exactly as a PC's: `@mod` = skill rank + attribute on the d20 (`getSkillTestRollData`), the same modifier appended to the damage (`rollDamage`, the damage skill falling through to the attack skill), the graze the bare dice. R-128 (a) let blocks state attributes; PR #388 stated them on seven; the deploy of 2026-09-15T01:50Z put them live. Verified on fresh imports the same night: the STR 2 Corvaine Raider's +4 / 1d6+1 Shortsword rolled `1d20 + 2 + 4` and `1d6 + 1 + 2`; Roek `1d20 + 2 + 5` and `1d8 + 2 + 2`; the attribute-less Cinderhound `1d20 + 0 + 4` and `1d6 + 2 + 0`. R-128's menu had said attack modifiers "would not move under any option". *(a) adversaries follow the same rules as the PCs — attributes, skill ranks in relevant skills, talents off the trees; a block that states attributes derives its attacks (attack = d20 + attribute + rank, damage = dice + the same modifier, graze = the dice), the flat numbers are refused there by a gate, and the flat model stays legal only on blocks that state no attributes until their nation pass. TOOLING + DATA, adversaries REBUILD + ⟳ Sync.* (b) adversary attacks ignore attributes — the previous session's proposal: the build strips the attribute from adversary skill tests and damage so the card's flat numbers hold; ENGINE or build work against the system's own model. *(Bestiary redo, item 156; the rerun, item 166.)*

> **ANSWERED (a) — Ben, chat 2026-09-14, verbatim (expletives elided): "adversaries need to follow the same rules as the PCs do, that's how the whole system works. They have attributes, they have skill ranks in relevant skills, they have talents right off the talent trees!"** → the PC attack model (`.claude/skills/bestiary-forge/STANDARD.md` §3 "the attack model"; `scripts/foundry-build-parts.js` `advAttackModel`), the `adversary-model` gate (`scripts/validate-adversary-model.js`, pinned by `tests/adversary-model.test.js` — a block with STR 2 and a flat +4 attack fails), the eleven migrated blocks re-statted with one skill modifier on test and damage, the census counting the two populations (12 on the model, 42 flat on 2026-09-15). Canon ruling 40's "adversary attributes stay 0" is superseded (bracketed there).

**R-138. Does R-134's ¼-per-round line bind the card or the encounter?** The line — an encounter's expected damage in per round at most ¼ of the party's pool at the "bruise, not wipe" tier — sat over target in every round an adversary acted in all three yardstick fights (item 155), and no card-level retune reaches it without breaking the per-hit rows: two Bough attacks at the boss row's 8.5 are about 40 % of the pool on their own, and so are three rootlings on fast turns. *Recommended default: **(a) the encounter** — cards are statted to the per-hit and Actions-to-drop rows; the per-round line is session-forge's budget (count, turn speed, a boss's terrain Actions, Call the Runners), and each run-sheet says how its encounter meets it. DOCS-ONLY.* (b) the card: boss and pack damage come down until the reference encounter meets the line; DATA, adversaries REBUILD per nation. (c) a boss fight is its own tier: the line binds minion and rival encounters, a boss encounter caps at ½ the pool per round; DOCS-ONLY. *(Bestiary redo, item 156 — the Thalendor pass.)*

> **ANSWERED (a) — Ben, chat 2026-09-15 ("defaults on all rulings").** → the sentence in STANDARD.md §3; session-forge's run-sheets carry the budget.

**R-139. On the PC model, which derived stats bind — defenses, HP, Focus, Investiture?** The published companions-and-adversaries pack (system 2.1.0, read live 2026-09-15) derives every defense (override null on all 20 blocks) and states HP, Focus and Investiture (its Focus equals 2 + WIL on every block); our blocks stated all four, so the sheet derived the defense pair beside the override and the override won. At the approved attributes the derivation moved six of the seven Corvaine + Riverlands blocks (Raider 13 / 12 / 11 from 12 / 11 / 11; Mistheron's Cog 14, the belief DC, would have fallen to 11), and two Thalendor blocks stated a Cog 8 that cannot derive. *Recommended default: **(a) defenses derive; HP, Focus and Investiture stay stated — the published shape.** The build writes no defense override on a PC-model block, the model gate refuses a stated pair that disagrees, the approved blocks' defenses move to the derivation or a block's attributes are re-chosen to hold a number that matters (the Mistheron's Cog 14 by INT 2 / WIL 2). TOOLING + DATA, adversaries REBUILD.* (b) everything stays stated — the July shape, the sheet's derived pair disagreeing with the card; DOCS-ONLY. (c) everything derives like a PC — HP 10 + STR at level 1 makes Roek 12; not recommended. *(The rerun, item 166.)*

> **ANSWERED (a) — Ben, chat 2026-09-15 ("defaults on all rulings").** → `advActorSystem` writes no defense override on a PC-model block; `validate-adversary-model.js` refuses a stated pair that disagrees; the pack validator expects no override there; the twelve blocks omit `defenses` (Raider 13 / 12 / 11, Line-Caller 12 / 13 / 13, Roek 13 / 13 / 14 — his talk-out DC is 14 now — Mistheron 14 / 14 / 14 with INT 2 / WIL 2, Tollbird 12 / 11 / 13, Surecat 15 / 11 / 14, Well-Warden 12 / 13 / 14, rootling 13 / 10 / 11, grove 13 / 13 / 12, ring 13 / 10 / 13, owl 13 / 11 / 13, the preacher 11 / 13 / 14).

**R-140. How are R-134's per-hit rows read under one modifier?** The rows ("a minion's average hit at most about 4") were written for split numbers; with one modifier a d6 minion at +2 hits for 5.5, and holding the row literally means d4 weapons or +1 to-hit. What the yardstick fights actually measured is damage per attack and Actions to drop. *Recommended default: **(a) read the rows as expected damage per attack, grazes charged** — hit chance × hit EV + miss chance × graze EV against the party's Physical 13 / 14 / 16: a minion at most about 4 per attack, a rival about 6, a boss about 8; every proposed line on the gate page sits on those (the Raider's +2 for 1d6+2 is 4.4 against Phy 14; the approved +4 for 1d6+1 was 4.0). DOCS-ONLY.* (b) hold the hit rows literally — minions on d4s at +2 or d6s at +1; DATA. (c) hold the to-hit numbers and let the hits rise — the published pack's line (Archer +4 for 1d6+4); DATA, not recommended for this party. *(The rerun, item 166.)*

> **ANSWERED (a) — Ben, chat 2026-09-15 ("defaults on all rulings").** → STANDARD.md §3's R-140 paragraph; a block's proposal prints its per-attack number beside its line.

### K.18 — R-141 (bench run 49a, answered 2026-09-15 in chat)

Filed 2026-09-15 by bench run 49a from checklist row FP-1 and answered (a) by Ben in chat the same day, in the message that started fix pass 13. Moved here by fix pass 13, which carried the card text; the entry is verbatim as filed, with its answer line.

**R-141. False Premise's Reaction denial clears at the END of the target's next turn at the table, but its card — your R-115 (a) text — says "until the start of its next turn." Which one moves?** Bench run 49a (checklist row FP-1, 2026-09-15): on a success the target gains the `noreactions` timed status stamped to its own next turn (`statusExpire: target` → `expireAfter {round 2, turn 0}`), and the engine's turn sweep (`edhaExpireTimedStatuses`, `module-src/scripts/engine/03-where-an-effect-lives.js`) deletes a timed status only once the turn sequence has PASSED that stamp — so it was still on at the START of the target's next turn and cleared when that turn ended, the chat card saying *"💢 No Reactions (Extract Thought) on Bench Target — Adjacent A ends (end of its turn)."* Every timed status in the engine works this way (Kneel's Compelled, Brace, Hollow Command's No Actions); **R-28 (a)** (2026-09-06) met the same split on Withering Touch and chose END, fixing the prose. For a Reaction denial the practical difference is small — the target also cannot react during its own turn — but the card you approved in R-115 (a) says the start. *Recommended default: **(a) END, fix the card — R-28 (a)'s precedent** — False Premise's text (`data/leyline.json` + `data/authored/leyline-blue.json`) reads "cannot take Reactions until the end of its next turn"; the engine is unchanged; DATA, leyline REBUILD + ⟳ Sync Talents, and FP-1's expiry clause retires on the rebuilt card.* (b) START, as the card says — one generic dial on the timed-status stamp (`statusExpire` gains a start-of-turn reference, so the sweep removes the status as that turn begins), set on False Premise only while every other timed status keeps END; ENGINE-ONLY (F5) + DATA (leyline REBUILD). (c) START for every card that says "until the start of … next turn" — the same dial swept tree-wide, re-opening R-28; not recommended without first counting which cards say which. *(Bench run 49a; item 107 / R-115 (a); TODO item 171 is the "(Extract Thought)" label on the same card.)*

> **ANSWERED (a) — Ben, chat 2026-09-15, verbatim: "R-141, yeah fix the card text."** → the END stands: False Premise's No Reactions keeps the engine's timed-status convention (`statusExpire: target`, cleared by `edhaExpireTimedStatuses` at the END of the target's next turn — R-28 (a)'s precedent), and the card text moved in fix pass 13: the authored value / chat / short forms in `data/authored/leyline-blue.json` and the source prose in `data/leyline.json` now read *"that character cannot take Reactions until the end of its next turn."* The engine does not change. DATA: leyline REBUILD + ⟳ Sync Talents; checklist row FP-1 re-asks for the END and a card re-read.

### K.19 — R-142 and R-143 (filed by the PM 2026-09-15, answered 2026-09-15 in chat)

Filed 2026-09-15 by the PM — R-142 from the review of item 169 (PR #396), R-143 at Ben's request in chat — and answered by Ben in one chat message the same evening: *"Confirming 143-a, and with all caveats. Go on the compatibility check. 142-a is fine as well."* (The compatibility check is TODO item 177, dispatched on that word.) Moved here by the PM's bookkeeping; both entries are verbatim as filed, each followed by its answer line.

**R-142. Ten talents still roll a decoy system damage roll, with Apply buttons, beside the damage their own rule deals — and blanking the formula (item 141's recipe) also changes two things the table never sees. How should the decoys go?** Filed 2026-09-15 by the PM from the review of item 169 (PR #396). A talent that carries an item-level `system.damage.formula` makes the cosmere system roll it as a real DamageRoll with Apply Damage buttons on every use, pass or fail, while the talent's own `events` rule applies the real damage (bench run 49a, item 169). Blanking the formula removes the decoy, and PR #396 did that for the three talents where it is harmless: Momentum of Victory, Warlord's Advance, Withering Touch. For the other ten it also changes: (1) **attack context** — the system rolls a `skill_test` item that carries damage through its attack path (d20 context `Attack`) and one without through `roll()` (context `Item`), and `edhaTestCtxMatch`, `edhaAggroRecord` and `edhaPackAdvantageApply` (`module-src/scripts/engine/01-shared-core.js` 541 / 781 / 790) key on the formula, so a blanked test stops counting as an attack for attack-scoped test riders, aggro and pack advantage: Volatile Strike, Cascade Collapse, Entropy Strike, Isolating Pressure, Isolating Ruin — and Killing Blow and The Final Study, already blanked by PR #379 (item 174); (2) **colour** — `edhaTalentColor` (`35-targeting-attunement-range-aoe-templates.js:31`) reads the formula first and otherwise falls back to `system.path`, which is wrong for deity talents (measured: Unravel Everything blue → black, Unstoppable Advance red → black, Cascading Failure and The Unmooring red → blue, Necrotic Cascade black only by coincidence), and that colour feeds the Attunement Range ⊙ button and Necrotic Grasp's heal-cut gate (`03-where-an-effect-lives.js:909`). Set Charge copies its formula into the charges it places, so it cannot be blanked at all.
- **(a) Keep the formulas, and add one generic field on the talent's own rule (editable on the Events tab) that tells the engine to suppress the system's own damage roll for that talent.** Colour and attack context stay exactly as today; Killing Blow and The Final Study get their formulas back under the same field; Set Charge can use it too. One engine item (the wrapper around the system's damage roll) plus a data pass. **Recommended.** Deploy: ENGINE (F5) + REBUILD leyline and deity + ⟳ Sync Talents.
- **(b) Stop keying attack context and colour on the damage formula** — attack context from the activation (a skill test against a defense), colour from an explicit per-talent field — then blank the formulas freely. A broader engine change that reaches every attack-scoped rider and every colour consumer in all trees. Deploy: ENGINE + REBUILD + ⟳ Sync Talents.
- **(c) Leave the decoys** (the table ignores their Apply buttons) and restore Killing Blow and The Final Study's formulas so their tests count as attacks again; #396's three stay blanked. Deploy: DATA — REBUILD deity + ⟳ Sync Talents.

> **ANSWERED (a) — Ben, chat 2026-09-15, verbatim: "142-a is fine as well."** → the formulas stay. One generic field on the talent's own rule, editable on the Events tab, tells the engine to suppress the system's own damage roll and its Apply buttons for that talent, so attack context (`edhaTestCtxMatch`, `edhaAggroRecord`, `edhaPackAdvantageApply`) and colour (`edhaTalentColor`) keep reading the formula exactly as today. Item 169's ten held talents keep their formulas and gain the field; Killing Blow and The Final Study get their formulas back under it (item 174); Set Charge may use it for the roll its placement posts. The build is TODO item 178 — ENGINE (F5) + leyline and deity REBUILD + ⟳ Sync Talents — which closes the rest of items 169 and 174; it waits for item 177's break list, because 3.0.0 reworked the item-use path the field hooks into. R-143's auto-apply (item 179) honours the field and never applies a suppressed roll.

**R-143. Should the engine apply attack damage itself, so the GM stops clicking the system's damage card on most hits?** Filed 2026-09-15 by the PM at Ben's request in chat (*"why can't we make it so I never need to click a card? I guess for grazes, and complication/opportunity, etc?"*). **How it works today.** The cosmere system rolls an attack's d20 and its damage together but never decides the outcome. Its damage card applies damage only when someone clicks it — to whichever tokens that user has selected or targeted at click time (`getApplyTargets`), after an optional modifier dialog, with a full / graze toggle and a heal button — although the message does record the tokens that were targeted when the attack was rolled (its `message.targets` flag). Three things sit between the roll and the damage: **hit or miss**, the attack test against the target's defense; **the graze**, "when an attack misses, the attacker may spend 1 focus per target to graze them instead, dealing the damage rolled on the damage dice (no skill modifier added)" (SR p.35) — Combat Training makes it free once per round, and some effects forbid it ("can't graze", Slippery Target's "attacks can't graze you"); and **the plot die and reactions** — an Opportunity or a Complication when the GM raises the stakes (an Opportunity also comes from the d20's Opportunity range, and one of its spends, Critically Hit, changes the damage), and reactions such as Dodge (1 focus) that change the attack roll and so must be called before the attacker rolls. The engine already wraps `applyDamage` (`edhaWrapApplyDamage`, `module-src/scripts/engine/03-where-an-effect-lives.js`), so every on-hit rider, damage bonus, heal cut, Bulwark reaction and GM cue fires on damage the engine applies exactly as it does on damage a click applies, and deflect is still applied by the system inside `applyDamage`.
- **(a) Auto-apply a clear hit; ask only when a choice exists.** One recorded target, a readable defense, and a hit → the engine applies the rolled damage and posts a card naming what landed, with an Undo button. A miss → the ATTACKER's player gets a one-click "spend 1 Focus to graze?" prompt (no Focus cost when a free graze is available, and no prompt when the attack or the target can't be grazed). An Opportunity or Complication on the roll → a card asks what it buys before anything applies (Critically Hit changes the damage); damage then auto-applies on the answer. No target, several targets, or a target whose defense the engine cannot read → today's card, unchanged. Dodge-style reactions are called before the attacker rolls, exactly as at the table now. **Recommended.** Deploy: ENGINE (F5) — one engine item, size M–L, plus bench rows; it makes R-142's leftover card moot for every attack the engine resolves.
- **(b) One-click confirm.** The engine works out hit / graze / miss against the recorded target and posts one button — for example "Apply 7 keen to Trooper (hit)" — so the GM no longer selects tokens or picks full versus graze, but still clicks once per attack. Deploy: ENGINE (F5), size M.
- **(c) Keep today's card.** Nothing changes.

> **ANSWERED (a), with all four caveats — Ben, chat 2026-09-15, verbatim: "Confirming 143-a, and with all caveats."** The caveats came out of Ben's first reply (*"I like a, with caveats"*, with four questions), as the PM restated them in chat and he confirmed: **(1) `cosmere-rpg` 3.1.0 first** — the system upgrade (TODO item 177; 3.0.0 replaced item Activations with Embedded Actions) lands before any of the build, so nothing is built twice; **(2) the plot-die choice comes before any damage applies** — an Opportunity or a Complication on the attack holds the damage, the canon spends and the talents' own spends are real buttons rather than text, and a Complication gets a prompt of its own; **(3) a reaction window when a player character is hit** — an adversary takes a hit's damage at once, a player character's player gets the chance to react before it applies, and every card the engine applies damage from carries Undo; **(4) Dodge stays a call before the roll** — the rules text: used before an enemy targets you with an attack, 1 focus, disadvantage on that attack, never against an area or multi-target attack — with an optional sheet button that arms it. → The builds, all behind item 177: TODO item 179 (the core — hit or miss against the recorded target, adversaries at once, the player-character reaction window, Undo, the attacker's graze prompt, today's card as the fallback; ENGINE, F5), item 180 (the plot-die choice before damage — the hold, the canon and talent spends as buttons, the Complication prompt; ENGINE, F5, plus a REBUILD + ⟳ Sync Talents for the talents it wires) and item 181 (the optional Dodge button; ENGINE, F5).

### K.20 — R-144 (filed by the PM 2026-09-15, answered 2026-09-15 in chat)

Filed 2026-09-15 by the PM from item 177's compatibility check (PR #403), and answered by Ben in chat the same evening. The entry is verbatim as filed, with its answer line.

**R-144. The `cosmere-rpg` 3.1.0 upgrade measures five blockers and about nine worker sessions — does R-143's caveat 1, "3.1.0 first", still bind the builds, or do items 178 – 181 go dual-mode on your 2.1.0 table now?** Filed 2026-09-15 by the PM from item 177's compatibility check (`docs/analysis/cosmere-rpg-3.1.0-compatibility.md`, PR #403). **What the check measured.** 3.0.0 replaced item Activations with Embedded Actions: a talent no longer carries `activation` or `damage` (they sit on an `action` item embedded inside it), `use()` returns null unless the item is an action, `preUseItem` / `useItem` hand the engine that action instead of the talent whose `events` carry Edha's rules, and the native `use` event fires only on actions. That is five blockers — every PC talent is unusable until both the pack builder emits an embedded action per talent and the engine resolves an action back to its talent — plus nine feature breaks, and every gate stays green through all of it, because lint pass 11's field union still contains `activation` and `damage`. The fix is six themed PRs at about nine worker sessions, two agent bench runs on a copy of the world, and about an hour of your own time: making a separate Foundry data path with your license (the agent never handles it), and later the live upgrade. **Why this re-asks caveat 1.** You confirmed R-143 (a) with "upgrade to 3.1.0 first" so the work would not be built twice. The check found a cheaper way to honour that: the engine PRs are written **dual-mode** — a resolver that treats an item as an embedded action only when `item.isAction?.()` is a no-op on 2.1.0, where items cannot embed items — so the builds can be written once, against both shapes, after **one** resolver PR rather than after the whole upgrade. Item 181 (the Dodge button) touches nothing that moved: the sheet render hook and the pre-attack-roll advantage seam are identical at both versions. *Recommended default: **(a)** build dual-mode after one resolver PR — items 178 – 180 are written once against both shapes and reach your 2.1.0 table as they land, while the remaining migration PRs run in parallel and the live upgrade keeps its own window.*
- **(a) Dual-mode: one resolver PR, then build, and deploy to your 2.1.0 table as each lands.** Item 181 starts now; the schema-lint PR and the resolver PR land first (two worker sessions); items 178, 179 and 180 are then built dual-mode and reach your table on an F5, while the remaining migration PRs run in parallel; the flip and the live upgrade happen once the bench on the copy is clean. Each build is written once — caveat 1's purpose — and you stop clicking damage cards in days rather than after the whole migration. **Recommended.** Deploy: ENGINE (F5) per build, as today; the upgrade keeps its own window.
- **(b) The letter of caveat 1: the whole upgrade first.** All six PRs, the copy bench, the fix pass and the live upgrade land before any of items 178 – 181 starts. Two to three nights of PM work, and nothing changes at your table until the flip; the builds are then written once against 3.1.0 alone.
- **(c) Defer the upgrade.** Stay on 2.1.0, build items 178 – 181 against it alone, and revisit 3.1.0 later — no official Mistborn modules until then, the same nine sessions come due whenever the gap is closed, and whatever the builds assumed about the item-use path is reworked at that point.

> **ANSWERED — Ben, chat 2026-09-15, verbatim: "144- we've got session one built in 2.1.0. I say we keep the capability to play that, and do the full 3.1.0 build on the side. Once I confirm session one has occurred, we can swap to 3.1.0"** → **(a)'s shape, with the flip gated on session one.** The migration runs off the live path exactly as (a) describes — the engine PRs dual-mode, the builder behind `EDHA_SYSTEM_TARGET`, the copy on its own data path — and Ben's live world keeps running `cosmere-rpg` 2.1.0, playable, until he says session one has happened. The swap (TODO item 187, and the Route B upgrade) waits on that word, not only on a clean bench. **The PM's operating reading, which Ben's answer does not itself state:** while session one is pending, nothing that changes play deploys to the live table — item 187 is gated by definition, and items 178 – 180's engine work, though dual-mode and F5-only, is held out of the live module until Ben says otherwise, since it changes how damage reaches a token mid-session. Item 181 and item 182 are unaffected either way and were dispatched the same evening. Ben may call for the damage-card builds sooner; the hold is a default, not a ruling of his.
...
- (b) Remap the rows — item 177's plan as written: keep selector injection and map an action row back to its root talent. Smaller now; breaks again on the next sheet redesign.
- (c) Both: sections where the API fits (the Actions and Talents tabs), selectors where it does not (the consume dialog, the header's budget readout).
...
- (b) Only R-145's pair now; the rest as they come up.
- (c) No — dials stay in code so every change is a PR with a delta and a test.
...
- (b) Yes, as the last PR of the 3.x re-platforming.
- (c) Spike one colour on the Route A copy first, to measure the churn before deciding.
...
> **Post-design, 2026-09-17 (item 212, `docs/design/channel-deity.md`, all seven gates approved):** the re-pricing is done by design under **Shape S, the supply** — a base action per god that supplies its charge, free while channelling either colour, with the two entries as its faces. Costed talents 80 % → 7 % (two bolts, two bodies, three rites keep a price), Investiture sum 125 → 13; twenty-six cards gate on "channelling 2 or more", nine capstones on the flood. The build is item 198's deity leg (§6).
...
- (b) Design it now as a `leyline-revision-guide` pass (one colour worked in full text, Ben's gate per section), build after 3.1.0.
- (c) Never — priced talents are Edha's identity.
...
- (b) Pilot one deity capstone behind a `goal` item now.
- (c) Never.
...
### K.22 — R-155 (advantage and disadvantage per Chapter 3, filed by the PM 2026-09-16 16:40 ET, answered 2026-09-16 in chat)

Filed 2026-09-16 by the PM from Ben's question after the Metalworks comparison, and answered by Ben in chat at 16:47 ET with one letter. The entry is verbatim as filed, with its answer line.

**R-155. Advantage and disadvantage the way Chapter 3 states them — countable, placed on a die of the chooser's pick, cancelling one for one — how should the engine implement it?** Filed 2026-09-16 by the PM from Ben's question in chat after the Metalworks comparison (*"should our talents be stacking up adv/disadv in a counter, and then when a roll occurs the popup appears … and you pick which die gets adv, and the gm picks which die gets disadv? Or some other implementation?"*).

**The rule (Mistborn Handbook, Ch. 3 Character Statistics → Skills, "Advantages and Disadvantages"; the Stormlight Starter Rules Part 1 carries the same text).** For each advantage on a test the roller chooses one die they are about to roll — the d20, the plot die, or any other die such as a damage die — rolls two of it and keeps one; each die may be chosen once, so two advantages go on two different dice. For each disadvantage the opponent chooses the die and which result is kept: the GM for a player's test, and on an enemy NPC's test the roles reverse (the GM picks for the NPC's advantages, a player — the one most affected — picks for its disadvantages). Advantages and disadvantages cancel one for one before anything is placed. A test against several targets rolls the extra dice separately and applies them only to the targets they concern. The source of a grant does not name the die unless the talent says so; in the 206 Mistborn talents one does (*Unexpected Burst* adds a disadvantage to the plot die roll), one grants two at once (*Critical Mass*: melee attacks gain 2 advantages), and the rest say "gain an advantage on the test".

**What the system implements (identical at 2.1.0 and 3.1.0).** One tri-state per die: `advantageMode` for the d20 (`kh` / `kl`), `advantageModePlot` for the plot die (`p` = the player picks the kept face, `gmp` = the GM picks, through the `pick-dice-result` dialog), and in the attack dialog one mode per individual damage die. Nothing counts sources and nothing cancels; the hotkey fold gives advantage when both keys are down. The roll and attack dialogs expose every die as a toggle, so a human can enact the rule; no code does.

**What Edha does today.** Twenty engine sites write the d20 channel and none writes the plot die or a damage die; `edhaNextModFoldMode` collapses every live source to one scalar, returning nothing when both kinds are present (right for one of each, wrong for two against one) and discarding a second advantage outright. 27 authored `edha-next-test-mod` rules and the condition riders feed it. `docs/analysis/talent-ecosystem/SYSTEM-PRIMER.md`, the ecosystem README's "rules facts" and `leyline-revision-guide` §"Advantage / Disadvantage" all state that fold as if it were the rule — corrected with dated blocks in this PR; R-98 and R-100's "collisions" are engine artefacts, not rule facts, and item 201's audit re-reads them.

- **(a) Counter, cancel, then place — through the dialog when it opens, by a fixed priority when it does not.** The engine harvests every live source at pre-roll (the `edha-next-test-mod` entries, the condition riders it already knows — Prone, Restrained, Disoriented, Empowered — and the Gain Advantage marker), nets them one for one, and: when the roll or attack dialog opens, shows "N advantages to place / M disadvantages to place" beside the system's own per-die toggles and will not roll until they are placed; the roller places advantages; the opponent places disadvantages — on a player's roll the GM's client through the relay whisper card, on an adversary's roll the most-affected player, in both cases defaulting to the d20 when nobody answers; a fast-forwarded roll places by a fixed priority, d20 first, the plot die if the stakes are raised, then the largest damage die. Numeric dice keep by `kh` / `kl`; the plot die uses `p` / `gmp`. **Two phases:** phase 1 the counter, the cancellation and the fixed-priority placement across all three die kinds (the arithmetic of Chapter 3 on every roll, no new UI); phase 2 the dialog placement and the opponent's choice. Deploy: ENGINE (F5), dual-mode — both dialogs and both plot modifiers exist at 2.1.0 and 3.1.0. **Recommended.**
- (b) **Phase 1 only** — counter, cancel, fixed priority; no choice UI ever. Chapter 3's arithmetic without its "who chooses". Deploy: ENGINE (F5).
- (c) **Net count on the d20 only** — cancel one for one, then a single mode on the d20; a surplus advantage or disadvantage is lost. Today's fold plus cancellation; contradicts "two advantages, two dice". Deploy: ENGINE (F5), smallest.

Ask: the full rule in two phases (a), the arithmetic with a fixed priority (b), or a net count on the d20 only (c)?

> **ANSWERED — Ben, chat 2026-09-16 16:47 ET, verbatim: "a"** → **(a) the full rule in two phases: a counter of live sources, cancellation one for one, placement on the d20 / plot die / damage die by a fixed priority when no dialog opens (phase 1), then the dialog placement with the roller placing advantages and the opponent placing disadvantages through the relay (phase 2).** The build is **item 202**, unblocked; dual-mode, 2.1.0-safe. R-98 and R-100's "collisions" stand as engine artefacts until item 202 lands; item 201's audit re-reads both.

---

### K.23 — R-156 … R-158 (the rules audit's three, filed by item 201 2026-09-16, answered 2026-09-16 in chat)

Filed 2026-09-16 by item 201's rules audit against the Mistborn Handbook (`docs/analysis/rules-audit-2026-09.md`, PR #417), and answered by Ben in chat at 17:57 ET with one word. Each entry is verbatim as filed, with its answer line.

**R-156. Movement rate: does Edha adopt the published Speed ladder, the way it adopted the senses ladder?** Filed 2026-09-16 by the rules audit (TODO item 201; `docs/analysis/rules-audit-2026-09.md`). **The rule (Mistborn Handbook, Ch. 3 → Attributes → Speed, and the Movement Rate table in Appendix 2).** Movement rate is a ladder off Speed, not a formula: **SPD 0 → 20 ft, 1–2 → 25, 3–4 → 30, 5–6 → 40, 7–8 → 60, 9+ → 80**, per Move action. The cosmere-rpg system implements exactly that table. **What Edha does.** `edhaWalkRateFtFromSpd(spd) = 20 + 5·spd` (`module-src/scripts/engine/52-green-instinct.js:591`) is written as a sheet **override** on every character at `:653`, so it wins over the system's ladder; `docs/ACTOR_STAT_DERIVATION.md` §3 names it "the one stat Edha still derives differently", inherited from `source-materials/legacy-uploads/Character_Building_Rules.md`. It is the last of the three legacy derivations still live — health went back to the published formula with R-54 (c), senses with R-56 final ("Cosmere ladder for everyone"), and this one was never asked about. **What differs at the table.** SPD 0 and 1 agree (20 / 25); from SPD 2 up Edha is faster — SPD 2 walks 30 ft instead of 25, SPD 3 walks 35 instead of 30, SPD 4 walks 40 instead of 30, SPD 5 walks 45 instead of 40. This party's PCs sit at SPD 1–3, so it is a 0–5 ft difference per Move today, and it widens with every attribute point. It also touches every distance derived from movement (Slowed halves it, difficult terrain, the walk-rate bonuses on five talents, `Siege Form`'s 0 override) and the adversary blocks that omit `movement` and fall through to SPD. *Recommended default: **(a)** take the published ladder and delete the override — it is the same answer Ben already gave for senses, it removes the last stat where Edha's sheet disagrees with the system's own derivation, and it is a deletion rather than new code. Deploy: ENGINE (F5) — an F5 moves every existing actor's sheet, and a granular effect bonus still adds on top.*
...
Ask: Does Edha take the published Movement Rate ladder for every actor, deleting the `20 + 5·SPD` override (a), keep the Edha formula and write it into the docs as a deliberate house rule (b), or keep the formula for PCs only and let adversaries read the ladder (c)?

> **ANSWERED — Ben, chat 2026-09-16 17:57 ET, verbatim for all three: "defaults"** → **(a) the published Movement Rate ladder for every actor; the `20 + 5·SPD` override is deleted**, as R-56 did for senses. **Item 203** (queue row 207) is unblocked — ENGINE, F5; PCs at SPD 2 – 5 lose 5 ft per Move, which the 🤖 rows record.

**R-157. Attribute points: the two published handbooks disagree about level 3 — which table does Edha use?** Filed 2026-09-16 by the rules audit (TODO item 201). **The rule, and the conflict.** The Mistborn Handbook's Character Advancement table (Ch. 1, reprinted in Appendix 2) grants 12 attribute points at level 1 and then **+1 at levels 6, 9, 12, 15 and 18 — there is no level-3 increase.** The cosmere-rpg system's `advancement.rules` (2.1.0 `index.js:718`) grants **+1 at level 3** as well, which is the Stormlight table; the sheet, the level-up flow and every Edha doc follow it (`.claude/skills/build-forge/SKILL.md:49` "attribute points at levels 3, 6 and 9"; `docs/ACTOR_STAT_DERIVATION.md` §3 "+1 at L3, L6, …"). Everything else in the two tables agrees term for term — health, skill ranks, talents, the tier and rank-cap bands. **Why it matters now.** The campaign plays levels 1–10, and level 3 is inside that window: under the system every PC is one attribute point ahead of a Mistborn PC from level 3 until level 6, which is one point of a defense pair, one point on a skill, and possibly a rung on the movement / senses / recovery ladders. `scripts/validate-build.py` and `docs/levelup-builds.json` are written against the system's table, so this is also a question about what the build gate enforces. *Recommended default: **(a)** keep the system's table (points at 3, 6, 9, 12, 15, 18) and record the Mistborn divergence in the docs — the sheet enforces it, nine level-up guides are written against it, and fighting the system's advancement table to remove a point from PCs is a lot of machinery for a small nerf. Deploy: DOCS-ONLY.*
- **(a) Keep the system's table; document the divergence — RECOMMENDED.** `docs/ACTOR_STAT_DERIVATION.md` and `build-forge` say which table Edha uses and that the Mistborn book differs. No PC changes. Deploy: DOCS-ONLY.
- **(b) Follow the Mistborn table.** Every PC at levels 3–5 loses an attribute point; the wizard, the budget bar and `validate-build.py` need the Edha table rather than the system's, and the nine published level-up guides are re-checked. Deploy: ENGINE (F5) + DOCS, and a re-run of the build gate. Truest to "our Chapter 3 would be exactly the same", but Chapter 1 is where the tables live and the two books genuinely differ.
- **(c) Keep the point, balance the total later.** Levels 1–20 end on the same number of points by adding one at 21+. Deploy: DOCS-ONLY today. Cosmetic for a campaign that will not reach level 21.

Ask: Does Edha keep the system's advancement table with its level-3 attribute point (a), follow the Mistborn table and drop the level-3 point (b), or keep the point and hand out a matching one at level 21+ so the twenty-level totals agree (c)?

> **ANSWERED — Ben, chat 2026-09-16 17:57 ET, verbatim for all three: "defaults"** → **(a) keep the system's advancement table with its level-3 attribute point, and document the divergence** from the Mistborn table. Filed as **item 208** (DOCS-ONLY: `docs/ACTOR_STAT_DERIVATION.md` and `build-forge` say which table Edha uses and that the Mistborn book differs).

**R-158. The published Minion and Boss role features — does Edha adopt them as written?** Filed 2026-09-16 by the rules audit (TODO item 201). **The rule (Mistborn Handbook, Ch. 13 → Using Adversaries, "Role").** A role is not only a band of numbers; each role is printed on the block as a rule. **Minion:** "The Minion's attacks can't critically hit, and they're immediately defeated when they suffer an injury" — so the first hit that would down a minion removes it, and the PC who lands it chooses death or Unconscious. **Rival:** no additional rules. **Boss:** "The Boss can take both a fast turn and a slow turn each round. After an enemy finishes a turn, the Boss can spend 1 focus to immediately use an additional 1 or 0. Additionally, they can spend 1 focus on their turn to remove a condition from themself." **What Edha does.** Nothing: `role` drives the leyline rank map (minion 1 / rival 2 / boss 3, canon ruling 122) and `bestiary-forge`'s numeric bands, and no block in `data/adversaries.json` or line in the engine carries any of the three features. **Why it is a ruling and not just a build.** A published Boss acts roughly twice as often as an Edha boss — five Actions a round against three, plus focus-bought extras — so adopting it does not add text to a card, it re-opens R-134's per-role targets and R-138's per-round line, both of which were measured in bench run 48 against bosses that take one turn (`docs/analysis/bestiary/YARDSTICK-2026-09-14.md`: the Briar-Gone Grove already sat over the ¼-per-round line on one turn a round). The Minion half cuts the other way and is the cheaper win: it would fix the measured "Actions-to-drop is over target for the minion" row by removing a minion the moment it would go down. *Recommended default: **(a)** adopt all three as written, Minion first and Boss behind a re-run of the yardstick — it is the published definition of the roles this bestiary already uses, R-137 already put adversaries on the PC rules, and the numbers were built against a boss that was quietly missing half its turn.*
- **(a) Adopt all three as written — RECOMMENDED.** A `minion` block gets the no-crit / defeated-on-injury feature and a `boss` block the two turns, the focus-for-an-Action and the focus-to-clear-a-condition, each as a stated feature on the block so it is visible in Foundry. The eight boss blocks are re-measured against R-134 before the change ships to a live encounter. Deploy: DATA (REBUILD + ⟳ Sync) for the feature text, ENGINE (F5) for whatever the double turn needs from the combat tracker, and a yardstick re-run. Item 206.
- **(b) The Minion feature and the Boss's condition-clearing; no double turn.** Takes the two halves that need no combat-tracker work and no re-tuning, and leaves the biggest change out. A boss stays a one-turn creature that hits harder, which is what the current HP and damage bands were built for. Deploy: DATA (REBUILD + ⟳ Sync), ENGINE only for the focus-to-clear button.
- **(c) Keep roles as numeric bands; record the divergence.** `bestiary-forge/STANDARD.md` states that Edha's roles are the published ones minus their features, and why. Deploy: DOCS-ONLY. Cheapest, and it leaves the bestiary using the published vocabulary for something the published rules define differently.

Ask: Does Edha adopt the published role features — all three as written, Minion first and Boss after a yardstick re-run (a), Minion and the Boss's condition-clearing only, leaving the double turn out (b), or keep roles as numeric bands and record the divergence (c)?

> **ANSWERED — Ben, chat 2026-09-16 17:57 ET, verbatim for all three: "defaults"** → **(a) adopt all three role features as written — Minion first, Boss after a yardstick re-run.** **Item 206** (queue row 210) is unblocked — DATA + ENGINE at REBUILD + ⟳ Sync; its brief re-measures the eight boss blocks against R-134 with the second turn in.

---

### K.24 — R-159 (the deity re-pricing on the Channel model, filed by the PM 2026-09-16 18:08 ET, answered 2026-09-16 in chat)

Filed 2026-09-16 by the PM from the Channel design pass's close-out (`docs/design/channel-actions.md` §5.3, PR #419), and answered by Ben in chat at 18:40 ET. The entry is verbatim as filed, with its answer line.

**R-159. The deity re-pricing, re-opened against the Channel design — R-151 (a) held it until `docs/design/channel-actions.md` existed; it does now. How do the ten deity trees take the rider / release split and the braid?** Filed 2026-09-16 by the PM from the design pass's close-out (§5.3 item 5, PR #419). **What the design settled for leyline** (§1.1, M13 (a), M16 (a)): a *rider* reads "while channelling [Colour]" and carries no Investiture cost — the Channel paid; a *release* keeps its cost and stands alone (the bolt, the barricade); White worked out at Passive + Special 68 % → 84 % and costed 60 % → 16 % (§3.5). **What it says about deity** (§1.6, M6 (c)): a deity rider reads "while channelling [Colour A] or [Colour B]"; a talent that tests with one colour may ride that colour by name; a tree may add one *braid* entry passive ("while channelling A you count as channelling B for [Deity] talents") so a two-colour capstone needs no second Channel; whether any tree wants one is this ruling's question. **The numbers the re-pricing answers** (`docs/analysis/talent-comparison-mistborn-radiant.md` §C.2, §D-1): deity 80 % Investiture-priced and 0 % Special against 8 – 46 % and 30 – 41 % in every published Invested family, because a deity tree had no base action to have paid on — it now has the colour's Channel.
- **(a) A gated deity design pass on the Channel model, in Ben's own session** — a brief like item 200's: every deity talent classed rider or release with its new cost and card sentence, one braid considered per tree, R-108's Special target revisited in the same pass, the two-colour test rule kept; the build rides item 198's deity leg after the White pilot. Deploy: DOCS-ONLY until the build. **Recommended.**
- (b) **Re-price mechanically now from the audit's rule** — drop the Investiture cost from every deity talent that is not a damage or effect source, keep it on the sources, no braids, no design pass. Deploy: DATA — REBUILD deity + ⟳ Sync Talents; every deity bench row re-run. Faster; it decides the deity identity ("priced talents", deity guide) by arithmetic.
- (c) **Leave the deity atlas priced as it is** until playtest-1 shows the cost at the table.

Ask: a gated deity design pass on the Channel model (a), a mechanical re-pricing now (b), or leave it until playtest-1 (c)?

> **ANSWERED — Ben, chat 2026-09-16 18:40 ET, verbatim: "Default for 159"** → **(a) a gated deity design pass on the Channel model, in Ben's own session** — every deity talent classed rider or release with its new cost and card sentence, one braid considered per tree, R-108's Special target revisited in the same pass, the two-colour test rule kept; the build rides item 198's deity leg after the White pilot. Filed as **item 212**; the PM writes its brief in the shape of `docs/briefs/channel-design-pass.md` when Ben asks for it.

---

### K.25 — R-161 (the deity refund passives as upkeep, answered 2026-09-19 in chat, filed the same day)

Filed 2026-09-19 from the review of the deity atlas against the published Invested families (`docs/analysis/talent-comparison-mistborn-radiant.md`), run in chat on 2026-09-19 after PRs #433 and #434 landed. R-161 was answered before it was filed — Ben's line in chat the same day — and is written here in the §K.17 shape, with a §L stub.

**R-161. The five deity refund passives pay Investiture whether or not the disciple is channelling — are they upkeep, or income between Channels?** Filed 2026-09-19 from the review of the deity pass (`docs/design/channel-deity.md`) against the published Invested families. **What the pass left standing.** Void Sense (Chaos), Accumulate (Knowledge), Prognosis (Life), Reaper's Harvest (Death) and Expose (Sovereignty) each refund 1 Investiture on a trigger, and the design reads each one as "the refund funds the maintain" (§5.2). **Why that is not what they do.** Every published Invested pool refills once — the Allomantic and Radiant pools at scene start, a Feruchemical metalmind from downtime storing (`talent-comparison-mistborn-radiant.md` §C.2). An Edha disciple has in-combat Draw Mana AND a trigger refund AND, under the Channel, riders that cost nothing at all. The refund accrues whether or not a Channel is up, so it is not upkeep: it is income, and a disciple banks pool between Channels. **Where the leak actually is.** Void Sense, Accumulate and Prognosis already read "once per round", so they pay at most 1 and the channelling gate is the only thing that bites. Reaper's Harvest and Expose carry no per-round limit — several deaths in range, or several failed tests by a Censured creature, each pay — and that half scales with the fight. **What the fix costs to build.** One field on the existing refund rule, read against the channelled-Investiture number the Channel status already stores: no per-talent code, and no new vocabulary, since "the Investiture you are channelling" is defined in `docs/design/channel-actions.md` §1.1.
- **(a) Upkeep — the refund pays only while you are channelling, and never more in a round than the Investiture you are channelling.** One card tail on all five: *"While you are channelling, [trigger], you recover 1 Investiture. You cannot recover more than the Investiture you are channelling this way in a round."* A one-point maintain is still fully refunded by a single trigger, which was always the stated job. Deploy: DOCS-ONLY now (`docs/design/channel-deity.md`); DATA — REBUILD deity + ⟳ Sync Talents when item 198's deity leg builds it. **Recommended.**
- (b) **Gate on channelling only, no cap.** A shorter sentence, and it closes the between-Channels banking. It leaves Reaper's Harvest and Expose paying several times in a bloody round, which is the half of the leak that scales with the fight.
- (c) **Unchanged.** The refunds stay as the pass approved them, and item 210's yardstick prices the result first.

Ask: Do the five deity refund passives become upkeep, gated on channelling and capped to the Investiture you are channelling (a), gated on channelling with no cap (b), or stay as approved until the yardstick prices them (c)?

> **ANSWERED (a) — Ben, chat 2026-09-19, verbatim: "And I agree with making the refunds sustains."** → the five passives are rewritten as upkeep, gated on channelling and capped to the channelled Investiture. `docs/design/channel-deity.md` carries the amendment at all five cards and the rule in §5.2; item 198's deity leg builds the card text and the one engine field. **The cap is inert today on Void Sense, Accumulate and Prognosis** — all three already say "once per round" — and is carried on all five anyway, as one principle and one field, so a later card cannot reopen it. **Reaper's Harvest keeps marking its Remain** whether or not Morrath is channelling: only the Investiture is gated, so Death's corpse economy is untouched and the tree's identity survives on the body rather than on the pool.

---

### K.26 — R-160, R-162, R-163 (the deity-atlas review's three open rulings, filed 2026-09-19, answered the same day in chat)

Filed 2026-09-19 from the review of the deity atlas against the published Invested families (`docs/analysis/talent-comparison-mistborn-radiant.md`, after PRs #433 and #434), and answered by Ben in chat that evening in one line — *"160 d defaults rest"* — which takes R-160's widening (d) and the recommended default on the other two. Each entry is verbatim as filed, with its answer line. R-161, the fourth of that review's rulings, was answered earlier the same day and is in §K.25.

**R-160. Four supply faces, not two, carry a payload bounded neither by a charge cap nor by a weapon Strike — what does such a face cost?** Filed 2026-09-19 from the review of the deity pass (`docs/design/channel-deity.md`) against the published Invested families (`docs/analysis/talent-comparison-mistborn-radiant.md`). **What the pass decided.** Two supply faces keep an Investiture cost — Pyre and Forge Construct, at 1 each — and each was argued on its own: a bolt that exists only because of the Investiture, and a body raised from it (§3.2, §3.3). Every other face is free while the Channel is up. **What the review found.** Those two one-off arguments generalise into one line that sorts all twenty faces: a face costs 1 Investiture when its payload is repeatable and bounded neither by a charge cap nor by replacing a weapon Strike. Nine faces are capped at tier or tier + 1; four replace a weapon Strike and are bounded by the Action and the attack roll; the line catches four, not two. The two the pass left free are **Tend's Green face** (Anaveth: [Tier][Die] + Awareness healing with no cap of any kind — the pass named it the marks' one real balance case itself, and handed it to item 210 as yardstick row 1) and **Place Omen's Blue face** (Maelith: the Omen cap bounds the charge, not the payload — "placements beyond the cap are lost" leaves the face dealing [Tier][Die] spirit on every Action once the cap is full). **Why not simply make every supply 2 Actions,** which was the first question asked: every published base action is 1 Action (Burn, Store, Tap, the surge actions), published Invested families run 0 – 10 % two-Action talents, and the deity atlas already sits at 29 %, so a 2-Action supply moves the atlas away from the Mistborn and Radiant shape this whole model is chasing — while four costed faces out of twenty is 20 %, inside the published 8 – 46 % cost band. **A fifth face may belong here, and is option (d):** Tend's Blue face is scene-long, party-wide, repeatable and carries no stated cap on how many characters may bear it, so it meets the same line, and pricing it at 1 would restore the cost today's Vital Diagnosis already carries. Ben approved two faces in chat, not three, so the widening is asked rather than taken. **Nothing is live at the table:** this is a design-doc change, and item 198's deity leg is where any of it would reach a card. *Recommended default: **(a)** 1 Investiture on Tend's Green face and Place Omen's Blue face, joining Pyre and Forge Construct — it keeps the supply a 1-Action base action the way every published Invested family does, and for Life it is the alternative this design already tabled as G-3 (b) and the cost Life Surge carries today.*
- **(a) 1 Investiture on the two faces.** Four costed faces. `docs/design/channel-deity.md` §2.2 and §2.4 carry this as the recorded amendment, so a different answer edits the design doc rather than the game. Deploy: DOCS-ONLY now; DATA — REBUILD deity + ⟳ Sync Talents at item 198.
- (b) **2 Actions on the two faces, no Investiture.** The supply stays clear of the pool and pays in the Action economy instead. It caps the repeat directly, but it pushes the deity atlas further from the published 1-Action base action and leaves a Draw Mana turn unable to supply.
- (c) **Leave both free.** Item 210's yardstick prices them against the party's damage intake first, and the pass's own recommendation stands until it does.
- (d) **(a), and price Tend's Blue face at 1 as well.** Five costed faces; the scene-long party-wide vital rider stops being free and repeatable.

Ask: Does a repeatable supply face bounded by neither a charge cap nor a weapon Strike cost 1 Investiture (a), take 2 Actions (b), stay free until the yardstick prices it (c), or take (a) with Tend's Blue face priced too (d)?

> **ANSWERED (a) AND (d) — Ben, chat 2026-09-19, verbatim: "160 d defaults rest"** → **(d): (a) plus Tend's Blue face priced at 1**, so **five** supply faces carry a 1-Investiture consume row — Place Omen's Blue, both of Tend's, Pyre and Forge Construct. Tend is the only supply in the atlas costed on both faces, which is what it means for one tree to hold two unbounded payloads: a scene-long party-wide vital rider and uncapped healing, neither bounded by a charge cap or an attack roll. `docs/design/channel-deity.md` carries it at §2.2, §2.4, the counts in §5.2 (sum 15 → 16, Life 1 → 2, ten priced cards), the build spec in §6.1 and §6.4 (five faces keep a consume row, fourteen do not), and the DS-L1 bench row. Item 198's deity leg builds it.

**R-162. A scene-long deity install bought behind the "2 or more" gate keeps working after the Channel that paid for it drops — should it?** Filed 2026-09-19 from the review of the deity pass, the third of that review's three findings and the only one not acted on. **What the pass decided.** Four arms end with the Channel — Crown of Thorns, Warlord's Fury, Concord and the Mantle of the Aspirant (§4.2, §4.5): the crown stands while the current runs. Every other scene-long install — Siege Form, Trade Routes, Arsenal, Bastion, Investiture of Authority, Fate's two links, Tend's Blue diagnosis — keeps its scene duration behind a one-time "channelling 2 or more" gate. **The shape that raises the question.** A disciple floods for one round, buys the install, and keeps it for the rest of the scene whether or not a Channel is ever up again: the same "benefit outliving its upkeep" shape R-161 just closed on the refunds. **Why this is a question and not a finding.** The pass's split is principled — things you wear end with the Channel, things you build stay built, and a Foundation or a Construct genuinely reads as built. The published analogue cuts the other way, since Feruchemy's Tap runs for ranks rounds rather than a scene (`talent-comparison-mistborn-radiant.md` §B), but Tap is a buff on the Feruchemist, which is an arm and not a building. Acting on this reopens gates 2, 3 and 4 of a pass Ben approved, which is why it is filed rather than taken. *Recommended default: **(b)** unchanged — the wear/build split stands, and item 210's yardstick should price a flooded install against a maintained one before anything reopens.*
- (a) **Scene-long installs end when the Channel ends,** as the four arms already do. One duration rule across the whole deity atlas, and no install outlives its upkeep. It makes a dropped Channel much more expensive, which is a real change to how a disciple plays.
- **(b) Unchanged.** The pass's wear/build split stands: arms end with the Channel, buildings persist. Deploy: nothing.
- (c) **Installs last rounds equal to your rank in the colour,** Feruchemy's Tap shape. Closest to the published model and the least like either of the other two; it adds a duration the engine does not track today.

Ask: Should a scene-long deity install end with its Channel (a), keep its scene duration as approved (b), or last rounds equal to your colour rank as Feruchemy's Tap does (c)?

> **ANSWERED (b) — Ben, chat 2026-09-19, verbatim: "160 d defaults rest"** → **(b) unchanged: the pass's wear/build split stands.** Arms end with the Channel — Crown of Thorns, Warlord's Fury, Concord, the Mantle — and buildings persist for the scene behind their one-time "2 or more" gate. No text changes. Item 210's yardstick should price a flooded scene-long install against a maintained one (§5.4 row 5); that measurement is what would re-open this.

**R-163. The refund surface is eight cards, not five — do the two leyline Black refunds take R-161's upkeep rule, when capping them contradicts an approved pass?** Filed 2026-09-19 by the wider audit that R-161 prompted. **What the audit found.** Exactly eight talents in the live data put Investiture back into the owner's pool on a trigger. R-161 (a) has just made upkeep of the five deity ones. The other three are leyline: **`Predatory Patience`** and **`Predator's Due`** (both `data/authored/leyline-black.json`) and **`Flashpoint`** (`leyline-red.json`). **Predatory Patience is the loudest card in the game on this axis** — *"On a successful attack against a Weakened creature, regain 1 Investiture"*, with no channelling gate and, by its rule's own note, no per-round limit. Its trigger is free: the Black Key's Draw Mana rider Weakens every isolated enemy in range, and M10 deliberately keeps the Key ungated (`channel-actions.md`), so a Black mage with **no Channel open at all** can Draw (1 Action, +2, and Weaken the field) then Strike twice into a Weakened target for +2 more — **+4 pool in a round for zero spend**, which is further than anything the deity five could do. **Predator's Due** (*"When you reduce a character to 0 health, regain [Tier][Die] health and 1 Investiture"*, Black 3+) is Reaper's Harvest's twin on the same trigger; after R-161 the deity half is gated and capped and the leyline half is neither, and a Morrath disciple already holds Black 2+, so from level 6 one character can own both and collect 2 a kill with one half uncapped. **Why this is a ruling and not a sweep.** `docs/design/channel-black.md` states the uncapped loop as Isolation's intended economy (§1.5: *"Isolation carries no Investiture at all — the hunt is free once the prey is named, and Predatory Patience pays the mage back on every hit against it"*), and its bench row **CK-14** expects *"+3 Investiture back; the Channel ran positive"* — which the cap makes false. Extending R-161 here amends a pass Ben approved on 2026-09-17. **`Flashpoint` is exempt under every option below:** it is capped once per round, it is one of two choices, and both its prerequisite feeders are costed releases (Flame Surge 2, Arc Flash 1), so earning 1 always costs at least 1 first — a discount on a release, never net income. The heroic atlas grants no Investiture at all. *Recommended default: **(a)** the two Black cards take the same upkeep rule as the deity five, and `channel-black.md` §1.5 and row CK-14 are amended to match — one rule for the whole atlas is worth more than one tree's stated economy, and the Key-fed loop is the single largest source of free pool in the game.*
- **(a) Both Black cards take R-161's rule** — gated on channelling, capped to the Investiture you are channelling. `channel-black.md` §1.5 is amended and CK-14's expected result becomes "+1, and the Channel ran level". One rule across all eight cards. Deploy: DOCS-ONLY now; DATA — REBUILD leyline + ⟳ Sync Talents at item 198's Black leg.
- (b) **Gate both on channelling, do not cap them.** Closes the no-Channel loop, which is the part that scales, and leaves Isolation's "free hunt" economy intact while a Channel is up. CK-14 survives unchanged. Predator's Due still pays uncapped in a round with several kills.
- (c) **Predatory Patience only.** It is the one fed by an ungated Key rider; Predator's Due needs a body and usually pays as the fight ends. Leaves the twin asymmetry with Reaper's Harvest standing.
- (d) **Leave all three alone.** The deity five are upkeep and the leyline three are not, and `channel-black.md`'s approved economy stands. Item 210's yardstick prices the asymmetry before anything moves.

Ask: Do the two leyline Black refunds take R-161's gate and cap (a), take the channelling gate only (b), does Predatory Patience alone change (c), or do all three stand as approved (d)?

> **ANSWERED (a) — Ben, chat 2026-09-19, verbatim: "160 d defaults rest"** → **(a) both leyline Black cards take R-161's rule** — gated on channelling, capped to the Investiture you are channelling. `docs/design/channel-black.md` carries the amendment at both cards, at §1.5's "Isolation carries no Investiture at all" economy, at the Isolation turn walk and the §1.5 round walk, and at bench row **CK-14**, whose expected result becomes **+1, the Channel ran level** (and nothing at all with no Channel). A new row **CK-17** covers Predator's Due's cap and proves its health half is never gated. `Flashpoint` (Red) is exempt and recorded as such. Predatory Patience was the largest free-pool source in the game: the Black Key's Draw rider Weakens the field ungated (M10), so a mage with no Channel open could bank +4 a round.

---

### K.27 — R-145 (the Dodge arm dials, filed 2026-09-15, applied as its default, accepted 2026-09-19 in chat)

Filed 2026-09-15 by the PM from item 181's build (PR #407), R-143 (a)'s caveat 4, and **APPLIED as its default from the day it was filed**, so it wanted a veto rather than a decision. It stood open for four days and was accepted unchanged by Ben in chat on 2026-09-19, in the same exchange that answered R-160, R-162 and R-163 (§K.26). The entry is verbatim as filed, with its answer line.
...
> **ANSWERED (a) — Ben, chat 2026-09-19, verbatim: "Continue with the default for R145"** → **the applied default is ACCEPTED, no veto.** The Focus is spent when you arm Dodge, and an unused arm clears at the end of your own next turn. **No change to the shipped code**: the two Foundry module settings item 193 registered — `dodgePayOnArm` and `dodgeArmExpiresOnOwnTurn` — stand at their registered defaults, and the behaviour has been on `main` since PR #407. The Reaction half of the cost stays GM-adjudicated, as the entry says. Filed here 2026-09-19; **§L is empty for the first time since it was created.**

---

## L. Waiting — **EMPTY as of 2026-09-19. Nothing is waiting on Ben.** R-145 — the Dodge arm dials, filed 2026-09-15 and APPLIED as its default — was accepted unchanged in chat that evening ("Continue with the default for R145") and is filed in §K.27; it is the last entry to leave this section, and the first time §L has stood empty since it was created on 2026-07-27w. Earlier the same day the review of the deity atlas against the published Invested families (PRs #433 / #434) filed four — R-161 (the deity refunds become upkeep, §K.25) and R-160, R-162 and R-163 (§K.26) — and Ben answered all four in chat the same day: R-160 **(d)** five costed supply faces, R-162 **(b)** the wear/build split unchanged, R-163 **(a)** the two leyline Black refunds take the upkeep rule. Before 2026-09-19 this section held R-145 alone, and was otherwise EMPTY since 2026-09-16 18:40 ET (R-159, filed 18:08 ET from the Channel design pass — the deity re-pricing re-opened against the rider / release split — answered (a) in chat and filed in §K.24, its design pass item 212; before that, since 17:57 ET — R-156, R-157 and R-158, filed 2026-09-16 by the rules audit (item 201), were answered by Ben in one word ("defaults": the published movement ladder, the system's attribute table with the divergence documented, the Minion and Boss role features adopted) and filed in §K.23. **R-145** is also open (filed by the PM on 2026-09-15 from item 181's build, and APPLIED as its default, so it wants a veto rather than a decision); before those, EMPTY since 2026-09-16 16:47 ET (R-155, filed 16:40 ET from Ben's question on advantage and disadvantage — Chapter 3 says per die and countable, the engine folds to one scalar — answered (a) in chat and filed in §K.22, its build item 202; before that R-146 … R-154, the Metalworks comparisons' menu filed that day, answered by Ben in one chat line — defaults on all but R-150 (b) and R-152 (b) — and filed in §K.21; before that, R-145 was open (filed by the PM on 2026-09-15 from item 181's build, and APPLIED as its default, so it wants a veto rather than a decision); otherwise EMPTY since earlier that evening (R-144, filed that evening from item 177's compatibility check and answered in chat the same evening — the full 3.1.0 build proceeds on the side while the live table keeps session one, and the swap waits for Ben's word — filed in §K.20; R-142 and R-143, both filed by the PM that day, answered (a) in chat the same evening — R-143 with four caveats — and filed in §K.19; R-141, bench run 49a, answered (a) in chat the same day and filed in §K.18; R-137 … R-140, the attack-model rerun, answered 2026-09-14 / 2026-09-15 in chat and filed in §K.17; R-136 answered (a) in chat and moved to §K.16; R-127 answered and moved to §K.14; the seven design proposals moved to §K.13; the bestiary menu R-128 … R-135 lives in §G.1 → §K.15)

*(R-145 — the Dodge arm dials: the Focus is paid on arming and an unused arm clears at the end of your own next turn; the applied default ACCEPTED, no veto — ANSWERED 2026-09-19, moved to §K.)*

*(R-160 — a supply face whose payload is bounded by neither a charge cap nor a weapon Strike costs 1 Investiture; (d) taken, so five faces are costed — ANSWERED 2026-09-19, moved to §K.)*

*(R-162 — a scene-long deity install keeps its scene duration; arms end with the Channel, buildings persist — ANSWERED 2026-09-19, moved to §K.)*

*(R-163 — the two leyline Black refunds take R-161's upkeep rule; Flashpoint is exempt — ANSWERED 2026-09-19, moved to §K.)*

*(R-161 — the five deity refund passives become upkeep, gated on channelling and capped to the channelled Investiture — ANSWERED 2026-09-19, moved to §K.)*

*(R-159 — the deity re-pricing on the Channel model: a gated deity design pass in Ben's own session — ANSWERED 2026-09-16, moved to §K.)*

*(R-156 — the movement rate is the published ladder off Speed; the override goes — ANSWERED 2026-09-16, moved to §K.)*

*(R-157 — the level-3 attribute point stays on the system's table, the divergence documented — ANSWERED 2026-09-16, moved to §K.)*

*(R-158 — the published Minion and Boss role features are adopted as written — ANSWERED 2026-09-16, moved to §K.)*

*(R-155 — advantage and disadvantage per Chapter 3: a counter that cancels and places, in two phases — ANSWERED 2026-09-16, moved to §K.)*
...
*(R-142 — keep the formulas; a field on the talent's own rule suppresses the system's own damage roll — ANSWERED 2026-09-15, moved to §K.)*

*(R-143 — the engine applies attack damage itself, with four caveats: `cosmere-rpg` 3.1.0 first, the plot-die choice before any damage, a reaction window for player characters and Undo, Dodge still called before the roll — ANSWERED 2026-09-15, moved to §K.)*

*(R-141 — False Premise's Reaction denial ends at the END of the target's next turn; the card text moves — ANSWERED 2026-09-15, moved to §K.)*

*(R-137 — adversaries follow the same rules as the PCs: attributes, skill ranks and talents, the PC attack model — ANSWERED 2026-09-14, filed in §K.17.)*
...
*(R-114 — Mantle of the Aspirant's redirect-unwind heal is gated at the emitter — ANSWERED 2026-09-13, moved to §K.)*

*(R-115 — False Premise's payload re-aimed to a Reaction-denial — ANSWERED 2026-09-13, moved to §K.)*

*(R-116 — Final Decree's Witness clause becomes a free Reactive Strike — ANSWERED 2026-09-13, moved to §K.)*

*(R-117 — Trade Routes reworked to link a Foundation to a point — ANSWERED 2026-09-13, moved to §K.)*

*(R-118 — Fate's White, Destruction's Blue and Life's Blue each get a sizing fix — ANSWERED 2026-09-13, moved to §K.)*

*(R-119 — Sovereignty's Decree becomes a moving radius on H9 — ANSWERED 2026-09-13, moved to §K.)*

*(R-92 — a PC hitting 0 HP gets a generic GM drop cue — ANSWERED 2026-09-13, moved to §K.)*
