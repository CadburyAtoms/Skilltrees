...
exists. Run it after adding, removing, or renaming a script. Since item 190 it is also gate
`scripts-readme` in `node scripts/gates.js`, so a new undocumented script fails the gate run
...
| `handler-schemas.js`       | The engine's registered handler-config field schemas (names + closed `choices` sets), EVALUATED through `tests/harness.js` `loadHandlerRegistry()` from the engine's `EDHA_HANDLER_TYPES` table (item 24; it regex-parsed the engine source before) — so lint pass 9/9b and the item-64 build guard can catch a field Foundry's DataModel would otherwise silently drop. `matchBrace`/`topLevelKeys` stay for `dump-native-vocabulary.js`, which still parses the SYSTEM bundle |
...
| `engine-split.js`          | Item 4 (2026-09-06): re-cuts `module-src/scripts/register-skills.js` at every column-0 `/* ===` banner into `module-src/scripts/engine/NN-<slug>.js` (55 files; lexical order = assembly order; each file an exact byte range of the engine). Re-runnable — after an engine change lands on `main`, re-run it on the new engine rather than hand-merging the sources. `--dry-run` prints the cut plan |
| `engine-assemble.js`       | Item 4: concatenates `module-src/scripts/engine/*.js` (sorted) byte-for-byte into `register-skills.js`, which STAYS the tracked and deployed artifact (PM-R15). **The edit rule: edit a source, run this, commit both.** `--check` = gate `engine-assembly` (right after `engine-check` in `gates.js`; also in the pre-commit body): exits 1 and names the first differing line + owning source file when the tracked engine is not the assembly |
...
| `validate-adversary-model.js` | Gate `adversary-model` (R-137): fails an adversary block that states `attributes` but still stores flat attack/damage numbers that would double-count with the cosmere system's own PC-style roll (attack = d20 + attribute + rank [+ explicit `attackBonus`], damage = dice + attribute + rank, graze = dice only); prints every migrated block's derived line and counts the blocks still on the flat model. `checkBlock` is pinned by `tests/adversary-model.test.js` on fixtures |
| `validate-build.py`        | Checks a character build ladder, or every adversary's talent list (`--adversaries`, informational only per R-94), against the live data's full requirement graph (prose prereqs AND `connections` — iron rule 7) plus the advancement table (1 talent + 2 skill ranks/level, attribute points at 3/6/9, rank caps). No-arg run self-tests `docs/levelup-builds.json`. Run by `build-forge` before any build reaches a player or the pack; not wired into `gates.js` (needs a spec/table argument, so there is nothing to run unconditionally) |
...
| `bestiary-census.js`       | Reads `data/adversaries.json` + the authored overlays and prints the bestiary's measured ground truth (item 153, R-101 (a)): role HP/defense/attack/damage bands, the colour ledger, every block's wiring shapes (cue vs effect vs native roll vs `noHook`), senses/movement stated-vs-derived, status ids, damage types. `--write` regenerates `docs/analysis/bestiary/CENSUS.md`; `--check` fails if it's stale (also pinned by `tests/bestiary-census.test.js`, part of the `unit-tests` gate). Run by `bestiary-forge` |
| `inspect-pack.js`          | Print a talent's rules/effects exactly as Foundry loads them        |
| `check-scripts-readme.js`  | Diffs this table against `git ls-files scripts` and exits 1 on drift (item 21's verifier; gate `scripts-readme` in `gates.js` since item 190) |
...
| `handoff-split.js`         | Moves every `## 2026-MM-DD …` dated delta above `## Reference` in `EDHA_FOUNDRY_HANDOFF.md` into `docs/handoff-changelog/<YYYY-MM>.md` VERBATIM (newest first; prepends to an existing month file) and regenerates the month headers + the folder's `README.md` index; leaves the reference in place. Prints the proof (SHA-256 of the removed text vs the month bodies, heading counts). Idempotent — re-run after a merge that lands a delta in the handoff on the old rule. `--dry-run` prints only; `--hash` hashes the month files as they stand (item 19b, PM-R1). Its `monthHeader()`/`readmeText()` templates source the item-100 blank line and rule sentences from `lib/changelog-rules.js` (item 102), so a re-run cannot regress either; `run()` is exported with injectable paths for `tests/handoff-split.test.js`'s fixture. |
| `lint-changelog.js`        | Gate `lint-changelog` (item 100): per `docs/handoff-changelog/2026-MM.md`, the marker line appears exactly once with exactly one blank line before a valid dated-delta heading, every `## ` heading below it is shaped like one, and the heading count matches both the month file's own header and `README.md`'s index row — catches a heading swallowed by a bad insert and a forgotten index bump alike. `checkMonthFile(dir, file)` and `lint(dir)` are exported (no process.exit, return the error list) so `tests/handoff-split.test.js` can run the same check against a fixture directory (item 102); `main()` keeps the original CLI messages/exit codes. |
...
| `build-levelup-guides.py`  | Generates `EDHA_LEVELUP_GUIDES.html` (9 pages, 3 builds per PC, added 2026-09-09) from `docs/levelup-builds.json` + its own `levelup-guides-prose.json`, pulling every talent's action/cost/tree verbatim from `data/leyline.json`/`domain.json`/`cosmere.json` so a rename or cost change flows through on the next run. `docs/levelup-builds.json` must pass `validate-build.py` first. Run ad hoc by `build-forge`/`handout-forge`, not a gate; the rendered PDF stays untracked per the `*.pdf` policy |
| `levelup-guides-prose.json`| The authored framing prose (`THE PLAN` / `IN A FIGHT` / `IN A ROOM` / `THE CATCH` / `YOUR NEXT LEVEL-UP` / `SPEND THESE`, plus each PC's stat-strip text) `build-levelup-guides.py` reads — mechanics (talent action/cost/tree, the ladders) stay in the atlas data and `docs/levelup-builds.json`; only prose lives here |
...
| `module-src-sync.js`       | Mirror the hand-edited live module runtime ↔ `module-src/` in this repo (push/pull). Its `FILES` list names the ASSEMBLED `scripts/register-skills.js` only — the per-section sources under `module-src/scripts/engine/` never reach Ben's module (item 4) |
...
| `lib/blank-strings.js`     | `blankStringsAndComments` — the OFFSET-PRESERVING comment/string blanker (template/regex-aware), moved out of `lint-refs.js` (item 182) so `lib/dead-field-check.js` and a test can share it without executing that whole script. Contrast `strip-comments.js`: that one cannot be indexed back into the source and does not understand string content |
| `lib/dead-field-check.js`  | `checkDeadFields` — the DEAD `system.<field>` detector behind `lint-refs.js` pass 11 (item 182), factored out so a test can reach it directly. Adds PER-TYPE sharpening on top of the pre-existing union check: an exact-match identifier name or a document-creation literal's sibling `type:` key narrows the check to that item type's own field set (`data/native-vocabulary.json`'s `systemSchemaFieldsByType`, absent from the tracked 2.1.0 file — every real call site still falls back to the union until item 187). Pinned in `tests/dead-field-by-type.test.js` against a hand-built fixture 3.1.0 snapshot |
...
| `lib/fold-die-math.js`     | `foldDieMath` — the build-time twin of the engine's `edhaFoldDieMath` (item 59 / R-71): folds a fully numeric computed-dice `damage.formula` into plain dice at pack-build time, leaves any formula still carrying `@`-refs untouched. Used by `foundry-build.js`; pinned against the engine's copy in `tests/fold-die-math.test.js` |
| `lib/changelog-rules.js`   | The changelog "shape" shared by `handoff-split.js` (generator) and `lint-changelog.js` (gate): the marker literal plus the item-100 rule-sentence lines for the month-file bullet list and the README paragraph, in exactly one place so a re-run of the split script cannot regress item 100's fix (item 102). Pinned by `tests/handoff-split.test.js`'s fixture |
