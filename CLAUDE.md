...
| Doc | What it is |
|---|---|
| `EDHA_FOUNDRY_HANDOFF.md` | THE knowledge base, in two parts. **The cold-start REFERENCE** (rewritten 2026-09-06, item 19a — ~670 lines, true today: a table of contents, the ⚑/🤖 marker vocabulary, §1 what this is → §10 gotchas, §H history of the reversals) sits at the BOTTOM under `## Reference — table of contents`; read it in one sitting. **The dated deltas** (newest first, one per session) sit above it and are the record of what changed — read only those newer than the reference's date. §9 = the canonical engine backlog (machine-read into the dashboard's Engine tab); §10 = gotchas that each bit us at least once. **Item 19b (pending)** moves the deltas to `docs/handoff-changelog/2026-MM.md` and lifts the reference to the top; until then a new delta still goes at the TOP of this file. |
