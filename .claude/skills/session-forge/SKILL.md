...
- **Combat**: statblocks on the `data/adversaries.json` schema (role/tier/defenses/deflect/HP +
  ...
  `lint-refs.js` pass 5 fails the commit otherwise. **Statting a block is `bestiary-forge`'s loop**
  (2026-09-14): its `STANDARD.md` fixes the fields, the role bands and the dice, and its census
  (`docs/analysis/bestiary/CENSUS.md`) is the yardstick every number is read against. The wiring
  standard, the vocabulary, and the traps
