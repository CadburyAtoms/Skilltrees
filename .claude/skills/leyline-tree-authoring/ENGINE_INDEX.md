...
| `RESOURCE-CONSUME DIALOG` | (1) TITLE — `edhaSetConsumeTitle` + `renderItemConsumeDialog` / `renderDialogV2`. Cosmetic. (2) PRE-TICK (item 50, R-70 (b), 2026-09-06) — **`edhaPreTickConsumeOptions`** (PURE, pinned: `{...options, shouldConsume: options.shouldConsume ?? true}`) + **`edhaInstallConsumeDialogWrapper`** (ready hook; libWrapper else prototype patch) — **THE ONE sanctioned wrapper of a system dialog**, an iron-rule-2a exception by Ben's ruling, declared in the engine's file header. The system's own `showConsumeDialog` ticks only row 0 (`?? i === 0`), so a two-cost card under-charged on a default click. `tests/consume-dialog-wrapper.test.js` pins that exactly one wrapper exists — do NOT add a second system-dialog wrapper anywhere; ask for a ruling. (3) THE REFUSAL, IN WORDS (item 119, fix pass 11, bench run 45) — **`edhaCostShortfalls(list, balances)`** and **`edhaShortfallText(actorName, itemName, shortfalls)`** (both PURE, pinned in `tests/consume-shortfall.test.js`) + a `cosmere-rpg.preUseItem` **announcer**. The system DOES refuse an underfunded `use()` — `GENERIC.Warning.NotEnoughResource` → *"Cannot consume, not enough of resource"* (2.1.0 `index.js` ~L7120) — but that toast names neither actor, item, resource nor amount and leaves no console line, which is why The Reckoning's 3-Focus Unbreakable Line against a 2-Focus pool read as a dead button. The announcer says it in the engine's own words (actor · item · resource · need · balance · gap) and logs it, and **never returns `false`**: a use vetoed here would be one more silent no-op, which is the bug. `edhaConsumeCost` (the burst/takeover charger, `SYNCHRONOUS FORMULA & DICE EVALUATION`) is retrofitted onto the same sentence, so both refusal points read identically. |
...
- **Ranks**: talent formulas read `@skills.<color>.rank` — the build writes leyline ranks from
  `leylines` + role (minion 1 / rival 2 / boss 3, ruling 40). A block that states no `attributes`
  has them at 0, so its rolled colour tests run at +rank only; a block that states them (R-128 (a),
  2026-09-14) rolls attribute + rank — and, **R-137 (Ben, 2026-09-14): it is then on the PC attack
  model**, its weapons deriving attack AND damage from attribute + skill rank exactly as the system
  rolls a PC's item (the flat `attack` / `1d6+N` shape is a gate error there — see
  `.claude/skills/bestiary-forge/STANDARD.md` §3 "the attack model" and `scripts/validate-adversary-model.js`).
