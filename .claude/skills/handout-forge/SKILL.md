...
session-zero sheet). That 2026-07-15 build also hand-wrote `EDHA_PLAYER_PRIMER.html`; on
`main` that file is instead generated by `scripts/build-player-primer.js` from
`EDHA_PLAYER_PRIMER.md` and must never be hand-edited — this skill's HTML-by-hand step
applies to a new handout like the one-pager, not to the primer. Every rule below was earned
by a real failure during that build:
