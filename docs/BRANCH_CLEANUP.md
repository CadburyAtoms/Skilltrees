...
Written for Ben. **66 remote branches besides `main`.** Verdicts: **61 SAFE to delete**,
**2 for your decision**, **3 KEEP** (they are the only copy of work that queued items re-land).
...
## KEEP — source for a queued item (3)

| Branch | Last commit | Why |
|---|---|---|
| `claude/handout-forge-skill` | 2026-07-15 `fbc8e20` | PR #93. The only copy of the handout-forge skill and the session-zero one-pager — **item 33** re-lands them. Deleting this branch auto-closes #93. |
...
git push origin --delete pm/fix-pass-2-run26
```

GitHub's *Branches* page (`…/Skilltrees/branches/stale`) works too, one click each. After
deleting, PRs #93 and #103 stay open until you close them or delete their KEEP branches once
items 33 and 34 have landed.
