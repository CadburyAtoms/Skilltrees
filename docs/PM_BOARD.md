...
| 2026-09-04 21:40 | Dashboard on the phone (Ben's request; built directly, no worker) | fable | ~70 min | — | `build-dashboard.js` exports its tab model + `mobileSnapshot()`; `pm-state.js --dashboard-dir` shards it (`dash/index` + `dash/c0..c2`, seeded); page gained Snapshot + Dashboard and was republished at the same URL; parity test pins phone rows == desktop rows. **Not merged**: on branch `claude/in-app-dashboard-snapshot-ecwudz`, needs a PR + Ben's merge; the PM pushes `dash/*` after every merge from now on (skill §"The mobile board") | branch |
