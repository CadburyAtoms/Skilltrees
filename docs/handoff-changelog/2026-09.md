...
## 2026-09-15 — Item 169: thirteen talents lose their decoy item-level damage formulas — the sweep the item invited found five more beyond the eight named (DATA — REBUILD leyline + deity + ⟳ Sync Talents)

Item 169 (bench run 49a) named seven talents plus Cascading Failure (widened from `edha-triggered-effect{kind:damage}` to `edha-detonate-list`) carrying an item-level `system.damage.formula` beside the engine rule that already deals their damage — the system rolls the item formula as a real DamageRoll with Apply Damage buttons on every use, pass or fail, on top of what the talent's own rule applies (item 141's defect, recurring in Red/Chaos/Power/Destruction). A full sweep of all 21 `data/authored/*.json` files (every talent whose `damage.formula` is set — 52 of them — cross-checked against every damage-dealing handler family: `edha-triggered-effect` kind `damage` and kind `damage-aoe`, `edha-detonate-list`, `edha-damage-bonus`) found five more with the identical, provable shape, taking up the item's own invitation ("widen the sweep… add any talent it finds"): **Necrotic Cascade** (Death) and **The Unmooring** (Destruction) share their tree-mates' handler types (`damage-aoe`, `edha-detonate-list`) on the same engine code path already audited for the named talents — Necrotic Cascade's own rule comment even read "the formula duplicates the talent's own damage line… edit BOTH if you retune it," never noticing the card-button implication; **Withering Touch** (Death), **Momentum of Victory** and **Warlord's Advance** (Power) use `edha-damage-bonus` — the exact handler family item 141 already fixed on Predatory Strike, same "arm via self-status, rider deals damage on a later hit" shape, same "don't also roll the card's damage by hand" card text. All 13 rules were confirmed against the engine's own code (`edhaTrigSpecFromCfg` reads the RULE's own `formula`; `edhaResolveCharges` reads each Charge's own stored formula plus the rule's `bonusFormula`/`doubleCaughtFormula`, never `item.system.damage.formula`; `edha-damage-bonus`'s `amountFormula` is a required, independently-read field) to never fall back to the item formula, so every one is a pure blank — no formula needed moving into a rule. `data/authored/leyline-red.json` (Volatile Strike), `deity-chaos.json` (Cascade Collapse, Entropy Strike, Isolating Pressure, Isolating Ruin, Unravel Everything), `deity-power.json` (Unstoppable Advance, Momentum of Victory, Warlord's Advance), `deity-destruction.json` (Cascading Failure, The Unmooring) and `deity-death.json` (Necrotic Cascade, Withering Touch) each now carry `damage: {formula: null, type: null}` — the same two-key shape every other non-rolling talent in these files already uses; source prose untouched. `Set Charge` (deity-destruction.json) was swept and explicitly left alone: its formula is copied into the Charge ledger at PLACEMENT by `edhaSetChargeMarker`, a genuinely different fix (move the formula into the `edha-zone` rule) out of this item's scope. **Pack parity** (scratch `edha-leyline` + `edha-deity`, before = pre-fix data, after = this change): item/folder counts identical both builds (136/20, 110/10); exactly 13 documents differ, all and only in `system.damage`, zero collateral. `node scripts/validate-packs.js` on the rebuilt scratch packs: VALIDATION PASSED. `docs/analysis/bestiary/CENSUS.md` re-stamped (`node scripts/bestiary-census.js --write`) — its authored-overlay hash moves since the census stamps all of `data/authored/*`, block/rule counts unchanged (54/188). REBUILD leyline + deity + ⟳ Sync Talents. 🤖 rows **169-RED**, **169-CHA**, **169-PWR**, **169-DES**, **169-DTH** filed, one per tree touched (`EDHA_FOUNDRY_TEST_CHECKLIST.md`). Found, not fixed: `module-src/scripts/engine/42-chaos.js`'s header comment ("all 9 talents keep events:{}… read item.system.damage.formula") is stale since the 07-24p preUseItem-takeover migration gave five of those talents real events and their own inline formulas — reported for the PM; engine changes stayed out of this item's scope since no rule fell back to the item formula.
