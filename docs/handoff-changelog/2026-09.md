...
The dated session deltas of 2026-09 (135 deltas, 2026-09-04 → 2026-09-13), newest first,
...
**Defects filed: items 116, 117, 118.** 116 — four of the new cue cards print the talent name twice
(`edha-note` already prefixes it; exactly four across all of `data/authored/`, all from PR #329).
117 — `The Reckoning`'s Unbreakable Line consumes **3 Focus** against a pool that maxes at **2**, so it
can never be paid, and a failed consume is a **silent no-op** (no card, no notification); the Crownox
Ring is fine. 118 — a gated `edha-regen` tick still prints its static "regains 5 HP" note beside "no
healing lands", because the rule's own `note` wins over the composed line. **One new ruling, R-112** —
117's cost/pool half is a design call (raise the pool to 3, make it free, or drop the cost to 2), and
