# EDHA handoff changelog — 2026-07

The dated session deltas of 2026-07 (42 deltas, 2026-07-01 → 2026-07-28), newest first,
...
- **R-58** — a scene reset now skips any actor fighting in another combat, and "another combat" means any that still EXISTS, started or not. Fail-safe direction chosen deliberately: a wrong skip leaves stale state you can clear; a wrong clear destroys a live encounter.
- **R-59** — a failed chat-card button now raises an error toast. The feel call is Ben's.

### Bug root causes

- **`ev.currentTarget` is null after an `await` — ONE bug, and the sweep is the evidence.** `Event.currentTarget` is set *only while the event is being dispatched*; an `await` in an async listener returns control to the browser, dispatch completes, and the browser nulls it. `edhaUpkeepInvClick` captured the actor uuid correctly on its first line and re-read `ev.currentTarget.dataset.item` one line later, after `await fromUuid(...)` — TypeError on every click, for every user, swallowed by the outer catch into a console line nobody was reading. **Family swept: 35 occurrences on 34 lines (L6245 carries two) across 33 chat-card handlers. 33 capture synchronously and are correct; both remaining reads are on the one reported line.** ⚠️ **Two sites look identical to the bug and are NOT** — `edhaIllusionRetestClick` L6109 and `edhaUpkeepInvClick`'s own first line read `currentTarget` *inside the await's own operand*, which is evaluated **before** suspension. A naive "any earlier await" check calls both bugs; that distinction is what pass 19 had to model, and it is why the honest answer to "one bug or a family?" here is **one**.
- **The swallowing catch was the deeper defect, and it is now fixed as a family.** All 33 outer click-handler catches ended in a bare `console.error`. At the table that is indistinguishable from "the button does nothing" — which is precisely how a hard TypeError was recorded as a silent no-op across **four** bench runs. New primitive **`edhaClickFailed(what, e)`** logs *and* raises `ui.notifications.error`. Bounded on purpose: the ~270 **inner** defensive catches guard genuinely optional work and stay quiet, because a toast that fires routinely trains the table to ignore it.
- **`deleteCombat` sweeps were unscoped — 20 of 24, and this one destroys player data.** `deleteCombat` and `combatTurnChange` hand you the combat; the sweeps ignored it and iterated `game.actors` / `canvas.tokens.placeables` / `game.scenes`. With one combat that is correct; with two, the end of one encounter deletes the other's live state — run 23 watched `lists.covenants` vanish off an actor in Ben's running combat, and runs 19/20 watched `trigRound` land on Corvaine and Stonebound from bench turn ticks. ⚠️ **The fix is NOT "sweep only this combat's combatants", and that is worth reading before anyone "improves" it.** The wide enumeration is deliberate and pinned — `tempHp` is meant to reach adversaries, summons and unlinked token actors that never rolled initiative, and `tests/temphp-scene-reset.test.js` asserts a token-only non-combatant still clears. Narrowing would break intended behaviour *and* that test. The invariant actually wanted is narrower and costs nothing: **ending combat A must not clear state on a combatant of a still-existing combat B** — so single-combat play is unchanged. New primitives `edhaCombatEndGuard` / `edhaStillFightingElsewhere`, threaded through **21** sweeps. The 2 registrations that still take no combat argument are correct: the in-memory watch-budget clear, and the isolated-marker sync, which now recomputes from **every started combat** instead of assuming `game.combat` is the only one. Un-attributable world props (charge templates, Fate markers/Regions) carry no owner, so those sweeps are skipped wholesale while another combat exists.
- **Pre-existing, found while gating: two tests were reading commented-out code as live.** JS `.` does not match `\r`, so `/\/\/.*$/` and `/^\s*\/\/.*$/gm` strip **nothing** on a `core.autocrlf=true` checkout — which is Ben's. `tests/terrain-ownership.test.js` therefore failed `2 !== 1` on a **clean tree** (a prose mention counted as a write), while CI on Linux/LF stayed green. A false red that fires only on the local pre-commit hook is the worst possible place for one. Verified pre-existing against base commit `8c16b29`: 1 hit on LF, 2 on CRLF. Both tests now normalise at the read.

### New REUSABLE primitives
- **`edhaCombatEndGuard(endedCombat)`** → `Set` of actor `id`s/`uuid`s fighting in some *other* still-existing combat. Build once per sweep.
- **`edhaStillFightingElsewhere(actor, guard)`** → the per-actor test; `continue` on true. Matching is by `id` **or** `uuid` — per the twin-actor family, two unlinked tokens off one prototype share an `id`, so a sibling of a real combatant is also skipped. Over-skipping, on purpose.
- **`edhaClickFailed(what, e)`** → the outer catch for any chat-card click handler: console *and* toast.
- **`lint-refs` pass 19** — no `currentTarget` read after an `await`. Built rather than declined, with numbers: corpus **35 across 33 handlers**; **0 false positives** where the naive form gives **2**; the defect is unreviewable, unobservable at runtime and cost four runs to localise. Reuses the existing nesting-aware `blankStringsAndComments` rather than adding a second blanker, and carries the house "the scan rotted" self-checks (blanking must preserve length, 500+ bodies must delimit, 20+ reads must be found, every read must resolve to a function). **Mutation-verified both ways:** reintroducing the historic bug is caught at the exact line naming the exact await, and a fresh instance planted in `edhaDesignateClick` is caught too.

### Verification
Every fix ships a pinned, **mutation-verified** regression: `tests/current-target-after-await.test.js` (a fake event that nulls `currentTarget` when the `fromUuid` stub is awaited, as the browser does — restoring the old read fails it with `4 !== 3`, run 23's exact symptom) and `tests/cross-combat-scope.test.js` (every case asserts **both** directions; neutering the guard fails all three negative controls with the ledger reading `undefined` while every positive control and `temphp-scene-reset` stay green). 408 tests pass, now on a **CRLF** working tree, which is the real-world state.

### Known limits / couldn't self-verify (no Foundry here)
- ⚠️ **The `deleteCombat` fix is verified in the harness, not at a live table** — three 🤖 re-test rows carry the paired controls, including the load-bearing "single-combat play is unchanged" one.
- ⚠️ **R-58's fail-safe edge is untested in the wild:** a stale un-started combat sitting in the sidebar containing a PC will now stop that PC receiving scene resets until it is deleted. Deliberate, but worth knowing.
- The `Determined` effect left on `Bench — Order` in run 23 is **not** attributable and was not chased — its start snapshot was lost to a page reload, as run 23 recorded.
